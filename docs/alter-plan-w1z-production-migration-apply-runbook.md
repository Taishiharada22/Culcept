...
| # | file | 役割 | size | 冪等性 |
|---|------|------|------|--------|
| 1 | `20260430100000_external_anchors.sql` | Plan tables (`external_anchor_sources`, `external_anchors`) + RLS + indexes + CHECK 制約 | 13.7 KB | ✅ `CREATE TABLE IF NOT EXISTS` |
| 2 | `20260519100000_create_external_anchor_bundle.sql` | W1-Y RPC function (`create_external_anchor_bundle()`) | 7.8 KB | ✅ `CREATE OR REPLACE FUNCTION` |
...
- 内容は idempotent (`CREATE TABLE IF NOT EXISTS` / `CREATE INDEX IF NOT EXISTS` / `CREATE POLICY IF NOT EXISTS` 等)、複数回 apply しても安全
...
| **β user 招待開始** | `docs/alter-plan-beta-readiness.md` に従い CEO 操作 |
...
- `docs/alter-plan-home-swipe-full-plan-pane-phase1-complete.md` (本 PR の対、Phase 1 完了報告)
- `docs/coalter-supabase-ref-canon.md` §1 (Production Supabase ref = `aljavfujeqcwnqryjmhl`)
