...
> ⚠️ **本 §2.1 の sub-phase 構成は Phase E-2-0 sequencing plan で修正されている** (E-3 の構成要素のうち kill switch / Sentry baseline / drill は E-2-α gate の prerequisite となるため、E-2-1 / E-2-3 に分割移管された)。最新の sub-phase 構成は `docs/coalter-aoo-phase-e2-0-sequencing.md` §4.2 を参照。E-3 の残部分は **E-3'** (post-α observability、reflection text catalog 等) として再定義。
...
### 3.1.1 E-1 の scope 制限 (CEO 補正 2026-05-19、Phase E-0 review 時)

- **E-1 は canary visible smoke**、Production rollout ではない。Production user に影響 0
- E-1 では **Production env を絶対に触らない**。`NEXT_PUBLIC_COALTER_MIRROR_FORCED_CANARY_ENABLED=true` も **branch-scoped Preview (canary branch) でのみ** 投入、Production / all-Preview には絶対投入しない (CEO 補正)
- E-1 で visible Mirror UI を観測する目的は **render の構造的成立確認**であり、user interaction (close/sleep button click) は **実施しない** (CEO Q4)
- E-1 PASS の効果は **E-2 着手 gate の 1 条件達成のみ**。E-1 PASS = Production rollout 開始ではない (E-2-α 着手にはさらに §3.9 / §9.2 の全 condition を要求)
...
### 3.9 E-2 着手の gate condition (CEO 補正 2026-05-19、Phase E-0 review 時)

E-2-α (Production env touch) に進むには、E-1 完了状態として以下 **3 つすべてが揃う**ことが必須:

| # | gate | 達成方法 |
|---|---|---|
| 1 | **E-1 visible smoke PASS** | §3.6 PASS conditions 7 項目すべて達成 (CEO 実機判定) |
| 2 | **canary cleanup 完了** | §3.8 cleanup 全 step 実施 (env 6 件削除 + branch + worktree + 不変確認 + 最終 D-1 reverify) |
| 3 | **E-1 close 記録着地** | E-1 close docs PR (Phase D D-5 pattern 踏襲、`docs/coalter-aoo-phase-e1-close.md` or decision-log entry) が main に merged |

→ この 3 つが揃わない限り **E-2-α 着手不可** (Production env touch しない)。本 gate は §9.2 (Production env touch を許容する 5 condition) と組み合わせて、Production env 緩和の構造的安全を保つ。
...
E-2 着手前に以下を **すべて満たす** (CEO 補正 2026-05-19 で gate を 5 → 8 に拡張):

| # | condition | 達成判定 |
|---|---|---|
| 1 | E-1 visible smoke が 3 gates 全 PASS、reflection-only canon 遵守確認済 | §3.6 PASS conditions、CEO 実機判定 |
| 2 | **E-1 canary cleanup 完了** (env 6 件削除 + branch + worktree + 不変確認 + 最終 D-1 reverify) | §3.8 cleanup all steps |
| 3 | **E-1 close 記録 main 着地** (`docs/coalter-aoo-phase-e1-close.md` or decision-log entry) | §3.9 gate condition #3 |
| 4 | kill switch L1 + L3 が **drill 済** | §5.3 / §11.7 failure injection drill |
| 5 | allowlist 実装 (Option A env-based) が runtime code に着地、tests PASS | E-2-α 着手時の別 PR (allowlist 実装) merged |
| 6 | reflection-only canon CI test が main に着地 | E-1 と同時着地 (CEO Q5) |
| 7 | **CEO 直接承認** (Production env 投入計画を明示承認) | PR description + AskUserQuestion 等 |
| 8 | Sentry baseline error rate 記録済 | E-3 monitoring 準備の一環 |

→ 8 condition すべて揃って初めて Production env touch を許容 (Mirror Channel 関連 2 key のみ、§4.5 表)。
...
## §12. CEO answers (Q1-Q10、2026-05-19 approved)

CEO 直接判定済 (本 PR review 時、Phase E sub-phase 設計に永続反映):

| # | 質問 | **CEO answer** | 根拠 / 制約 |
|---|---|---|---|
| Q1 | E-2 で Production env を touch する canon 緩和を承認するか | **YES** | E-2 で Mirror Channel 限定の Production env 緩和を承認。ただし **2 key のみ** (`NEXT_PUBLIC_COALTER_MIRROR_CHANNEL_ENABLED` + `NEXT_PUBLIC_MIRROR_ALLOWLIST_USER_IDS`)、**allowlist 必須**、**kill switch 必須** |
| Q2 | allowlist 実装方法 (Option A / B / C、§4.4) | **Option A** (env-based) | 採用確定 |
| Q3 | kill switch L1 + L3 両方実装するか、L1 のみで足りるか | **L1 + L3 両方必須** | env kill switch (L1) と runtime kill switch (L3、Supabase `app_settings`) の両方を必須化 |
| Q4 | E-1 visible smoke で close/sleep button click を試すか、表示確認のみか | **表示確認のみ** | E-1 では close/sleep button は **押さない** (state 変化させない、最小 smoke 維持) |
| Q5 | reflection-only canon CI test (§11.1) を E-1 と同時に着地させるか | **YES、E-1 と同時** | reflection-only canon CI test は E-1 PR 内で着地、Phase E 全期間の safety net として運用 |
| Q6 | user onboarding tooltip (§11.6) を E-2-β で導入するか | **YES、E-2-β 以降** | E-2-α (CEO のみ) では tooltip 不要、E-2-β で invited user 向けに導入 |
| Q7 | failure injection drill (§11.7) を E-2-α 前に実施するか | **YES、E-2-α 前必須** | drill PASS なしで Production env touch しない (gate condition) |
| Q8 | reflection text catalog (§11.5) を E-3 で実装するか、Phase E 後継 phase に deferred するか | **YES、E-3 で実装** | Production rollout の audit trail として E-3 で着地 |
| Q9 | A/B rollout (§11.4) を E-2 内部で実施するか | **Deferred** | Phase E では見送り (小規模で統計的有意性なし)、後継 phase で再検討 |
| Q10 | Phase E 全体期間 (3-4 週間 目安) は許容範囲か、加速 / 減速の希望あるか | **3-4 週間目安で OK** | ただし **PII leak / user discomfort / false positive 観測時は即停止** (期間より安全優先、CEO 補正) |

### 12.1 CEO 追加補正 (Phase E 全期間遵守、本 PR review 時に明示要求)

- **「Production env touch」は E-2 以降**、E-1 では Production env を **絶対に触らない** (§3 / §9 で明示済、§12.1 で再宣言)
- **E-1 は canary visible smoke** であり、Production rollout ではない。canary branch-scoped Preview のみで完結
- E-1 で `NEXT_PUBLIC_COALTER_MIRROR_FORCED_CANARY_ENABLED=true` は **branch-scoped Preview でのみ** 投入、Production / all-Preview には絶対投入しない
- E-1 で close/sleep button は **表示確認のみ**、click しない (Q4 と同義、明示反復)
- **E-2 に進むための gate condition**: E-1 visible smoke **PASS** + canary cleanup 完了 + E-1 close docs 着地 — この 3 つ揃って初めて E-2-α 着手可 (新規追加 condition、§9.2 にも反映)

### 12.2 安全優先の停止 trigger (CEO Q10 補正、Phase E 全期間)

以下を観測したら **期間より優先して即停止** + 原因 audit + kill switch 発動:

- PII leak 1 件以上 (Mirror text に他 user の data 混入等)
- user discomfort 報告 (invited user からの不快感表明、SleepUIToggle 高頻度 click 等)
- false positive (Mirror engine が誤って visible candidate を出す、過剰 frequency)

→ 観測時、E-3 kill switch L3 (Supabase runtime) を最優先発動、L1 (env removal) で恒久 OFF、原因解析後に CEO 判断で再開可否を決定。
