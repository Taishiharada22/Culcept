...
### A-4b 実装済（2026-06-01・flag + PlanClient 最小接続 + 保存）

flag 方式: CEO 決定で **env（server-only）+ page.tsx prop 経由**（`PLAN_FLAGS` は client 直読み不可 = `featureFlags.ts:32` の server-only ルール準拠。`homeSwipeEnabled` と同方式）。

| 追加/更新ファイル | 役割 |
|---|---|
| `lib/plan/featureFlags.ts`（本流・加算） | `composeTimelineEnabled = process.env.PLAN_COMPOSE_TIMELINE_ENABLED === "true"`（**既定 false**・server-only） |
| `app/(culcept)/plan/page.tsx`（本流・server） | `PLAN_FLAGS.composeTimelineEnabled` を読み取り `PlanClient` に prop（CEO 承認の3つ目の本流） |
| `app/(culcept)/plan/PlanClient.tsx`（本流・最小分岐） | prop 受領 + 対象日/既存block memo + `openAdd` mount を `shouldUseComposeSheet` で分岐（**OFF=既存 AddAnchorModal 完全不変**／ON=AddAnchorComposeContainer） |
| `lib/plan/compose/composeGate.ts`（新規） | `shouldUseComposeSheet`（単一 gate・pure） |
| `.../compose/anchorsToTimelineBlocks.ts`（新規） | 当日 `anchorsForDay()` → 既存 read-only block（pure・表示用 end 既定） |
| `.../compose/AddAnchorComposeContainer.tsx`（更新） | `dateISO`/`onSaved` 追加・完了 = A-4a converter → `createAnchorBundle` → onSaved・日跨ぎ警告 notice |
| `.../compose/AddAnchorComposeSheet.tsx`（追加 optional） | `notice` slot |
| tests ×2 新規（gate / adapter）+ container test 更新 | compose+geometry 計 **93 PASS** |
