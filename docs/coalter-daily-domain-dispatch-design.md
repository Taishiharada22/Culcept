...
### §11.3 期待される CoAlter 全体寄与 (CEO 2026-05-15 補正: design completion vs runtime completion 分離)

**重要 (CEO 補正反映)**: 本 PR は **docs-only = design completion only**。Daily × Domain runtime dispatch logic は **未実装** (DD1-DD6 は別 PR の future work)。下記の状態変化は **「設計上の未整理項目」** が減少することを指し、**実装完了 (runtime completion) ではない**。

PR #122 §6 集計 (CoAlter 全体 **設計上の未整理** 14 件) のうち、本 PR (design only) で減少するもの:

| 未整理領域 | 設計レベル状態変化 | runtime impl 状態 |
|---|---|---|
| Daily × Domain dispatch **design** | ❌ → ✅ (本 PR で design completion) | **runtime 未実装** (DD1-DD6 別 PR で着手) |
| Daily mode domain body 機能化 | ⚪ → ⚠ 設計上の道筋確定 | runtime 未実装、各 Domain impl 完成度に依存 |
| PresenceMode daily UI/state 機能薄 | ⚪ → 設計上は発火可能 path 明示 | runtime 未実装 |

→ **設計上の未整理項目 14 件 → 12 件** (Travel 完了で 11 件 → Daily 完了で 9 件 → Gap 4 D7 完了で **7 件**)。**ただし runtime 完了件数は不変** (impl は全て future PR 待ち)。
