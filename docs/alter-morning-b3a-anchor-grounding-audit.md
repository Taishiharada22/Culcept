...
### 9.1 B-3a: audit doc only (本 doc) ✅ 完全 docs only

**CEO 2026-05-03 確定**: B-3a では `classifyLabel` helper も含めて **実装変更ゼロ**。
- 本 doc 起草 + CEO/GPT review + 11 章最終方針反映
- 実装は **一切なし** (= helper / type / route / reducer / test も追加禁止)
- audit doc final 化 → admin merge

### 9.2 B-3b: candidate presentation (= candidate 提示まで)

**CEO 2026-05-03 確定**: B-3b は **candidate presentation までで切る**。selection 後の更新は B-3c。

scope:
- **最初の commit**: `classifyLabel` pure helper (= B-3a で延期、ここで実装)
- `PresentationTarget` discriminated union 導入 (= sentinel と **併存**、廃止しない)
- `dialogReducer` の `SEARCH_CANDIDATE_SELECTED` 拡張 (target field 追加)
- `placesHandoffOrchestrator` で journey_origin target に対応
  - `userOverrideOriginLabel` の classification 別 grounding 起動条件 (= Q1 確定方針反映)
  - public_poi_proper_noun: 同 turn で candidate 提示
  - generic_category: anchor/chain 待ち、即 grounding しない
  - private_semantic / ambiguous: Places API に流さない (= grounding 試みず)
- 既存 `PlaceCandidatePicker` 流用 (= UI は変更なし、上層で target 切り替え)

**B-3b ではやらない (= B-3c に分離)**:
- ❌ selection 後の `known_exact` 昇格 logic
- ❌ travel segment 生成までの確認
- ❌ `applyPlaceSelectionByTarget` 等の event/anchor dispatch helper

### 9.3 B-3c: selection 後の known_exact 昇格 + travel 生成

**CEO 2026-05-03 確定**: B-3c で初めて anchor 更新と travel 生成までつなぐ。

scope:
- `applyPlaceSelectionByTarget` (or new helper) を追加
- selection route で `target.kind` 別の dispatch
  - `event_where`: 既存 `applyPlaceSelection` で event 更新
  - `journey_origin`: `journeyOrigin` を `known_exact + coordinates` に昇格
  - `journey_end`: `journeyEnd` を同様に昇格
- `events` 経路は既存 `applyPlaceSelection` 維持 (= regression 不変)
- travel segment 生成確認
- 失敗 path:
  - candidate 0 件 / cancel → `known_label_only` 維持、travel 不生成 (= Q4 確定方針)
  - 自動再 ask **しない** (= 同 session 内でしつこく再提示しない)

### 9.4 B-3d: derivedFrom / AnchorSource type-level 分離 + sentinel 廃止

**CEO 2026-05-03 確定**: sentinel 廃止は **本 PR (B-3d) で初めて検討**。B-3b/c では併存。
...
- `PLAN_ORIGIN_SENTINEL_EVENT_ID` 廃止検討 (= targetKind で代替できる前提が固まった後)
- backward compat: 旧 session 読み取り経路の互換維持 (= migration 必要)

### 9.5 各 PR のサイズ目安 (= CEO 確定 scope 反映)

| PR | 推定行数 | 影響範囲 |
|----|---------|---------|
| B-3a | +603 (本 doc のみ、helper 一切含めない) | docs only |
| B-3b | +500-700 | classifyLabel + dialog/types, reducer, placesHandoff*, hook |
| B-3c | +400-600 | selection route, applyPlaceSelectionByTarget, legacyAdapter |
| B-3d | +300-500 | journey/anchorState type 分離 + sentinel 廃止 + migration |

合計 ~1800-2400 行を 4 PR に分割 (= 1 PR 700 行未満を維持)。
...
## 11. 最終方針 (CEO/GPT 2026-05-03 確定)

### 11.1 grounding 起動タイミング — **classification 別**

CEO/GPT 確定方針:

| Classification | 起動タイミング | 備考 |
|----------------|--------------|------|
| `public_poi_proper_noun` | **同 turn で candidate UI 提示可** | 例: 「サドヤ」「東京駅」 |
| `generic_category` | **原則すぐ grounding しない** | anchor / chain / 地域情報を user が追加した時だけ候補検索 |
| `private_semantic` | **Places API に流さない** | known_label_only 維持 |
| `ambiguous_or_demonstrative` | **grounding しない** | known_label_only 維持 |

**重要規律 (CEO/GPT 強調)**:
> generic_category の **「ホテル」だけで即「どのホテル？」と聞くのは禁止**。
> 質問連発を避け、known_label_only のまま保持する。

### 11.2 `PLAN_ORIGIN_SENTINEL_EVENT_ID` の扱い — **B-3b/c は併存、B-3d で廃止検討**

CEO/GPT 確定方針:
- **B-3b / B-3c では `PresentationTarget` と sentinel を併存**
- **B-3d の type-level 分離で sentinel 廃止を検討**
- **B-3b でいきなり廃止するのは禁止** (= migration リスク)

### 11.3 saved places の扱い — **B-3 では「自宅」のみ**

CEO/GPT 確定方針:
- **B-3 では「自宅」のみ扱う** (`userHomeLat/Lng` がある場合のみ解決可能)
- **「会社」 / 「友達の家」 / 「実家」 は別 PR** (= `saved_places` table が必要)
- **private_semantic label は known_label_only のまま維持**、Places API には流さない

### 11.4 失敗 path の再質問 — **自動再 ask しない**

CEO/GPT 確定方針:
- **candidate 0 件 / cancel 時に自動再 ask しない**
- **known_label_only のまま維持し、travel segment は生成しない**
- **同 session でしつこく再提示しない**
- UI hint は許容するが、質問アプリ化させない

### 11.5 同 turn 質問連発 — **classification 別 (Q1 と整合)**

CEO/GPT 確定方針:

| Classification | 同 turn 連発 |
|----------------|-------------|
| `public_poi_proper_noun` | ✅ candidate UI 提示は同 turn 可 |
| `generic_category` | ❌ 「ホテル」 → 即「どのホテル？」 は **禁止** |
| `private_semantic` / `ambiguous` | ❌ 同 turn grounding しない |
...
## Approval status (CEO 2026-05-03 確定)

✅ **B-3a audit doc は CEO/GPT 確定方針を反映済み (= final)**

**CEO 判断**:
- B-3a は **完全 docs only** (= `classifyLabel` helper も含めて実装ゼロ)
- `classifyLabel` は **B-3b の最初の commit** に回す
- B-3b は **candidate presentation までで scope を切る**
- selection 後の `known_exact` 昇格 + travel segment 生成は **B-3c に分離**
- 11 章は CEO/GPT 確定 5 件を反映 (= 私の推奨が確定された)

**次のステップ**:
1. PR #64 を admin merge (= docs only 例外処理、Vercel failure 許容)
   - 通常 PR で Vercel failure を無視する前例にはしない
2. B-3b 実装に進む (= `classifyLabel` 最初の commit + candidate presentation のみ)
