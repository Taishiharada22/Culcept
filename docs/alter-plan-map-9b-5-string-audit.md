...
## 7. 🚧 Deferred 評価 (= CEO 補正 2026-05-25 反映)

**CEO 補正 1 件**:
> 「旧 path だから defer」 ではなく、 **「new path の通常体験に出るかどうか」 で判断**
> - flag ON の通常到達パスで見えない → closeout defer でよい
> - flag ON の loading / key-missing / fallback で実際に見える → closeout 送り固定ではなく後で整理対象に戻す

以下は newMode 時にも一部表示されるが、 PlanMapView 内 shared (= OFF path 影響) のため 9b-5 で触らない。
ただし **flag ON 通常体験で実際に見えるか** で再評価する:
