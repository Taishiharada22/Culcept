...
- **改訂 v0.4（2026-06-12・W0）**: CEO visual policy 緩和（2026-06-11・Session B B4/B5/B13 裁定）の正本追従。**表示層のみ解除**（採点不能・正本保存禁止は不変）。新原則 =「**数値は具体的に。ただし嘘でないこと**」（出自 5 分類は visual-contract §0.1 が正本）。**スコープ = Alter タブ表面限定 — A3 What-if / N-3 表面の数字・語彙禁止（別の CEO 確定決定）は不変**（§4.2 の A3 連動文言を分離）。あわせて Stage 1 前契約 2 件を v0.4 で確定: `MomentStateV0.timeBucket: TimeBucket | "unknown"`（placeholder 廃止・実装 W2）/ dailyModeHint 供給契約（§3.3）。B5（ヘッダー削除）/B13（操作盤化）/「今日の推移予測」を §7.2 に反映
...
| b-2 | 採点対象と禁止 | 連続値（48% 等）は**採点不能 → 正本保存禁止**（表示は v0.4 緩和により Alter タブ限定・出自付きで可 — visual-contract §0.1）。採点対象は enum 帯の見立て 3 個（energyLevel・recoveryNeed・dayFeasibility）と drift。focusReserve / emotionalReserve / outingTolerance は補正タップによる採点補助のみ。数値は導出中間体としてのみ許可（§4） |
...
| b-5 | Alter タブ vs Dock | **前回の Dock 推奨を撤回し、専用タブ案を採用**。根拠 3 点: 情報量・N-3 適合性・進行中 dogfood の非汚染。v0.1 のミニ Composer は **v0.4（B13）で操作盤の状態入力スリットへ**（§7.1-5。既存 route source:"plan" への入口という性質は不変） |
...
| b-7 | 人体バッテリー | v0.1: **3 系統（脳=集中/心臓=心の余力/体=体力）が人体内部を巡る表現**。外出耐性は周辺カードへ。visualFill 連続値可・数値は出自付きで表示可（v0.4 緩和・Alter タブ限定）・「見立て」バッジ・根拠チップ・ワンタップ補正・unknown 正直表示（数値禁止）。「今日の開始残量」不採用（§9 + visual-contract） |
...
- **導出するもの（速い）**: MomentState — 開いた瞬間に、現在時刻 × DayGraph × DayStateRecord から純関数で計算。保存しない。push しない（N-3）。**型契約（v0.2 で凍結。CEO 提案 11 フィールドを基本採用 + 3 追加 + 語彙整合 1）**:

```ts
type MomentStateV0 = {
  nowHHMM: string;                          // 注入された now（strict "HH:MM" — DayGraph の時刻形式と同じ）
  timeBucket: TimeBucket | "unknown";       // 既存 7 値（dayGraphTypes.ts:56）。**v0.4 契約改訂（Stage 0 受領監査 3）: parse 不能時は placeholder でなく "unknown"**（実装は W2。Stage 0 実装の暫定 placeholder は W2 で廃止）
  nowSegment: { kind: "event" | "travel" | "gap";
  ...
  receptivity: "silent" | "on_open" | "unknown";   // 既存 DeliveryMode（receptivity-gate.ts:25）と同語彙の「上限」表現。push 系は B2/R6 まで値域から除外
  interventionWindow: "open" | "narrowing" | "closing" | "closed" | "unknown";
  isNightCheckWindow: boolean;              // timeBucket ∈ {evening, night, late_night}（Night Check カード表示の駆動源）
};
```

導出規則（定数は named constant・fixture 必須・画面非表示）: timePressure = minutesUntilDeparture（無ければ minutesUntilNextFixedEvent）≤15 → high / ≤45 → medium / それ以上 → low / fixed event なし → low / DayGraph 欠如 → unknown。currentMode = event 内 → in_event / 出発期限超過 or 開始 30 分前以内 → pre_event / 終了 20 分後以内 → post_event / evening・night の gap 内 ∧ 余白あり → evening_recovery / その他 → open。interruptibility = in_event → low / pre_event ∧ pressure high → low / pre_event → medium / post_event → medium / open・evening_recovery → high。receptivity = interruptibility low → silent / それ以外 → on_open。interventionWindow = 出発まで >45 → open / 16-45 → narrowing / 1-15 → closing / 超過 → closed / fixed event なし → open。

**境界 2 点**: (i) timePressure / interruptibility / receptivity / interventionWindow の v0 の消費者は**表示選択のみ**（どの事実注記を出すか・Night Check を出すか）。proactive 配信判定への使用は B2/R6 gate 後。(ii) MomentState は保存も採点もしない（瞬間状態の actual は取得不能・取得しようとすると EMA 負担違反）。CEO 原案の `receptivity: "only_short_prompt"` は採用せず — 既存 DeliveryMode と二重語彙になるため値域を {silent, on_open} に整合し、「短い一言のみ」は interruptibility=low で表現する（語彙規律 §3.1。CEO 判断で原案復活も可）。
...
| ④Alter がセンサー | チップ + 状態入力スリット（B13）**入口のみ。センサーとしては未完**（過剰に言わない） | Stage 1.5: 入力 → DayStateRecord 構造抽出契約（extractDailyGuidanceFrame パターンの転用設計） |
...
type DayStateRecordV0 = {
  ...
  facts: {
    ...
    weather: { condition: WeatherCondition; pop: number } | null; // 既存 union（weatherService.ts: sunny/cloudy/rainy/snowy）を import type
  };
...
| dailyMode | resolveDailyMode（既存）に facts を渡す。**v0.4 供給契約: Stage 1 配線では server 側（タブの data 取得層）が resolveDailyMode を実行し、`dailyModeHint` + `dailyModeHintConfidence`（resolveDailyMode 入力 ConfidentValue 群の min confidence）を併送する**（buildDayStateRecord への additive input・実装 W2-W3。固定 0.5 は廃止） | 入力の min confidence |
...
### 4.1 「集中余力 48%・体力 61%」は採点可能か → **不可能。よって正本にしない**（表示は v0.4 緩和で derived 層・出自付きに限り可）
...
| ユーザー表示（v0.4 改訂） | **数値は具体的に。ただし嘘でないこと** — 全数値に出自（visual-contract §0.1 の 5 分類: vm_derived / flow_derived / user_reported / mock_reference / unknown）。unknown は数値禁止（`—`/「まだ読めていません」。0% 禁止）。mock_reference は「参考値」明示 + 実測断定禁止 + 本番 activation 前再裁定。帯語 + 「見立て」バッジは維持。**スコープ: 本緩和は Alter タブ表面限定 — A3 What-if（数字/%/score/確率なし = a3-soft-connection-mini-design.md:31）と N-3 表面の禁止は別決定であり不変** | 「からだの余力: 少なめ（見立て）+ 32%」 |
...
禁止リスト（v0.4 改訂）: 連続値スコアの**正本化**（DayStateRecord は enum 帯のまま — 表示は出自付きで可）、collapseRisk 数値の正本化、軸由来の「性格だから」帰属、本人が入力していない感情状態の断定、出自のない数値・unknown への数値付与。
...
5. A.10「チャット中心にしない」とは両立する: タブの主役は「人体バッテリー + 今日の読み」（上段）。**v0.4 更新（B13 CEO 裁定）: 吹き出しの会話往復は廃止し、操作盤（cockpit input panel）の状態入力スリットとして実装**（Session B 最終デザイン 5bb868fd が正）。入口の性質は不変 — **既存 Alter route への入口**: POST body は既存型の `source?: string` でそのまま受領可能なため新 API 不要（"plan" 固有の route 分岐は追加しない。payload = `{ message, sessionId, source: "plan", mode: "warm" }`、`hooks/useAlterChat.ts` の軽量パターンを踏襲、セッションは `PLAN_ALTER_SESSION_KEY`（新設定数。Stage 1 で定義）で独立管理）。会話の往復表示はしない（対話は既存 Alter 面へ）。会話内容→DayStateRecord への構造抽出は後段（Stage 1.5 以降）。「Alter が住んでいる感」と状態の自然吸収の両方をこれで担保する。

### 7.2 タブ構成 v0（上から）

```
[Alter タブ]（ラベル案: "ALTER"。N-3 契約語彙「ALTER で見る」と同系）
1. ~~Alter ヘッダー~~【B5 CEO 指示で削除】alterMessage（見立て 1 行）は操作盤タイトル帯へ移動
...
4. 今日の推移予測（B5 改名・B4 採用。flow_derived 軸 + mock_reference 動態）← 旧事実帯は消滅・復旧/統合は Stage 1 判断（preflight）
...
7. 状態入力スリット（操作盤内・B13。送信は Stage 1 で既存 route source:"plan" へ）— 会話往復表示なし
...
1. **数値表示（v0.4 改訂）**: 旧条文は「数値解禁は採点履歴が成熟してからの CEO 判断」を将来余地として留保していた — **CEO がこの判断を 2026-06-11 に前倒し発動**（理由 = テスト時の成果物密度）。ただし条件付き: 出自宣言必須（visual-contract §0.1 の 5 分類）・正本保存禁止・採点対象外の数値は mock_reference 明示・unknown は数値禁止・**Alter タブ表面限定（A3/N-3 表面は不変）**。水位（visualFill）は連続値 0-1・帯語と「見立て」バッジは維持。
...
7. **不安増幅の防止（v0.4 精密化）**: **履歴トレンド**（過去推移の捏造 sparkline 等）・streak・他者比較・通知・赤色警告は引き続き禁止。**当日の推移予測グラフは緩和で許可**（flow_derived 軸 + mock_reference 動態・履歴なし）。now マーカーの 1 分更新は表示時刻の更新であり常時監視ではない。
...
- ❌ 出自のない数値・unknown への「0%」・履歴の捏造・「疲労に注意」「回復を推奨」（N-3）・「今日の開始残量」・実測断定（mock_reference の無宣言表示）
