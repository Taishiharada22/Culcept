...
> **CEO 補正 2026-05-26（2 回目、 さらに 3 回目）**:
> - 2 回目: 初版「Layer 1 = 6 件」を `.from()` 数で決定したが、 product 優先度 / replay 必要度を混同していた → 3 区分構造（L-A / L-B / L-C）に変更
> - 3 回目: 2 回目の「Frozen Compatibility Layer」表現は強すぎる。**「product 上の主役ではない可能性」 と「Aneurasync で未使用」は別**。現時点で「凍結確定」「未使用確定」とは言えない → **L-C を Unknown / Legacy-candidate に変更**
...
#### L-A: Runtime Active Layer（確定 2 件）

product 上 active core + 完全な CREATE TABLE + INDEX + RLS + POLICY 補完が必要。

| name | category | 最終更新日 | 用途 |
|---|---|---|---|
| `profiles` | core user | 2026-05-20 | stargazer / genome / my-page 現役利用 |
| `notifications` | core notification | 2026-04-19 + counselor C1-2/C7 関連 commit | Stargazer / Counselor の核心と直結 |

**合計: 2 件**（CEO 確定 2026-05-26）

#### L-B: Replay Blocker Candidate Layer（要 audit、 件数未確定）

L-A 以外で、 後続 172 migration の ALTER / FK / policy が前提とする table。Step 4 で audit:
- 既存 172 migration file 内の `REFERENCES "public"."xxx"` 全抽出
- 該当先が prod-only 154 件に含まれるか確認

完全補完（INDEX / RLS / POLICY 含む）が必要。

#### L-C: Unknown / Legacy-candidate Layer（要 7 軸 audit）

> ⚠️ **3 回目補正で「Frozen」表現を撤回**。
> .from() 累積数 と 最終更新日 だけでは「未使用確定」とは言えない。
> 以下の 7 軸 audit が完了するまで判定保留。**この間は削除も補完判断もしない**。

##### 候補（"主役ではない可能性が高い" 段階、 未確定）

| name | 観察 | 状態 |
|---|---|---|
| `shops` | 2026-03-30 全 file 同一 commit | Unknown |
| `orders` | 2026-04-04 stripe webhook + cron のみ | Unknown |
| `conversations` | 2026-03-30 / 2026-02-02 旧 commerce 交渉 messaging | Unknown |
| `messages` | 2026-02-02 4 ヶ月前、 file 1 つのみ | Unknown |
| `drops` 系 8 件 | commerce 由来 | Unknown |
| その他 commerce 系 | （L-C audit で確定） | Unknown |

##### 必要な 7 軸 audit（Step 4-pre）
...
| 結果 | 分類 |
|---|---|
| 7 軸全て 0 痕跡 | **Confirmed Unused**（それでも削除しない、 補完最小化のみ） |
| 一部でも痕跡あり | **Active**（L-A に再分類、 完全補完） |
...
| name | 初版 | 2 回目 | 3 回目（現） |
|---|---|---|---|
| profiles | Layer 1 | L-A | **L-A** |
| notifications | Layer 1 | L-A | **L-A** |
| shops | Layer 1 | L-C (凍結) | **L-C Unknown** |
| orders | Layer 1 | L-C (凍結) | **L-C Unknown** |
| conversations | Layer 1 | L-C (凍結) | **L-C Unknown** |
| messages | Layer 1 | L-C (凍結) | **L-C Unknown** |
