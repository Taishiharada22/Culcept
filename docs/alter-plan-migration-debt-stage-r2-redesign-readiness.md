...
#### 想定対象（要 CEO 確認、 154 件中から抽出）

| name | category | 想定起源 |
|---|---|---|
| `profiles` | core user | Supabase Auth 連携 |
| `conversations` | core messaging | application 中核 |
| `messages` | core messaging | application 中核 |
| `notifications` | core | 既知 Stage B1 障害源 |
| `app_admins` | core access control | admin 機能 |
| `brands` | core taxonomy | product 軸 |

合計: **6-15 件想定**（精査は Stage R2-1 で）
