...
> **CEO 補正 2026-05-26（2 回目）**: 初版「Layer 1 = 6 件」を `.from()` 数で決定したが、 product 優先度 / replay 必要度を混同していた。
> 修正: 3 区分構造（L-A / L-B / L-C）に変更。
...
#### L-B: Replay Blocker Layer（要 audit、 件数未確定）
...
#### L-C: Frozen Compatibility Layer（推定 30-50 件）

product 上 凍結（commerce / 旧 messaging）、 ただし migration replay 互換のため **CREATE TABLE のみ最小限**補完。

| name | 凍結判定 |
|---|---|
| `shops` | 2026-03-30 全 file 同一 commit = 凍結 commit |
| `orders` | 2026-04-04 stripe webhook + cron のみ |
| `conversations` | 2026-03-30 / 2026-02-02 旧 commerce 交渉 messaging |
| `messages` | 2026-02-02 4 ヶ月前、 file 1 つのみ |
| `drops` 系 8 件 | commerce 由来 |
| その他 commerce 系 | （L-C audit で確定） |

**復活させない、 互換土台だけ作る**。CEO 方針「マネタイズ設計は後回し」と整合。

#### 初版 6 件の再分類

| name | 初版 | 修正 |
|---|---|---|
| profiles | Layer 1 | **L-A**（変更なし） |
| notifications | Layer 1 | **L-A**（変更なし） |
| shops | Layer 1 | **L-C**（凍結、 最小限） |
| orders | Layer 1 | **L-C**（凍結、 最小限） |
| conversations | Layer 1 | **L-C**（凍結、 最小限） |
| messages | Layer 1 | **L-C**（凍結、 最小限） |
