...
#### L-B: Replay Blocker Candidate（要 audit、 件数未確定）

L-A 以外で後続 172 migration が前提とする table。 Step 4 audit 範囲:
...
#### L-C: Unknown / Legacy-candidate（未 audit、 大半）

> ⚠️ **削除禁止 / 補完不要ともまだ言わない**。
> 154 件の prod-only のうち、 まだ 5 軸 audit を実施していない table。

現状: 154 件 − 7 件（audit 済）= **147 件が未 audit**。
...
L-B (Blocker のみ)       = audit 中、 件数未確定
L-D (Active + Non-blocker) = 5 件確定: shops, orders, conversations, messages, drops
L-C (未 audit)           = 147 件（残り、 Step 4 以降で audit）

合計確定: 7 件 (L-A 2 + L-D 5)
残り: 147 件 (L-C 未 audit)
```
