...
application 起動に **不可欠**な根幹 table。これがないと repo を clean environment で再生できない。

#### 確定対象（**CEO 確定 2026-05-26**、 Stage R2-0 audit 結果）

| name | category | `.from()` 数 | files 数 | 確定理由 |
|---|---|---|---|---|
| `profiles` | core user | **44** | 69 | application 中核、 最大 active query |
| `shops` | core commerce | **35** | 60 | commerce 導線 中核 |
| `notifications` | core notification | **13** | 28 | 既知 Stage B1 障害源 + active |
| `orders` | core commerce | **13** | 7 | active query 多、 focused 利用 |
| `conversations` | core messaging | **5** | 16 | active messaging |
| `messages` | core messaging | **3** | 90 | active messaging（file 多）|

**合計: 6 件確定**（L1-standard 採用、 CEO 2026-05-26）

#### Stage R2-0 で除外した候補

| name | 除外理由 |
|---|---|
| `brands` | `.from() = 0`、 別経路 / dormant → Layer 2/3 |
| `items` | files=726 で false positive ノイズ大、 `.from()=0` → Layer 外 |
| `app_admins` | `.from()=0`、 files=0、 dead/dormant → Layer 外 |
| `stripe_events` | `.from()=0`、 files=0、 dead/dormant → Layer 外 |
| 残り 20 件 singleton prefix | 高い使用度 sign なし → Layer 2/3 段階補完 |
