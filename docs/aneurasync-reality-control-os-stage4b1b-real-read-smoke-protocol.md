...
## 7. 4-B-1C 変更プレビュー（**今は実装しない**・CEO が事前にレビューできる形）

4-B-1C で**新規追加する唯一のファイル**（承認後）:

```ts
// lib/plan/reality/integration/dev-runtime-realsource.ts（新規・server-only・barrel 非 export）
import "server-only";
// 実 Supabase server client を SupabaseLikeClient として注入する薄い adapter。
// - createColumnRestrictedAnchorSource(realClient) をそのまま使う（query 形は 4-B-1A 固定）
// - start_time の実フォーマット（ISO 等）→ HH:MM 正規化を projection 前段に足す（raw を増やさない）
// - 呼び出しは dev harness の単発手動のみ（route/cron/UI から呼ばない）
```

- **既存ファイルは変更しない**（gate/adapter/guard は確定済）。追加は real client wiring の 1 ファイルのみ。
- 実行は flag on＋capability＋CEO user＋dev のときだけ。1 回読んだら終了（常時化しない）。
- 戻り値は `aggregateShadowReport([runShadow(...).summary])` ＝ `DevReportRedacted`（counts のみ）。
