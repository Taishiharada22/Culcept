...
### Q2 採用補正 (= 同日確定 2026-05-26)

P3-A-1 readiness §1.Q2 (= Google Calendar scope) を以下で確定:

- **採用 scope**: `calendar.events.readonly` + `calendar.calendarlist.readonly`
- **v1 制約**: selector UI 出さず、 内部 calendar 判定 logic で primary + accessRole='owner'/'writer' 自動 ON、 shared (= reader) default OFF、 user は設定画面で個別 override
- **撤回**: Claude 初案 「primary 固定」 (= CEO ゴール 「最良の予定」 = 全制約捕捉 未達のため)
- **棄却**: GPT 初案 `calendar.readonly` (= 広すぎ、 user trust 上 過剰要求)

CEO 確定文:
> 「primary 固定だと制約取りこぼしが出て、 「最良の予定」 に届かない。
>  かといって広すぎる権限も不要。 ユーザーに選ばせず、 でも primary だけにも閉じない設計が、
>  今の理想に最も合います。」
...
### F-β: P3-A-1 phase closeout (= 同日 2026-05-26、 GPT 推奨順 F-β → F-γ)

C-α / E-α / G-α 完了で **DB 非依存範囲を全網羅** → CEO + GPT 「ここで一度 closeout を固定」 採用。

**closeout doc 起草** (= `docs/alter-plan-p3-a-1-closeout.md`):
- §0 背景: 「完了」 ではなく 「区切り」、 実 動作は migration apply 後
- §1 完成済範囲: 23 commits (= 私 19 + ics merge 経由 4)、 4 routes、 8 helpers、 4 UI components、 1 migration draft
- §2 DB 非依存で閉じた範囲: 197 unit tests + production smoke fail-safe 確認済
- §3 D-e により止めた範囲: DB persist 動作 / real sync / sourceType 'google_calendar' 分離
- §4 migration debt 別 phase 切り出し理由: production 8 file 未適用 / staging 完全空 / 重複 timestamp
- §5 再開点: 5 step (= migration apply → sourceType 分離 → initial sync → toggle 永続化 → 通し smoke)
- §6 達成した革新性: subscriptions 自動判定 / sourceProvenance integration / sourceType 暫定流用 3 箇所 TODO 残置 / D-e 不変原則徹底 / multi-state mock 網羅
- §7 関連 docs / commits 一覧
- §8 次着手 (= F-γ Phase Next-1 readiness)
- §9 closeout 不変原則

**閉じた数字**:
- **23 commits** (= 19 P3-A-1 私の独自 + 4 ics merge 経由、 詳細は closeout doc §10.2)
- **197 unit tests** 全 PASS
- **0 production / staging schema 変更** (= D-e 不変原則 20 commit 通して遵守)

**次着手** (= GPT 順序):
1. F-β = 本 closeout (= 本 commit 着地)
2. F-γ = Phase Next-1 (= Rhythm baseline 学習) readiness 起草
3. その後に他トラック判断 (= LLM Track 3 / migration apply / etc)

### F-γ: Phase Next-1 Rhythm baseline 学習 readiness 起草 (= 同日 2026-05-26、 GPT 順序通り)

F-β closeout 直後、 CEO + GPT 「scope をかなり絞って、 本丸の入口を設計する」 を採用。

**起草 doc**: `docs/alter-plan-phase-next-1-rhythm-baseline-readiness.md`

**core message**:
- 「観察して、 最小の rhythm を言える」 までに厳密限定
- 予定を作る AI ではなく、 **リズムを言える AI** の設計
- 「自分って、 そういうリズムなんだ」 の最小実現

**9 section + CEO + GPT 5 必須項目 全網羅**:
  §0 背景 + Phase Next 6 軸との位置関係 + やらないこと 5 項目
  §1 観察源 inventory 5 source (= 今 vs 将来分離 / 優先度 / 不足分 / reliability ranking)
       - v1 fuel: 取り込んだ予定 (= source #1) のみ + Stargazer 軸 (補助)
       - 含めない: 編集履歴 / 達成未達 / 会話 (= 別 phase)
  §2 v1 出力定義 (= 限定的、 親 Q12 採用案 c と接続)
       - 4 種 card candidates (= 頻度集中 / 時間帯 / 平日週末 / 連続パターン)
       - 1 user 1 card / 1 文 / 数字露出禁止 / hedging 必須
  §3 descriptive vs prescriptive 厳格境界
       - 範囲表 + 文体規約 + 「禁止された言い方」 リスト
       - 境界曖昧 case の対比例
  §4 P3 接続点
       - P3-A-1 完成済 chain との接続
       - Next-1 動作前提 5 条件 (= apply + sourceType 分離 + sync persist + 接続 + data)
       - minimum data 件数 (= 過去 30 日 ≧ 7 件、 未来 7 日 ≧ 3 件)
  §5 成功条件 + fail-safe 3 種
       - 「そういうリズムなんだ」 4 最小条件
       - data 不足 / pattern 不明瞭 / 誤判定リスク → card 出さない default
  §6 着手禁止事項 (= GPT 5 禁止項目 厳守)
  §7 関連 docs
  §8 Next-1 着手段階 (= 別 readiness + CEO 着手 GO 制)
  §9 本 readiness 自身の不変原則

**着手禁止 (= 本 commit の不変原則)**:
- code 実装は CEO 個別 GO
- 自動予定生成 / multi-rhythm / failure learning / 詩学的言語 詳細 → 本 readiness 範囲外
- scope creep 防止
