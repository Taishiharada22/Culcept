...
## 2026-05-24 [Build/Product] sub-phase 3.5 採用 + 第 11 補正 (UI 責務分離 + source link) + sub-phase 3.6 着地 + 次 UI sub-phase 4 進行承認 [承認: CEO + GPT 第 11 補正後最終判定]

### 背景

sub-phase 3.5 (= source model 2 軸 refactor、 commit `b6c4b2e2`) 着地後、 CEO + GPT 第 11 補正後最終判定:
- 「sub-phase 3.5 採用、 次の sub-phase 4 へ進行 OK」
- 「UI に入る前に 2 点明示必須」
...
| # | 留意点 | 反映 |
|---|---|---|
| **1** | **UI 責務分離** (= origin と authority を混ぜない、 3 axis 独立に扱う) | Spec §19.9.1 で 4 行 mapping table 確定 (= provenance/操作可否/状態 chip/派生関係 → 各 axis 独立) |
| **2** | **cloneImported source link 保持** (= 元 imported ↔ 派生 user の関係追跡可能) | sub-phase 3.6 で実装 (= commit `90af5d32`): UserOwnedSource.clonedFrom metadata + isClonedFromImported + getClonedSourceLink helper |

### sub-phase 3.6 実装 (= commit `90af5d32`)

**lib/plan/list/sourceProvenance.ts**:
- UserOwnedSource に clonedFrom optional metadata 追加
- cloneImported update (= 戻り値の clonedFrom 自動付与)
- 新 helper 2 件: isClonedFromImported / getClonedSourceLink

**tests/unit/plan/list/sourceProvenanceFactoryContract.test.ts**:
- §5.6 cloneImported test update
- §9 (新) source link tests 7 件追加

### 検証 (= contract checkpoint PASS)

| step | 結果 |
|---|---|
| tests | 75/75 PASS (= sub-phase 3.5 から +7) |
| tsc surface | wave list 関連エラー 0 |
| forbidden wording grep | ZERO HIT |
| privacy/PII grep | ZERO HIT |
| git diff scope | 既存 file refactor (= +172/-6) |

### UI 責務分離 (= 第 11 補正 #1、 UI 実装で必須遵守)

| UI 要素 | axis |
|---|---|
| provenance 表示 (= SourceIndicator) | origin |
| 操作可否 / 編集可否 | authority |
| 状態 chip | authority |
| 派生関係表示 | clonedFrom (= 第 11 補正 #2) |

→ UI 実装 (= sub-phase 4+) は必ず 3 axis 独立に扱う。 混在禁止。

### 第 7+8+9+10+11 補正 累計反映 (= sub-phase 3.6 で完成)

| 補正 | 反映 |
|---|---|
| 第 7 補正 #1 多軸表現 | COMPACT/FULL VARIANT |
| 第 7 補正 #2 imported lock 逃がし道 | overrideImported / cloneImported |
| 第 8 補正 #2 完全消失防止 | acceptedAt + origin 永遠保持 |
| 第 9 補正 #1 不正組み合わせ機械的禁止 | discriminated union 5 variant |
| 第 9 補正 #2 checkpoint 表現統一 | contract checkpoint |
| 第 10 補正 source model 2 軸分離 | Origin + Authority |
| **第 11 補正 #1 UI 責務分離** | **Spec §19.9.1 mapping table** |
| **第 11 補正 #2 source link 保持** | **clonedFrom metadata + 2 helper** |

### 次 sub-phase 4 (= TimelineSpine + EventCard、 UI 入る)

- branch: `feat/alter-plan-list-impl-timeline-spine-event-card`
- 完了 trigger: visual smoke (= UI 入った後)
- UI 実装で第 11 補正 #1 責務分離を必須遵守

### 補正履歴 (= 累計 11 補正)

| commit | 内容 |
|---|---|
| ... | direction + IA + Spec + 第 1-10 補正 |
| `b6c4b2e2` | sub-phase 3.5 (= 2 軸モデル) |
| `32f58780` | 第 10 補正引き継ぎ + decision-log |
| `90af5d32` | sub-phase 3.6 (= source link refactor) |
| **本 commit** | **第 11 補正引き継ぎ + sub-phase 3.6 着地記録** |

### 承認 + ステータス

- **承認**: CEO + GPT 第 11 補正後最終判定 (= 2026-05-24、 「sub-phase 3.5 採用、 UI 前に 2 点明示必須」)
