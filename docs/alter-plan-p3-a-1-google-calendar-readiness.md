...
**論点**: Google Calendar API scope の選択 — 「最小権限」 と 「CEO ゴール 全制約捕捉」 の両立。

**選択肢検討経緯** (= CEO + GPT + Claude 合議 2026-05-26):
- 案 (a): `calendar.readonly` (= 広すぎ、 user trust 上 過剰要求)
- 案 (b): `calendar.events.readonly` 単独 + primary 固定 (= 狭すぎ、 仕事/個人 calendar 取りこぼし、 CEO ゴール 「最良の予定」 未達)
- 案 (c) ✅ **採用 (= CEO 確定 2026-05-26)**: `calendar.events.readonly` + `calendar.calendarlist.readonly` の組み合わせ

**採用 scope (= 案 c)**:
- `calendar.events.readonly` (= 各 calendar の events 取得)
- `calendar.calendarlist.readonly` (= user の calendar list 取得、 メタデータ)

**v1 制約 (= readiness 確定事項)**:
- **selector UI は出さない** (= user に聞かない、 CEO 思想 「意識させない」)
- **内部 calendar 判定 logic**:
  - primary calendar: 必ず取り込み
  - subscribed calendars: `accessRole='owner'` or `'writer'` のもの **default ON**
  - shared calendars (= reader 権限): **default OFF** (= 他人の予定混入防止)
- 取り込み対象 calendar は **設定画面** で user が個別 ON/OFF 可能
  (= Plan header の主導線には出さず、 設定画面の管理導線で扱う)

**採用理由 (= CEO ゴール逆算)**:
- CEO ゴール 「ユーザーが作らなくても予定が作られている」 には **全制約捕捉** が必要
- 仕事 / 個人 / 家族 calendar を持つ user では primary 固定だと制約取りこぼし → 「最良の予定」 未達
- ただし selector で user に聞くと CEO 思想 「意識させない」 に反する
- → **system 側で自動判定 + 設定画面で個別 override** の Aneurasync 独自 logic

**Soft connect 整合 (= 内部 rollout 設計)**:
- shared default OFF は内部 rollout 設計の典型例
- user trust が育てば設定画面で個別解放、 Alter が proactively 提案する余地は Phase Next
- → CEO 「Soft connect は内部運用 / 安全設計として保持」 と整合

**CEO 判断**: ✅ **A 採用 (= 案 c) 確定 (2026-05-26)**
