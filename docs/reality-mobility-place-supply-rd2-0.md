...
| `candidate_selected` | candidate を 1 つ選択（だが exact 確認前） | placeResolver で選択済・**ただし確認 provenance なし** | **inferred(低)**（選択済だが未確認） |
| `exact_confirmed` | **本人確認 provenance** で exact place 確定 | **本人選択 / persisted selected / explicit confirmation / trusted exact source**（§2.1） | **confirmed**（確認由来のみ） |
...
- **絶対則（CEO #1）**: **locationText があるだけで placeCertainty confirmed にしない**。`location_text_only` / `candidate_*` は **unknown ないし inferred 止まり**。

### 2.1 exact_confirmed の不変条件（CEO 補正 2026-06-14・確認由来で confirmed にする）

**整形状態 ≠ 確認**。`canonicalLocationText` 形・`isPlaceUnconfirmed=false` は「文字列が整っている」だけで「本人が確認した」ではない。RD2a 以降の不変条件:

- `locationText` だけで confirmed にしない
- Places API top-1 だけで confirmed にしない
- municipalityCoords / prefectureCoords だけで confirmed にしない
- canonical text format だけで confirmed にしない
- **`isPlaceUnconfirmed=false` だけを過信しない**
- exact_confirmed には **本人選択 / persisted selected place / explicit confirmation / trusted exact source**（= **確認 provenance**）のいずれかが必須
- それ以外は `candidate_selected`（inferred）または `candidate_unresolved`（unknown）に留める
- confirmed の根拠は **field-level evidenceRefs に残す**（source/confidence/provenance）

→ **判定は「文字列の整形状態」でなく「確認由来（source provenance）」で行う**。RD2a で型レベル強制（`createExactConfirmedResolution` は `ConfirmedPlaceSource` のみ受理し、canonical_text/places_api_candidate からは型エラーで構築不能）。
...
- **絶対則（CEO #2）**: **v0 で currentLocation を使うなら別 gate**。**勝手に現在地を読む実装は NO GO**。`current_location_candidate` は opt-in gate（§8）を通った時のみ・default は `unknown_origin` または `home_assumed`（inferred・断定しない）。
- home/work は **assumed（inferred）**であって confirmed origin ではない（本人が今日どこから出るかは不明）。

### 3.1 origin の confirmed 不変条件（RD2c 実装・確認由来のみ・現在地でも candidate）

place（§2.1）と同型: **「現在地」が最強シグナルでも confirmed にしない** — 「今いる場所」≠「出発する場所」（出発前に移動しうる）。RD2c の不変条件:

- **confirmed は `user_confirmed_origin`（本人確認）のみ**
- `previous_event_end` / `home_assumed` / `work_assumed` / `current_location_candidate` は全て **inferred 止まり**（confirmed にしない）
- **`current_location_candidate` は gate 済み evidence が渡された時のみ表現**（RD2c は gate を実行せず・navigator/geolocation に触れない・結果のみ受け取る = RD2b の provider 注入と同型）
- **origin 不明を home / currentLocation で勝手に補完しない**（`unknown_origin` は source none・auto-fill 禁止）
- **革新的安全則: confidence の `high` は confirmed に予約**（inferred は `moderate` 上限）。reality OS は本人が確認していない origin に「高確信」と言わない。gated GPS でも candidate ゆえ moderate 止まり。
- raw lat/lng/address/location label は consumer field に出さない（originRef は opaque）・source/evidence/confidence 必須。
