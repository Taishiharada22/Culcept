...
> ⚠️ **貼付先確認 step (2026-05-09 Yellow note 5 反映、procedure error 防止)**:
>
> canary throw は **必ず Browser DevTools の Console panel** に貼付。
> **Supabase SQL editor / SQL console / その他コードエディタには絶対貼付しない**。
>
> - DevTools Console は `Ctrl+Shift+J` (Win/Linux) / `Cmd+Opt+J` (Mac) で開ける
> - 開いた panel が「Console」と明確にラベル表示されていることを確認
> - prompt が `>` (JavaScript Console) であることを確認
> - 過去事例 (Stage 2.4-B 2026-05-09): SQL editor に貼付して `syntax error at or near "setTimeout"` で base canary 取得失敗 (Yellow note 5)、Sentry session 固定証跡欠落
...
> ⚠️ **貼付先確認 step (2026-05-09 Yellow note 5 反映、§6.5.1 と同)**: canary throw は **必ず Browser DevTools の Console panel** に貼付、**Supabase SQL editor 等に絶対貼付しない**。詳細は §6.5.1 の reminder block 参照。
...
## Appendix E — Stage 2.4-B Yellow付きPASS 完了通知 (2026-05-09 確定)

### E.1 完了状態

CEO 確定 (2026-05-09): B-2 wire (`39566cfd`) + B-3 Phase 1 (`ae7b6ecf`) + B-3 Phase 2 smoke harness (`cce40487`) 配備後、CEO 自身による mini-smoke 全 13 base scenarios + F-1 standalone 3 試行 = **16 sample 完了**。LLM 経路 100% 安定 + Stage 2.3 Round 6-9 強化 runtime 準拠確認 → **Stage 2.4-B Yellow付きPASS 確定**。

### E.2 集約結果 cross-reference

全 16 scenario 結果 / 各 body 完全記録 / 統計サマリ / Round 6-10 強化 runtime 確認 / F-1 三軸 record / canary status / Yellow notes 5 件 / 残課題 4 件 → **`docs/decision-log.md` の [2026-05-09] [Build] [Stage 2.4-B Yellow付きPASS 確定 + 全 16 scenario 集約 + canary status + Yellow notes 5 件 + 残課題 4 件 別 phase へ] entry** を参照。

### E.3 Yellow notes サマリ

| # | scenario | 内容 | 残置先 |
|---|---|---|---|
| 1 | 2.1.6 C@S5 normal | body 末尾 "?" 抜け micro | informational のみ、追加対応なし |
| 2 | 2.1.9 D@S5 travel | 文脈補完強 (CEO 指摘) | 別 phase (Stage 2.3 prompt refinement) |
| 3 | 2.1.14 F-1 standalone | 3/3 試行で F-1 primary 未到達 | 別 task (UI spec §7.12 S7 normal sharpen) |
| 4 | 2.1.12 / 2.1.13 | F-1 secondary daily/travel runtime 未確認 | 別 phase 判断 |
| 5 | base canary | 手順 error (DevTools / Supabase 混同) | 本書 §6.5 / §6.7 procedure 改善 (E.4 参照) |

### E.4 canary procedure 改善 (本書 update、Yellow note 5 反映)

§6.5 / §6.7 canary throw 実行手順に **貼付先確認 step** を追加:

> **重要 (2026-05-09 reminder)**: canary throw は **必ず Browser DevTools の Console panel** に貼付。**Supabase SQL editor / SQL console / その他コードエディタには絶対貼付しない**。
> - DevTools Console は `Ctrl+Shift+J` (Win/Linux) / `Cmd+Opt+J` (Mac) で開ける
> - 開いた panel が「Console」と明確にラベル表示されていることを確認
> - `setTimeout` を貼付前に、prompt が `>` (JavaScript Console) であることを確認
> - SQL editor は `>` ではなく `;` で複数文区切る、Console とは UI 別

このルールは Yellow note 5 (procedure error) の再発防止のため。impl 修正は伴わない、docs 改善のみ。

### E.5 残課題 (本 phase 完了後の別 phase / 別 task)

1. **Gap 4 production context detection** (executor watcher / heuristic / LLM 検出) — impl 残作業、§9 保留継続、別 phase
2. **F-1 standalone primary trigger spec ambiguity** (UI spec §7.12 S7 normal) — spec sharpen 別 task
3. **2.1.9 D@S5 travel 文脈補完** — Stage 2.3 prompt refinement 別 phase
4. **F-1 secondary daily/travel runtime 未確認** — Sentry Discover 後検索 or 追加 smoke、別 phase 判断

### E.6 Stage 2.4-B 完了後の凍結解除 / 不変

- ✓ Stage 2.4-B 凍結解除 (Appendix C.7 / D.5 / E.1 で達成記録)
- ✓ B-2 / Phase 1 / Phase 2 wire は production code として残置
- ✓ Preview env `NEXT_PUBLIC_COALTER_PRESENCE_SMOKE_CONTEXT=true` は Stage 2.4-D で削除判断
- ✗ Stage 2.4-B Yellow付きPASS は **production reachability PASS と呼ばない** (CEO/GPT 補正準拠)
- ✗ production context detector 実装しない (別 phase)
- ✗ selectPattern / prompt / validator / model / max_tokens / timeout 不変
- ✗ Production env 不接触
- ✗ 追加 smoke / Sentry Discover 追加調査しない (本 Stage 2.4-B 範囲)
- ✗ Stage 2.4-C / D 自律着手しない (CEO 個別承認後)
...
Stage 2.4-A ✅ PASS (commit 34067d98 / e14682cd)
   ↓
Stage 2.4-B ✅ Yellow付きPASS (commit 39566cfd / ae7b6ecf / cce40487 + 本記録)
   ↓
...
| 0.1-draft.7 | 2026-05-09 | B-2 wire (`39566cfd`) + B-3 Phase 1 (`ae7b6ecf`) + B-3 Phase 2 smoke harness (`cce40487`) deploy 後、CEO mini-smoke 全 13 base scenarios + F-1 3 試行 = 16 sample 実施完了 → **CEO 確定 Stage 2.4-B Yellow付きPASS**: (1) **Appendix E 新規追加** で完了通知 + 集約結果 cross-ref (decision-log [2026-05-09] entry) + Yellow notes 5 件 + 残課題 4 件 (別 phase / 別 task) + canary procedure 改善 (Yellow note 5 反映、§6.5 / §6.7 「貼付先確認 step」追加: DevTools Console と Supabase SQL editor 混同防止)。 (2) Stage 2.4-B 凍結解除確定 (Appendix C.7 / D.5 / E.1)。 (3) production reachability PASS とは呼ばない (CEO/GPT 補正準拠、smoke harness 経由のみ)。 (4) 次 phase Stage 2.4-C / D 着手判断は CEO 個別承認後 |
