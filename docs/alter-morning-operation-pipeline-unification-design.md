...
**改訂**: 2026-05-04 v2 — CEO+GPT 指摘 (journeyOrigin / segmentOrigin 分離、 source の独自信頼性) を反映。
...
| **regex (deterministic)** | 狭い構文での precision 高 (= 「X から Y へ」「明日 8 時」 等)、 誤検出 0 | LLM が同じ意味を出した場合は LLM と一致確認に使える / LLM が出さなかった時は独自に candidate を出す |
...
| **caller request** (= route.ts の input.today 明示) | システム的に確定した値 | LLM より上位 |
...
| `llm_explicit` (= comprehension.targetDate) | 700 |
| `regex_deterministic` (= extractTargetDate) | 600 |
| (default) `todayYmd()` | 100 |
...
| **OP-2** | LLM schema 拡張 (journeyOrigin / journeyEnd / segments[] field 追加 + day-level / segment-level 完全分離 + unknown 正式値) + Operation 型 5 種追加 + envelope 追加 + LLM prompt micro 改修 (= day-level signal 限定指示) | 受け皿のみ追加、 dispatcher / legacyAdapter は touch しない |
