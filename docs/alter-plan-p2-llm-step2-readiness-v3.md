# Alter Plan P2 — Step 2 Readiness v3 (= 「世界トップ級」 + 並行運用ルール固定版)

**Status**: readiness 補強起草 (= 着手前、 ただし **「実データ blocker 禁止」 ルール固定後**、 着手の概念整理)
**Date**: 2026-05-25
**Author**: Build Unit (Claude)
**Scope**: Step 2 readiness v1 を CEO + GPT 「世界トッププロダクトに劣らないものに」 要求で 4 軸補強 + v3 で **「実データ並行運用ルール」** を固定。
...
- Step 2 実装着手は **本 readiness 採用 + Q1-Q7 確定 + 50 件 synthetic dataset 完成 + forced-failure smoke 設計完了** で GO (= v3 更新、 「実データ待ち」 を blocker から外す)
...
**v3 で追加した固定ルール (= §13 詳細)**:
- 「実データ不足」 は今後 **停止理由にしない**
- 実データは **並行トラック** で取得、 **calibration material** として後段で精度改善に使う
- Synthetic / representative 50 件 dataset を **先行評価の主軸** にする
- 着手 / 実装 / canary の 4 gate を **実データ非依存** で定義 (= §6 修正)
...
- **実データは開始条件ではなく、 後から精度を上げる燃料** (= v3 固定)
...
## 6. preview canary plan (= v3 更新、 「実データ非依存」 で gate 再定義)

### 6.1 段階展開 4 step (= v3 で gate 再定義)

| Step | env | 入力 dataset | gate (= 通過条件) |
|---|---|---|---|
| **着手 gate** | — | readiness + synthetic 50 件 + forced-failure 設計 | Step 2 実装着手 GO |
| **Step 2 実装 + Local smoke** | `.env.local` | synthetic 50 件 dataset (= 先行評価主軸) | 50 件採点合格 + forced-failure smoke 5 件 PASS |
| **Preview canary** | Vercel preview env で `PLAN_ALTER_NOTE_LIVE=true` | 上記 + 限定 user 実データ (= calibration material) | latency / fallback 健全、 観測値 (= §6.2 基準) PASS |
| **Production canary** | production env、 10% rollout | 上記 + 全 user 実データの 10% | scale で observe (= §6.2 基準) |
| **Production 全面** | 100% rollout | — | kill switch 維持 |

### 6.2 各 step の判定基準 (= v3 更新、 実データ非依存 ⇄ 実データ補強 を明確化)

#### 着手 gate (= 実データ非依存、 v3 で追加固定)
- readiness 完成 (= 本 v3 採用)
- synthetic / representative 50 件 dataset 完成 (= §3 dataset、 LLM 呼ばない pure data file)
- forced-failure smoke 設計完了 (= §6.3 5 件、 実施手順具体化済)

これら 3 つが揃えば、 **実データ未取得でも Step 2 実装着手 GO** (= v3 固定ルール)。

#### Step 2 実装 + Local smoke → Preview canary GO (= 実データ非依存)
- 評価 dataset 50 件で Step 2 LLM 採点合格 (= §3.2.5 基準)
- forced-failure smoke 5 件 PASS (= §6.3 timeout / 503 / validation_failed / cost_cap / sensitive)

(= 実データなしで進める。 採点は LLM-as-judge + CEO 5-10 件サンプル)

#### Preview canary → Production canary GO (= 実データ activation)
- 1-2 week 観測で:
...
(= ここで初めて実データが入る。 ただし synthetic 評価で先行判定済のため、 「実データ待ちで止まる」 状態にはならない)

#### Production canary → 全面 GO (= 実データ scale calibration)
- 1-2 week 10% rollout 観測で:
  - error 率 ≤ 1%
  - cost ≤ monthly $50 (= 安全枠)
  - user 反応 (= dismissal / edit 率) で 「明らかに劣化」 signal なし

### 6.3 forced-failure smoke (= GPT 「fallback の live 実証」、 v3 で具体化必須化)

**Local smoke → Preview canary GO の必須前提**。 Step 2 実装直後 + Preview canary 直前の 2 回実施。

| # | Forced failure | 実施手順 | 期待挙動 (= 機械検証) |
|---|---|---|---|
| 1 | Gemini timeout | `lib/plan/llm/alterNoteGenerator.ts` で `timeoutMs: 1` に一時変更 → dev server で /plan 開く | 全 anchor `unavailable (reason: timeout)`、 UI は deterministic 文表示、 console error 0 |
| 2 | OpenAI fail | `.env.local` で `OPENAI_API_KEY=invalid_key` に一時変更 (= Gemini primary は通る) | Gemini で正常に LLM 経路、 OpenAI failover 不発、 UI 変化なし |
| 3 | Both fail | `.env.local` で `GEMINI_API_KEY=invalid_key` + `OPENAI_API_KEY=invalid_key` | 全 anchor `unavailable (reason: llm_failure)`、 deterministic 表示、 console error 0、 UI 不変 |
| 4 | validation_failed | mock prompt builder が違反語含む文を返すよう一時 hack | LLM 出力後 validator reject、 deterministic fallback、 analytics に `validation_failed` |
| 5 | cost cap | 21+ 件 anchor を作成 (= テスト user に dummy anchor を 21 件投入) | 最初 20 件 LLM、 21+ 件目 `unavailable (reason: cost_cap)` で deterministic 表示 |

各々の **検証 evidence**:
- dev server log で `[ai/run] ...` パターン確認
- Playwright screenshot で deterministic 表示確認
- UI runtime error 0 (= console clean)

実施後、 全 hack 削除して状態を元に戻す。 これで Step 2 「fail-open」 を **live 実証** 完了。
...
**結語**: 本 v2 補強で Step 2 は 「ちょっと賢いテンプレ」 から 「世界トッププロダクト級の個人化体験」 へ昇格できる readiness が確立。

**v3 更新で着手 timing も明確化**: Q1-Q7 確定 + synthetic 50 件 dataset 完成 + forced-failure smoke 設計完了 で **実装着手 GO** (= 実データ待ちなし、 §13 並行運用ルール固定)。

---

## 13. 並行運用ルール (= v3 で固定、 「実データ blocker 禁止」)

### 13.1 ルール宣言 (= 2026-05-25 CEO + GPT 合議で固定)

**「実データ不足」 は今後 停止理由にしない**。 実データ収集は **並行トラック** で進め、 設計・実装・評価は止めない。

### 13.2 6 つの運用ルール (= GPT 明示)

1. Step 2 readiness / 実装は実データ待ちで **止めない**
2. 50 件 synthetic / representative dataset を **先行評価の主軸** にする
3. forced-failure smoke を **必須化** する
4. preview canary は:
   - 50 件採点合格
   - forced-failure smoke PASS
   で進める (= 実データ非依存)
5. 実データは並行取得し、 入手後は:
   - prompt 補正
   - Personal Model calibration
   - correction memory 改善
   - thresholds 再調整
   に使う
6. 「実データがないので着手不可」 は **今後禁止**

### 13.3 実データの役割の再定義 (= v3 で明確化)

| 旧 (= readiness v2 まで) | 新 (= v3 固定) |
|---|---|
| 実データは **採用 blocker** | 実データは **calibration material** |
| 実データなし → 停止 | 実データなし → synthetic dataset で前進 |
| Preview canary → 「実データ採点」 で判定 | Preview canary → synthetic 採点 + 実データ補強観測 で判定 |
| 評価 dataset = 実データ待ちの代替 | 評価 dataset = **先行評価の主軸** |

### 13.4 並行トラック (= 2 本同時)

| トラック A | トラック B |
|---|---|
| Step 2 設計 + 実装 + synthetic 評価 | 実データ ingest 並行取得 |
| 着手 / 実装 / canary 進行 | A の各 milestone 後に **補強材料** として合流 |
| **stop ルール: A の gate 通過状態に依存** | **stop ルール: A の停止理由にしない** |

### 13.5 5 gate (= v3 で固定、 実データ非依存設計)

| Gate | 通過条件 | 実データ要否 |
|---|---|---|
| **G1 着手 gate** | readiness v3 採用 + synthetic 50 件 + forced-failure 設計 | ❌ 不要 |
| **G2 Step 2 実装 gate** | G1 + Q1-Q7 確定 | ❌ 不要 |
| **G3 Local smoke gate** | G2 + 50 件採点合格 + forced-failure smoke 5 件 PASS | ❌ 不要 (= synthetic 主軸) |
| **G4 Preview canary gate** | G3 + Preview env deploy | ⭕ 並行取得開始 |
| **G5 Production canary gate** | G4 観測値合格 + 10% rollout | ⭕ scale calibration |

各 gate は **synthetic dataset + machine 検証** で通過判定。 実データは G4 以降で **補強** に使う。

### 13.6 「世界トップ級」 達成への影響 (= v3 で再確認)

CEO 「世界トップ級個人化体験」 への 並行運用ルール の貢献:
- ✅ 「実データ待ちで止まる」 状態を排除 = 開発速度維持
- ✅ Synthetic + LLM-as-judge による **数値ベース品質保証** (= 勘ではない)
- ✅ 実データは入手後 calibration として精度向上 = **進化する Alter**
- ✅ Step 2 採用判定が **synthetic 評価のみで実行可能** = blocker 不在

---

## 14. v3 採用後の即着手リスト (= 並行トラック開始)

CEO Q1-Q7 + readiness v3 採用後、 **本セッション内で即着手可能** な並行作業:

### トラック A1: synthetic dataset 構築 (= readiness v3 採用と同時)
- `tests/eval/planAlterNoteDataset.ts` 新規 (= 50 件 anchor、 pure data、 LLM 呼ばない)
- category 別 8-12 件、 時刻帯 / location / sensitive バリエーション網羅
- 着手前停止不要 (= 「止まらず進む」 ルール、 dataset は LLM 不使用 pure data)

### トラック A2: forced-failure smoke 設計詳細化
- 本 readiness §6.3 を strict 手順化 (= 既に v3 で具体化済)
- 実施 script / hack 戻し手順を明文化

### トラック A3: Step 2 module 骨格 (= readiness 採用後の即実装)
- `lib/plan/llm/personalModelExtractorV2.ts` (= server-only、 3 層 PM 抽出)
- `lib/plan/llm/hdmPhaseGate.ts` (= server-only、 Phase ≥ 2 判定)
- `lib/plan/llm/alterNotePromptBuilderV2.ts` (= pure、 PM 注入対応 + Output Contract V2)
- `lib/plan/llm/outputContract.ts` (= ALTER_NOTE_CONTRACT_V2、 既存 stargazer/outputContract pattern 流用)
- `lib/plan/llm/alterNoteValidatorV2.ts` (= contract field detector 拡張)

### トラック B: 実データ ingest 並行 (= CEO / 知人 5-10 名による preview canary 利用)
- Preview deploy (= 既存 Vercel preview 経由)
- 実利用ログ取得 (= 既存 home_alter_judgment analytics pattern 流用)
- A の各 milestone 後に補強材料合流

---

**v3 結語**: 「止まらず進む」 固定ルールで Step 2 readiness を採用判定 → 即並行着手可能。 実データは calibration として後段精度向上に貢献、 開発速度の blocker にはしない。 CEO Q1-Q7 + readiness v3 採用判定をお願いします。
