...
| **postSelectionFlow ✅ DONE** | activity null→"" | **−12** | **main `d2f3b64d` 着地**（74→62・累計 1114→62=-1052/94%）。test fixture が「what 未指定」を null で表していたが production は ""（WhatSlot.activity:string 正しい・consumer は null/"" 同一扱い）→ test fixture を "" に整合。test-only・挙動不変。closeout: `…-postselectionflow-closeout.md`/mini design `…-postselectionflow-mini-design.md`。 | 低 | 完了 |
| **S6 ✅ batch4 DONE** | test 型エラー（実型） | **−7** | **main `60ee0a9e` 着地**（62→55・累計 1114→55=-1059/95%）。phaseC isWeekday/timeOfDay 除去(5・removed-old-spec dead field)・realityCandidate anchors を mutable 局所型に(2・cast なし)。test-only・挙動不変。**残置（無判断 safe 尽きた）**: rawRef 系 mock 構築・semantic 値 mock(placeResolver/planHistory)・ceoScenario/planIntakeGate/domainRouter/presenceTelemetry(prod export)/個別・stargazer 7(S5)。closeout: `…-s6b4-closeout.md`。 | 低 | batch4 完了 |
| **S6 ✅ batch1+2+3 DONE** | test 型エラー（実型） | **−64** | **batch1 `6138d99a`(138→101)**: entry(17)/narrowing(19)/fail(1)。**batch2 `08ff945d`(101→87)**: anchor cast(10)/@ts-expect-error(2)/walking(1)/narrowing(1)。**batch3 `8eeec516`(87→74・累計 1114→74=-1040/93%)**: urgentLayerDismiss reason+typed dismissMock(12・cast 不使用)/journeyOriginDebugLog implicit any(1)。closeout: `…-s6/s6b2/s6b3-closeout.md`。**残置（全て judgment 要・無判断 safe ほぼ尽きた）**: postSelectionFlow(13・prod 型不整合)・phaseC(5・spec review)・mock multi-field stale(b3b/placeResolver/planHistory 等)・ceoScenario/planIntakeGate/presenceTelemetry 等個別・stargazer(7・S5)。 | 低 | batch1+2+3 完了 |
