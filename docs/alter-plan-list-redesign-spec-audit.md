...
### 19.13 第 15 補正: 表現補正 + 3 tab 先行準備明示 + sub-phase 6 範囲制限 (= sub-phase 5 採用後明示、 GPT 第 15 補正)

GPT 第 15 補正 (= 2026-05-24): 「sub-phase 5 採用、 sub-phase 6 進行 OK。 軽い補正 2 + 範囲制限 1」

#### 19.13.1 表現補正 (= 「既存 UI / レイアウト / デザイン変更 0」 → 3 表現)

| 旧表現 (= 撤回) | 新表現 (= 採用) |
|---|---|
| 「既存 UI / レイアウト / デザイン変更 0」 (= 不正確) | **「既存画面への反映 0 + 既存画面 component 改変 0 + 統合前の新規 component 追加のみ」** |

理由: 旧表現は「新規 component 追加 = 既存 UI 変更」 とも読めて不正確。 第 15 補正で 3 分離。

#### 19.13.2 3 tab union 扱い (= 先行準備のみ、 先回り禁止)

- EmptyDayEntry の `calendar / flow / map` 3 tab 対応は **先行準備のみ** (= type union 受入のみ)
- **calendar / map 実装への先回り禁止**
- 本 sub-phase 5 + 後続 sub-phase 6+ の対象は **List (= flow) のみ**
- calendar / map actual UI 実装は **別 phase** (= Map redesign Spec + 後段)

#### 19.13.3 sub-phase 6 範囲制限 (= first-pass の枠)

| 含める | 含めない |
|---|---|
| SourceIndicator (= first-pass) | 詳細 sheet 専用 component (= sub-phase 7+) |
| ExecutionLayerChip (= first-pass) | ExecutionLayer **本体** (= 学習ループ / 詳細 fields) |
...
### 19.12 第 14 補正: sub-phase 5 範囲制限 + test pickup 注意 (= sub-phase 4 採用後明示、 GPT 第 14 補正)

GPT 第 14 補正 (= 2026-05-24): 「sub-phase 4 採用、 sub-phase 5 へ進行 OK。 但し範囲制限 1 + 軽い注意 1」

#### 19.12.1 範囲制限 (= sub-phase 5 で必須遵守)

| 含める | 含めない (= 後続 sub-phase) |
|---|---|
| **TransitionChip** (= first-pass) | SummaryFooter (= sub-phase 8、 第 8 補正 #1 score / 評価文凍結維持) |
| **EmptyDayEntry** (= first-pass、 N-3a 連携) | ExecutionLayer **本体** (= 学習ループ / 詳細 fields = sub-phase 7+) |
| import test + render contract test | ImportedLockEscape **本実装** (= sub-phase 7 で modal 専用) |

→ 「first-pass」 = 構造の箱まで。 詳細 logic / 本実装 は後続 sub-phase。

#### 19.12.2 軽い注意: `.test.tsx` include 追加の今後

- 妥当だが、 今後 test pickup 範囲が広がる可能性
- 反映: 本 sub-phase 5 以降の new test file は `tests/unit/plan/list/` 内に限定 (= scope 拡大しない)
- blocker ではない (= 第 14 補正で明示)

### 19.11 第 13 補正: checkpoint 表現補正 + render contract test 必須 (= sub-phase 4 後明示、 GPT 第 13 補正)

GPT 第 13 補正 (= 2026-05-24): 「sub-phase 4 概ね妥当、 次に進んでよい。 但し 2 補正必須」

#### 19.11.1 補正 #1: 表現補正 (= 「visual checkpoint」 → 「UI foundation checkpoint」)

| 段階 | 旧表現 (= 撤回) | 新表現 (= 採用) | 理由 |
|---|---|---|---|
| sub-phase 2-3 (= pure type / factory) | foundation / contract checkpoint | (= 維持) | UI なし、 既存表現で正確 |
| **sub-phase 4-6 (= component 単位、 render contract まで)** | ~~visual checkpoint~~ | **UI foundation checkpoint** | UI 入ったが actual DOM 表示確認は未実施 (= render contract test まで) |
| sub-phase 7+ (= modal / 統合、 demo route で実 render) | visual smoke | (= 維持) | actual user visual 実機確認 |

→ **正確な表現を使う**: sub-phase 4 完了 = **UI foundation checkpoint PASS** (= visual checkpoint ではない、 GPT 指摘で「強すぎる」)

#### 19.11.2 補正 #2: render contract test 必須 (= 機械保証で UI hierarchy 担保)

GPT 第 13 補正: 「import / type / grep / 既存不触ベースだけでは『見た目が正しい』 は未確認。 render contract test を 1 枚追加」

**手段**: `react-dom/server` の `renderToStaticMarkup` で string HTML 出力 → string 検査 (= @testing-library 不要)

**必須 5 項目**:

| # | 項目 | 検査方法 |
|---|---|---|
| 1 | primary 情報が出る | title / 時刻 / 場所 / Alter 補助文 が HTML に含まれる |
| 2 | proposed の見え方 | `border-dashed` + `opacity-70` + 「受け入れる」 text |
| 3 | origin 表示 | `aria-label="source: imported"` or `"source: Alter generated"` + 対応 dot 色 |
| 4 | clonedFrom が main card に出ない | cloned event の HTML に importedSource (= 「シフト表」 等) 不在 |
| 5 | focus-visible slate | `focus-visible:border-slate-300` 含む、 `focus-visible:border-{brand}` 不在 |

#### 19.11.3 反映 sub-phase

- **sub-phase 4 内で render contract test 追加** (= 同 branch、 「次 sub-phase 前」 推奨を遵守)
- 完了 trigger: **UI foundation checkpoint PASS** (= import test + render contract test 5 必須項目 PASS)
