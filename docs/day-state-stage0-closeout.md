...
| # | 事項 | 処置 |
|---|---|---|
| 裁定 1 | §4.3「同規約」と §5.2 明示セルの矛盾（recoveryNeed の ±1 吸収） | **§5.2 を正と裁定**: 3 値スケールは ±1 吸収なし（吸収すると凍結 medium が永遠に match = 学習信号消滅）。設計書 §4.3 に裁定を追記済み・実装/テスト準拠 |
...
| 注記 4 | parse 不能時の MomentState.timeBucket は placeholder（currentMode=unknown とセットでのみ解釈。14 フィールド契約のため単独変更不可 — 次 additive 改訂で `| "unknown"` 検討） |
