...
#### §3.0-b schema_migrations 未登録確認 (= 4 version すべて未登録であることを保証)
...
ORDER BY version;
-- 期待: 0 行 (= 4 version すべて未登録)
```

**1 行でも返ったら**: 即停止 → CEO 報告 (= 既登録 version あり、 重複 INSERT 兆候)
...
-- 検証 1-2a: RLS policy 数 (= 4 × 2 table = 8)
...
-- 検証 1-2b: RLS policy 名前確認 (= 8 個、 全部揃っているか)
SELECT tablename, policyname FROM pg_policies
WHERE schemaname = 'public'
  AND tablename IN ('external_anchor_sources', 'external_anchors')
ORDER BY tablename, policyname;
-- 期待: 8 行 (各 table × 4 policy):
--   external_anchor_sources:
--     external_anchor_sources_owner_delete
--     external_anchor_sources_owner_insert
--     external_anchor_sources_owner_select
--     external_anchor_sources_owner_update
--   external_anchors:
--     external_anchors_owner_delete
--     external_anchors_owner_insert
--     external_anchors_owner_select
--     external_anchors_owner_update
...
-- 検証 1-4: indexes 名前確認 (= 2 + 3 + 2 pkey = 7)
...
-- 期待: 7 行 (個別名前):
--   external_anchor_sources:
--     external_anchor_sources_pkey
--     idx_external_anchor_sources_stored_expiry
--     idx_external_anchor_sources_user_captured
--   external_anchors:
--     external_anchors_pkey
--     idx_external_anchors_source
--     idx_external_anchors_user_date
--     idx_external_anchors_user_validity

-- 検証 1-5: schema_migrations INSERT 確認 (= §6 INSERT 後に実行)
SELECT version FROM supabase_migrations.schema_migrations
...
-- 検証 3-2: RLS policy 名前 + cmd 確認 (= 3、 UPDATE 意図的不在)
SELECT policyname, cmd FROM pg_policies
WHERE schemaname = 'public' AND tablename = 'plan_drift_events'
ORDER BY policyname;
-- 期待: 3 行 (個別名前 + cmd):
--   plan_drift_events_owner_delete  | DELETE
--   plan_drift_events_owner_insert  | INSERT
--   plan_drift_events_owner_select  | SELECT
-- update 不在 (= W1-5b 設計、 append-only)
...
--   idx_plan_drift_events_user_pattern
--   plan_drift_events_pkey

-- 検証 3-5: schema_migrations INSERT 確認 (= §6 INSERT 後)
SELECT version FROM supabase_migrations.schema_migrations
...
-- 検証 4-2a: seed row が 期待 key のみ で 1 行だけ存在
SELECT COUNT(*) AS row_count FROM app_settings
WHERE key = 'mirror_channel_enabled';
-- 期待: 1 (= ちょうど 1 行)

-- 検証 4-2b: seed row の内容
SELECT key, value, updated_at FROM app_settings
WHERE key = 'mirror_channel_enabled';
-- 期待: 1 行、 value = {"enabled": true}

-- 検証 4-2c: app_settings 全体に未知の key が混入していないか
SELECT key FROM app_settings ORDER BY key;
-- 期待: 1 行のみ (= mirror_channel_enabled だけ)

-- 検証 4-3: trigger 名前 + event 確認 (= 3 個、 全部揃っているか)
...
-- 期待: 3 行 (個別名前 + event):
--   mirror_kill_switch_audit_trigger    | app_settings                      | UPDATE | AFTER
--   prevent_audit_delete_trigger        | coalter_mirror_kill_switch_audit  | DELETE | BEFORE
--   prevent_audit_update_trigger        | coalter_mirror_kill_switch_audit  | UPDATE | BEFORE

-- 検証 4-4: function 名前確認 (= 2 個)
SELECT proname FROM pg_proc
WHERE proname IN ('audit_mirror_kill_switch', 'prevent_audit_row_modify')
ORDER BY proname;
-- 期待: 2 行 (個別名前):
--   audit_mirror_kill_switch
--   prevent_audit_row_modify
...
-- 期待: 3 行:
--   app_settings_pkey
--   coalter_mirror_kill_switch_audit_pkey
--   idx_mirror_kill_switch_audit_triggered_at

-- 検証 4-4c: policy 名前確認 (= 1 個、 app_settings 上)
SELECT tablename, policyname, cmd FROM pg_policies
WHERE schemaname = 'public'
  AND tablename IN ('app_settings', 'coalter_mirror_kill_switch_audit')
ORDER BY tablename, policyname;
-- 期待: 1 行:
--   app_settings | app_settings_anon_read_mirror_kill_switch | SELECT
-- (= coalter_mirror_kill_switch_audit は policy 不在 = default deny)

-- 検証 4-5: immutability test (= 失敗するべき、 production では実行不要)
-- 注: staging で R3 完走時に全 trigger 機能確認済。 production では skip。

-- 検証 4-6: schema_migrations INSERT 確認 (= §6 INSERT 後)
SELECT version FROM supabase_migrations.schema_migrations
...
### §4.4 「object 実体 + history 整合」 完了条件 (= CEO 2026-05-27 軽補正)

各 Step は **object 実体 (= table / index / policy / trigger / function) + migration history (= schema_migrations row)** の **両方が揃って初めて完了** とする。

#### §4.4.1 verify が通っても schema_migrations 手動 INSERT が失敗 → 即停止

- verify (= §3.x の 1-4 系) が全て PASS でも、 §6 の `INSERT INTO supabase_migrations.schema_migrations` が duplicate key violation や column mismatch で失敗した場合 → **即停止**、 CEO 報告
- 状態: object 実体は適用済、 history は未登録 → **partial state**
- 復旧: §5.x の Rollback SQL で object 実体を drop してから再着手 (= CEO 判断)

#### §4.4.2 INSERT 不整合の即停止条件

INSERT 直後の確認 (= 検証 X-5 系) で:
- 期待 1 行のところ 0 行 → INSERT 失敗、 即停止
- 期待 1 行のところ 2 行以上 → 二重 INSERT、 即停止 (= 通常ありえないが defense)

#### §4.4.3 「object 実体と history の両立」 を完了の単一条件にする理由

- object のみ存在 (= history 不在) → 将来の CLI `supabase db push` で「未適用」 判定 → 再 apply 試行 → CREATE 系 SQL 失敗 → CLI 停止
- history のみ存在 (= object 不在) → application 動作不能、 RLS / function 不在で error
- 両立して初めて、 後段 phase (= 残 4 file 適用後の通常運用) で CLI が正しく history を読める
...
### §6.0 各 Step の実行順序 (= 厳守、 CEO 2026-05-27 軽補正)

各 Step は **必ず以下の順序** で実行する:

```
1. apply        — DDL SQL を SQL Editor で Run
2. verify       — §3.x の検証 SQL × N (= 名前確認込み)
3. INSERT       — schema_migrations 手動 INSERT 文
4. INSERT verify — §3.x-5 で INSERT 結果が 1 行確認
5. stop         — CEO 確認 / 次 Step 着手判断
```

**先に INSERT しない**:
- ❌ apply 前に INSERT (= history 先行登録、 object 不在の不整合)
- ❌ verify を飛ばして INSERT (= 不完全 object を完了扱い)
- ❌ INSERT 失敗を無視して次 Step 着手 (= partial state 残置)

**理由**:
- verify 不合格でも先に INSERT を撃つと、 後で object が不正常だった時に history だけ残って混乱
- §4.4 「object 実体 + history 整合」 を完了条件にする以上、 apply 完了 → object 完成 → history 登録 の順序を物理的に守る
...
## §10. CEO レビュー判断 (= 2026-05-27 軽補正 3 点反映済)

### §10.1 CEO 軽補正 3 点の反映箇所

| # | CEO 補正内容 | 反映箇所 |
|---|--------------|---------|
| 1 | 適用順は補正不要 (Step 1→2→3→4 維持) | §2 据え置き |
| 2-a | verify は名前確認も入れる (policy / index / trigger) | §3.1 検証 1-2b / 1-4 (= 名前列挙)、 §3.3 検証 3-2 / 3-4、 §3.4 検証 4-3 / 4-4 / 4-4b / 4-4c |
| 2-b | schema_migrations 手動 INSERT 前に対象 version が未登録であることを確認 | §3.0-b (= Pre-flight、 4 version 全部未登録チェック) |
| 2-c | Step 4 seed row は期待 key / 1 行だけか確認 | §3.4 検証 4-2a / 4-2b / 4-2c |
| 3 | verify が通っても schema_migrations 手動 INSERT 失敗 / 不整合は即停止、 object 実体 + history で完了 | §4.4 新規 (= 三段補強) |
| 4 | 順序は apply → verify → INSERT → stop、 先に INSERT しない | §6.0 新規 (= 厳守順序明示) |

### §10.2 Step 1 着手 GO 条件 (= CEO 確定済)

- ✅ §10.1 軽補正 3 点 (= 実質 4 領域) すべて反映
- ✅ Scenario β (= Dashboard 個別 SQL Editor) 採用
- ✅ Step 1 = `external_anchors.sql` apply は **CEO 手動実行 GO**
- ⏳ Step 2 以降は Step 1 pass 後に CEO 順次判断

→ R4 Step 1 (= File 1 apply、 SQL Editor 手順) に進む。 本 doc を参照しながら CEO が production Dashboard で 1 file ずつ実行する。
