...
### 8.1 既存 Presence Layer 不可侵

**Phase A から継承する不変境界**:
- `lib/coalter/presence/` (30+ files) を一切 touch しない
- `app/components/chat/` (17 files) を一切 touch しない
- 既存 `PresenceState` S0-S8 の遷移を改変しない
- 既存 `ModeSwitcher.tsx` を改変しない

### 8.2 Mirror の表示位置

**禁止**:
- チャット本文 (user message / assistant message) への自動挿入
...
- 既存 presence layer / chat layer への一切の touch
...
4. 第 1 マイクロ PR が UI surface 作成のみ（Speak ロジックなし）であること
5. Kill switch flag が default `false` で起票されること
6. 既存 presence layer に diff が 0 行であること
