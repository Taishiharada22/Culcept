...
> A1-5-0…§8.43 / **A1-5-7-4 Consumption Surface Bridge（landed・§8.44・canonical core `computeCapturedSeedConsumption`(1 計算で summary+enriched placements) 抽出・既存 summary-only runner は core 委譲で不変(21 tests)・新規 `runCapturedSeedConsumptionWithSurface→{summary,surface}`(candidateCount と surface items を同一 core 由来=再算出 drift 解消)・redaction は presentCandidateSurface 1 箇所(seedRef drop)・CandidateDraft 不使用・15 tests・reality 872・route.ts 不変）**。**再算出 drift 解消・candidateCount/surface 単一 canonical 化・redaction 1 箇所固定**。次は **A1-5-7-5 visible route integration**（route.ts に bridge.surface→`appendCaptureCandidateToMorningResult` 配線・別 GO）→ pending seed read 接続（別 GO）→ UI 接続（別 GO）。**route.ts 変更 / route response 実変更 / UI·PlanClient 接続 / DB / production / remote は必ず別 GO で停止**。raw を同じ読み取り表面に置かない・column-restricted・fail-closed・no default duration を全段で維持する。
