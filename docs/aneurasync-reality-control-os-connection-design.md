...
> A1-5-0…§8.25 / **A1-5-5a Capture Live Gate（landed・§8.26・pure・fail-closed 9 層・reason code・`PLAN_FLAGS.realityCaptureLive`+`realityCaptureKill`・canonical refs 単一ソース・kill 最優先・21 tests）**。**以降の全 capture slice が必ず通る安全 gate**（production nodeEnv / production project ref(aljav) / 非 staging / 非 canary / flag off / kill → block）。次は **A1-5-5b Extractor Contract skeleton**（pure・`SeedExtractor`+`validateExtractorOutput`+fake・extractor 出力は必ず intake guard 通過・別 GO）。**LLM/runtime/実 write 接続は必ず別 GO で停止**。raw を同じ読み取り表面に置かない・column-restricted・fail-closed・no default duration を全段で維持する。
