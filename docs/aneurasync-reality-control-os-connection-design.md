...
> A1-5-0…§8.38 / **A1-5-6-0/1 Captured Seed Consumption Shadow Runner（landed・§8.39・server-only pure runner `runCapturedSeedConsumptionShadow`: DI captured row→sanitize(allowlist 再構築・raw/source_ref drop)→project(read seam)→enrich(seed_explicit/correction high=strong・prm_typical=weak)→generateComplete→redacted summary{counts+reason}・DB write/read 0・route/UI 0・21 tests・reality 822）**。**書いた seed/evidence → candidate を canonical 1 経路に固定・消費境界で raw/source_ref を構造遮断・runtime visible behavior 変更 0**。次は **A1-5-6 real DB read smoke**（staging で実 read→consumption・別 GO）→ その後 visible integration（route/UI・別 GO）。**real DB read smoke / route response 反映 / UI·PlanClient 接続 / production / PRM·correction 実接続 / remote は必ず別 GO で停止**。raw を同じ読み取り表面に置かない・column-restricted・fail-closed・no default duration を全段で維持する。
