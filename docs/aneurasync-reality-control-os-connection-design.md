...
> A1-5-0…§8.42 / **A1-5-7-3 Surface Assembly Smoke（landed・§8.43・doc-only・untracked harness 実行後削除・file capture 1回・実 LLM 0(synthetic structured input→fake extractor)・controlled write(real RPC1・seed1+evidence1)→real read(allowed col)→consumption(candidateCount=1)→presentCandidateSurface(hasCandidate・item1・seed_explicit)→appendCaptureCandidateToMorningResult(captureCandidate additive・existing keys intact・no-candidate deep-equal・envelope ok)→cleanup rows=0/userTotalReality=0・service_role 0・raw発話保存なし・staging のみ）**。**capture pipeline 全段(write→read→consumption→surface→assembler→cleanup)が実 staging row で end-to-end 実証済**。ギャップ: consumption runner は summary のみ返す→visible integration では enriched placements 露出 slice が必要。次は **A1-5-7-4 consumption result 拡張**（runner が candidate placements を露出・no-visible・別 GO）→ その後 visible route integration（route.ts に配線・別 GO）→ UI 接続（別 GO）。**route.ts 変更 / route response 実変更 / UI·PlanClient 接続 / DB / production / remote は必ず別 GO で停止**。raw を同じ読み取り表面に置かない・column-restricted・fail-closed・no default duration を全段で維持する。
