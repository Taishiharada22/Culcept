...
> A1-5-0…§8.45 / **A1-5-7-6 Capture Candidate UI Surface（landed・§8.46・pure presenter `presentCaptureCandidate`(evidenceSource→友好ラベル・absent→null)+`CaptureCandidateBanner`(控えめ・absent→空 markup・「候補があります」)+MorningPlanCard optional captureCandidate prop additive(未提供→既存 UI 完全不変)・renderToStaticMarkup tests・enum/source_ref/UUID が DOM 非出・19 tests・reality 908・DB/network 0）**。**captureCandidate UI banner + MorningPlanCard 配線済(additive・後方互換・控えめ・技術名/UUID 非露出)・live 未表示(V2 route 未消費)**。次は **A1-5-7-7 V2 route 消費配線**（AskHero→alter-morning/plan fetch→captureCandidate を MorningPlanCard へ・別 GO）or **production canary**（surface flag を限定 production・別 GO）。**production 接続 / PlanClient 実 DB/API 変更 / PRM·correction 実接続 / production flag ON / remote は必ず別 GO で停止**。raw を同じ読み取り表面に置かない・column-restricted・fail-closed・no default duration を全段で維持する。
