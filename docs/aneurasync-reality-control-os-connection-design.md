...
> A1-5-0…§8.36 / **A1-5-5g-3 redacted observation sink + observe staging smoke（landed・§8.37・helper に redacted sink（既定 log / set・reset・emit・never-throw）+ `fireMorningCaptureObserve` を sink 接続・実 route 経由 observe smoke で route 200・response 不変・observerStarted・outcome=captured・wouldCapture/wouldEvidence=true・llmCallCount=1・DB write/RPC/Supabase write 0・service_role 0・staging(hjcr) のみ・helper sink 4 tests・reality 791）**。**実 route observe が staging-gate で would-capture まで到達・response 不変・write 0 を実証済**（observe-mode 監視基盤）。次は **A1-5-5g-4** 候補: ① observe-mode を staging で広めに観測（複数発話の would-capture 分布）/ ② write mode（実 write・atomic RPC・実 DB・別 GO）。**DB write/RPC/write mode/production/UI は必ず別 GO で停止**。raw を同じ読み取り表面に置かない・column-restricted・fail-closed・no default duration を全段で維持する。
