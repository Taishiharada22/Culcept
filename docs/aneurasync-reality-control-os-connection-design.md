...
> A1-5-0…§8.16 / **A1-5-4b-0/1 structured capture write seam skeleton（landed・§8.17・fake/no-run・実 DB write 0・atomic payload 契約・owner/linkage 検証・barrel 非 export）**。次は A1-5-4b real write（atomic RPC migration 作成 + real Supabase client・user-RLS・要強い GO）。**capture 全経路（抽出→mapper→write seam→[未来 RPC INSERT]→read projection→enrich→candidate）の pure/skeleton 部分は全て fixture/fake で実証済**。raw を同じ読み取り表面に置かない・column-restricted・fail-closed・no default duration・no DB write を全段で維持する。
