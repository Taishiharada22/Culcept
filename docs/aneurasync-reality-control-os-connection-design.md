...
> A1-5-0…§8.44 / **A1-5-7-5 Visible Route Surface Integration（landed・§8.45・新 flag `realityCaptureSurface`(default off)・`morning-capture-surface.server.ts`(read-only・canonical source 委譲=single-read-source 遵守・fail-open・gated)・route.ts に `data.captureCandidate?` additive(fail-open・既存 keys/envelope/error 不変・LLM await なし)・projected core `computeConsumptionFromProjected`(row 経路と共有=drift なし)・staging smoke で実 route captureCandidate 確認(hasCandidate/candidateCount=1/seed_explicit)・cleanup 0・surface module 17 + route 17 tests・reality 889）**。**route response に captureCandidate? を additive 配線済(C案・後方互換・production gated・UI 非接続)**。次は **A1-5-7-6 UI surface**（MorningPlanCard 等で captureCandidate を "候補があります" 表示・別 GO）→ その後 production canary（別 GO）。**UI·PlanClient 接続 / production flag ON / PRM·correction 実接続 / remote は必ず別 GO で停止**。raw を同じ読み取り表面に置かない・column-restricted・fail-closed・no default duration を全段で維持する。
