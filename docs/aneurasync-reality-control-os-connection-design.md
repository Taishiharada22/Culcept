...
> A1-5-0…§8.22 / **A1-5-4d-0/1 Structured Capture Orchestrator Skeleton（landed・§8.23・DI・no-run・3 部品を 1 本の pipeline に束ねる配線層・fail-closed raw 遮断 end-to-end・stage 判別 result・owner/linkage 構造的整合・server-only・19 tests）**。**capture 全経路（intake guard→mapper→write seam→RPC adapter→実 atomic RPC→atomic INSERT→read seam→enrich→candidate）が pure/fixture + 実 staging で実証済 + orchestrator で 1 本に配線済**。次は seed capture runtime（実 capture: chat→抽出 LLM→`runStructuredCapturePipeline`(実 client 注入)）/ PRM·correction 実接続 / runtime·UI を各別 GO（**LLM/runtime/実 write 接続は必ず別 GO で停止**）。raw を同じ読み取り表面に置かない・column-restricted・fail-closed・no default duration を全段で維持する。
