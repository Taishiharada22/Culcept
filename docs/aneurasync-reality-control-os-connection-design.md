...
> A1-5-0…§8.35 / **A1-5-5g-2 Route Capture Observe Wiring（landed・§8.36・alter-morning/plan route に fire-and-forget observe を接続・observe-only・dry-run fake write（実 DB 0）・route response 不変（二重 try/catch）・cheap guard（flag off/kill 即 return）・production hard block・helper 16 + route 9 tests）**。**capture observer が実 route に observe-only で接続済**（runtime path 存在・実 DB/RPC 到達 0・実 LLM は gate default off）。次は **A1-5-5g-3 observe staging smoke**（staging+canary+REALITY_CAPTURE_OBSERVE=true で実 route 経由 observe を 1 回・実 LLM・dry-run write・別 GO）→ その後 write mode（実 write・別 GO）。**DB write/RPC/write mode/production/UI は必ず別 GO で停止**。raw を同じ読み取り表面に置かない・column-restricted・fail-closed・no default duration を全段で維持する。
