...
> A1-5-0…§8.46 / **A1-5-7-7 V2 Route Consumer Bridge（landed・§8.47・client flag `realityCaptureSurfaceClient`(NEXT_PUBLIC_・default off)・`captureCandidateClient.ts`(selectCaptureCandidate=response→DTO+client redaction / fetchCaptureCandidate=gated fetch・DI fetchImpl・enabled=false→fetch 0・fail-open)・AskHero optional morningCaptureCandidate prop→MorningPlanCard 配線(dormant・親未供給→undefined→既存 UI 不変)・fake fetch tests(real network 0)・bridge→banner propagation・15 tests・reality 923）**。**V2 response→MorningPlanCard.captureCandidate の client bridge + AskHero 配線済(dormant・flag default off・既存 UI 完全不変)・live 未表示(親 fetch 未配線)**。次は **A1-5-7-8 live fetch 配線**（AskHero parent が V2 route を fetch→fetchCaptureCandidate→morningCaptureCandidate を AskHero へ・staging fetch・別 GO）→ production canary（別 GO）。**live staging fetch / real network / production 接続 / PlanClient 実 DB/API 変更 / production flag ON / remote は必ず別 GO で停止**。raw を同じ読み取り表面に置かない・column-restricted・fail-closed・no default duration を全段で維持する。
