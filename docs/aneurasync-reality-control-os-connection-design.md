...
### 10.37 A1-7-35 設計（Confirm/Correct Loop）— write 実装は stop gate・設計のみ

A1-7-34（second-self surface）後の confirm/correct loop 設計。**実装一切なし**（write/Alter/本線/公開は stop gate）。詳細: docs/prm-confirm-correct-loop-design.md。

**■ loop**: events→review(M2)→model(M3)→user が surface を見る→「合っている/違う/向き/文脈」→confirm/correct 記録→provenance/重み更新(**certainty は ≤tentative 不変**)→次 surface に co-create。
**■ affordance**: 合っている(confirm・最強 signal・自己認識「自分って そういう人間だったのか」)/違う(reject)/向き(direction)/文脈(context)。
**■ データモデル(migration 不要)**: confirm=新 M2(reviewer=user・approve・provenance)/correct=M3 UPDATE(user_correction・reject は retracted_at で論理削除可逆)。
**■ route**: POST /api/reality/tendency-feedback(flag REALITY_TENDENCY_FEEDBACK_WRITE default OFF・server 再導出・redacted・partial failure)。
**■ 安全**: owner-RLS・certainty no high(user confirm でも上げない)・enum code のみ raw なし・reject 可逆・Alter/本線/公開は別 gate。
**■ CEO 判断**: confirm 記録方式/correct effect/copy/write 実装 GO。
**■ 不接触**: 本 slice docs のみ・実装 0。

### 10.38 A1-7-35 実装（Confirm/Correct Loop）— operator-only dev-preview・confirm/correct/reject 可逆 write

A1-7-35 設計（§10.37）の実装。**operator-only / dev-preview / staging controlled smoke のみ**（Alter 連結・Home/Stargazer 本線・本格 user-facing 公開・production は stop gate）。

**■ 何を**: second-self surface 上で operator が tendency に「合っている / 向きを調整 / 文脈を補う / これは違う」と feedback → **可逆**に記録。confirm=user M2(approve)+新 M3 version(supersedes old)+old retracted / correct=M3 を user_correction で UPDATE(破壊削除なし) / reject=user M2(reject)+M3 retracted_at(可逆)。
**■ 追加/変更**: flag REALITY_TENDENCY_FEEDBACK_WRITE(server default OFF)+NEXT_PUBLIC_REALITY_TENDENCY_FEEDBACK_UI(client)・prm-model-entry-read.ts(PrmModelEntryFeedbackEntry/PRM_MODEL_ENTRY_FEEDBACK_COLUMNS=id 付き column-restricted/prmModelEntryRowsToFeedbackEntries/tendencyKey)・tendency-feedback-core.ts(**pure** executeTendencyFeedback={entries(server 再読込 id 付き),rawRequest,m2,m3Insert,m3Update,nowMs}→parseRequest(tendencyKey+feedback+correctionKind・unknown/invalid fail-closed)→entries.find(tendencyKey===)(entry_not_found fail-closed)→correct=m3Update.update(id,{userCorrection})/reject=user M2 insert→m3Update.update(id,{retractedAtISO})/confirm=user M2(approve)insert→approvedReviewToModelEntryRow({reviewDecisionId,snapshot,supersedesId:entry.id})→m3Insert→m3Update.update(old,{retractedAtISO})・**partial failure 明示**(model_entry_insert_failed/m3_retract_failed/old_retract_failed)・id/raw を return に出さない)・supabase-prm-model-entry-reader.ts(readModelEntriesForFeedback=server-only owner-RLS id 付き select.eq(user_id).eq(user_visible,true).is(retracted_at,null) fail-open)・supabase-prm-model-entry-repository.ts(createSupabasePrmModelEntryUpdater=**UPDATE のみ・破壊削除なし**・update(patch).eq(id).eq(user_id) owner-RLS・空 patch no-op・fail-closed・updater 用 client 型を insert 型と分離=既存 insert mock 不破壊)・second-self-presenter.ts(SecondSelfCard に tendencyKey 追加=feedback 宛先)・app/api/reality/tendency-feedback/route.ts(POST・auth・flag OFF→{disabled}write 0・reader.readModelEntriesForFeedback で server 再読込→executeTendencyFeedback→redacted)・TendencyFeedbackButtons.tsx(別 file・fetch はここだけ・confirm/correct(direction/context)/reject→POST・partial failure 表示・enum のみ free-text なし)・SecondSelfPreviewClient.tsx(feedbackEnabled prop・ON で実 feedback ボタン/OFF で「直す（準備中）」disabled)・dev-second-self/page.tsx(feedbackEnabled=UI flag∧surface flag)・realityTendencyFeedbackCore.test.ts(8)。
**■ staging controlled smoke PASS**（scripts/reality-tendency-feedback-smoke.ts・staging-only guards: URL host ref===STAGING_PROJECT_REF・本番 ref denylist・service_role 検出 fatal・GO flag 必須・env 永続変更なし process env のみ）: 3 M2+3 M3 seed(marker 一意)→**実 adapter**(reader/m2 repo/m3 repo/m3 updater)+pure core を **実 staging DB + RLS** で confirm/correct/reject→14 checks 全 ✅(reader id 付き再読込 3/3・confirm: ok/reviewed/modelEntryCreated/retracted no-partial + user M2(approve)1 + old M3 retracted + 新 version(supersedes)1 + certainty≠high・correct: corrected + user_correction=direction_adjusted 非 retracted + user M2 0・reject: reviewed/retracted + user M2(reject)1 + M3 retracted)→cleanup marker 一致 M3=4/M2=5 DELETE→残 0(痕跡なし)。**M1 は本 smoke で生成 0**(M3 直接 seed)。route HTTP gate(flag OFF→write 0/auth)は本 smoke で未実行=A1-7-33 route と構造同一+core/route unit で担保(delta を明示)。
**■ 守った条件**: client snapshot 不信(server 再読込 id 付き)・entry 無 fail-closed・**破壊削除なし**(retracted_at/supersedes 可逆)・certainty no high(snapshot≤tentative+DB CHECK)・enum code のみ raw/personality/free-text 作らない・id/raw を response/log に出さない・owner-RLS service_role 禁止・flag OFF で write 0 既存不変・partial failure 隠さない・Alter 連結 0・Home/PlanClient/Stargazer 本線 0・notification 0・env 永続 0・production 0。
**■ 自己監査**: tendency-feedback-core 8 PASS(confirm/correct/reject 可逆・entry_not_found/unknown/invalid fail-closed・M2 fail fail-closed・partial failure 2 系統・reviewer=user・certainty≠high・redacted id/raw 出さない)・tsc 55(baseline 不変・新規 InsertableFrom 拡張による既存 mock 破壊を updater client 型分離で解消し回帰 0)・reality 52 files 849 PASS(+8)。
**■ 不接触**: production 0・Vercel/deploy/remote push/PR 0・Alter 連結 0・Home/Stargazer 本線 0・notification 0・env 永続 0。次 stop gate: **A1-7-36 Alter 連結 Design は設計のみ→停止**（Alter 連結実装/本線接続/本格 user-facing 公開/production は CEO 承認 stop gate）。

### 10.39 A1-7-36 設計（PRM ⇄ Alter Bridge）— Alter 連結実装は stop gate・設計のみ

PRM 6 層目 = M3 review 済 tendency を Home Alter の判断に返す設計。**実装一切なし**（Alter 連結実装/本線接続/公開/production は stop gate）。詳細: docs/prm-alter-bridge-design.md。

**■ なぜ**: 「未来の自分が先に試す」の実体＝Alter が一般論でなく**本人の tendency を内側に持って**判断する層。surface/feedback で止まれば PRM は「観測の鏡」止まり。Human OS 5 層の Decision Engine★ に PRM を注入して初めて「判断する第二の自己」になる。
**■ 接続（新機構を作らない）**: 既存 alter route の「**判断傾向（内部参照のみ）**」注入（route.ts:5402–5436・行動傾向を trust-gated hint 注入）/ heartBlock append（5334–5365）/ counterfactualLive kill switch を**踏襲**。新規は pure 2 関数のみ＝resolvePrmContext（現在判断文脈に一致する tendency だけ relevance gating・fail-closed・上限 K・evidence 閾値・rejected 除外・矛盾は揺れとして）+ buildPrmTendencyBlock（非断定「内部参照のみ」block・counter/stillPossible 同梱・「現在発話優先」「確信上げない」明記・verbatim 禁止）。reader は A1-7-34 の readSecondSelfTendencies 再利用（owner-RLS・fail-open）。
**■ 声の制約は再利用**: M3 certainty(≤tentative)→skeleton.confidence_level low/medium へ写像＝既存 validateResponseQuality の STRONG/MODERATE_ASSERT hedge enforcement(7741–7752)がそのまま効く。新 hedge を作らない。ForceBalance 数値上書きは v1 でしない（判断を静かに歪めるリスク・hint のみ・将来 sub-phase）。
**■ 原則**: tendency-not-trait・断定しない(confirm でも上げない)・本人の現在入力 > 過去 tendency・counter/stillPossible を Alter にも見せる・user_correction 尊重(rejected 除外/direction·context 反映)・verbatim 引用しない・flag OFF/0 件/解決不能で現行 Alter と bit 同一。
**■ 安全/rollback**: 本層は read-only(M3 読むだけ・書かない)・副作用は全て prompt 経由・**REALITY_ALTER_BRIDGE_LIVE=false(default OFF)で即時完全巻き戻し**・他経路不接触の append のみ。
**■ CEO 判断**: (a)hint のみ/ForceBalance override (b)flag 置き場(STARGAZER_FLAGS/PLAN_FLAGS) (c)trust 開始点 (d)K/E_MIN (e)実装 GO。
**■ 不接触**: 本 slice docs のみ・実装 0・Alter 連結 0・本線 0・公開 0・production 0。

### 10.40 A1-7-36 実装（PRM ⇄ Alter Bridge）— flag default OFF・本線 dormant

CEO GO（2026-06-09）で A1-7-36 設計（§10.39）を実装。**flag REALITY_ALTER_BRIDGE_LIVE default OFF**＝OFF 時は alter route が**現行と bit 同一**（本線 dormant・production enable は別 gate）。

**■ 採用判断**: 設計の (a)hint のみ / (b)STARGAZER_FLAGS / (c)trust≥1 踏襲 / (d)K=2・E_MIN=4 を採用。ForceBalance override は v1 でしない。
**■ 追加/変更**: STARGAZER_FLAGS.realityAlterBridge(default OFF)・`lib/plan/reality/learning/prm-alter-bridge.ts`(pure: resolvePrmContext=band fail-closed+rejected/薄い/counter支配除外+上位K / buildPrmTendencyBlock=非断定「内部参照のみ」block・counter/訂正併記・現在発話優先・確信上げない・verbatim禁止 / bandFromHour=深夜 none)・alter route 注入(既存「判断傾向（内部参照のみ）」注入の直後に並置・`STARGAZER_FLAGS.realityAlterBridge && hasMinTrust && responseMode≠clarify` gate・readSecondSelfTendencies(A1-7-34 再利用 owner-RLS fail-open)→resolve→build→homeSystemPrompt += ・try/catch fail-open)・realityPrmAlterBridge.test.ts(12)。
**■ 声の制約は再利用**: certainty は構造的に ≤tentative・block 内に「確信を上げない」明記・既存 validateResponseQuality hedge enforcement を通す（新 hedge なし）。
**■ 検証**: bridge 12 PASS・reality 53 files 861 PASS(+12)・tsc 55(baseline 不変・**alter route 15 で新規 0**)。flag OFF で homeSystemPrompt 不変＝現行 Alter 不変。flag ON の shadow/staging canary は次 operator step（read-only ゆえ低リスク・LLM E2E は enable 意図時に実行）。
**■ 不接触**: production 0・flag enable 0・Vercel/deploy/remote/PR 0・他 alter 経路 0・Home/Plan 本線 0・notification 0。次: 未構築フェーズ（4 種記憶・empty-day 等）を pure から。

> A1-5-0…§10.39 / **A1-7-36 実装 PRM ⇄ Alter Bridge（§10.40・**flag REALITY_ALTER_BRIDGE_LIVE default OFF で本人傾向を alter route に「内部参照のみ」hint 注入・OFF 時 bit 同一・staging enable は別 gate**・A1-7-36 完了）。CEO GO で設計(§10.39)を実装。採用: hint のみ / STARGAZER_FLAGS.realityAlterBridge / trust≥1 / K=2・E_MIN=4・ForceBalance override v1 せず。prm-alter-bridge.ts(pure): resolvePrmContext(band 導けねば [] fail-closed・contextDimension=band ∧ contextValue=現在band ∧ userCorrection≠rejected ∧ evidenceCount≥4 ∧ counterCount<evidenceCount のみ・evidence 上位 2)・buildPrmTendencyBlock(非断定「# 本人の傾向（内部参照のみ・断定しない・そのまま引用しない）」・counterCount>0 で「反証 N 件・決めつけない」/0 で「手がかり少なめ」併記・direction_adjusted/context_refined を併記・末尾に「今この人が言っていることを最優先し、この傾向で確信を上げないこと」)・bandFromHour(5-11 morning/11-17 afternoon/17-23 evening/深夜・NaN none=注入しない)。alter route(app/api/stargazer/alter/route.ts): 既存 decision-pattern「判断傾向（内部参照のみ）」注入(5403-5441)の直後に並置・`STARGAZER_FLAGS.realityAlterBridge && hasMinTrust(discreteTrustLevel≥1) && responseMode≠clarify` gate・createSupabasePrmModelEntryReader(A1-7-34 再利用).readSecondSelfTendencies(owner-RLS fail-open[])→bandFromHour(new Date().getHours())→resolvePrmContext→buildPrmTendencyBlock→homeSystemPrompt+=・try/catch fail-open。声の制約再利用: certainty≤tentative+block「確信上げない」+既存 validateResponseQuality hedge がそのまま効く(新 hedge なし)。検証: realityPrmAlterBridge 12 PASS(fail-closed/band 一致/dimension 除外/rejected·薄い·counter 支配 除外/上位 K/direction·context 残す/block 断定·trait 語なし/counter·現在発話優先·確信上げない·verbatim 禁止 明記/方向動詞/訂正併記/bandFromHour)・reality 53 files 861 PASS(+12)・tsc 55(baseline 不変・alter route 15 新規 0)。flag OFF で現行 Alter bit 同一。不接触: production 0・flag enable 0・Vercel/deploy/remote/PR 0・他 alter 経路 0・Home/Plan 本線 0・notification 0**。**PRM⇄Alter Bridge を flag default OFF で実装(本人傾向を alter に内部参照 hint 注入)・既存「判断傾向（内部参照のみ）」seam に並置し新機構作らず・certainty→confidence_level で既存 hedge 再利用・relevance fail-closed・OFF で bit 同一・staging enable は別 gate・次は未構築フェーズ(4 種記憶・empty-day)を pure から**。raw を同じ読み取り表面に置かない・column-restricted・fail-closed・seedRef を client に出さない・production hard block を全段で維持する。

> A1-5-0…§10.38 / **A1-7-36 PRM ⇄ Alter Bridge 設計（§10.39・**M3 review 済 tendency を Home Alter 判断に「内部参照のみ」hint として注入する設計・実装一切なし・Alter 連結実装/本線接続/公開/production は stop gate**・A1-7-36 設計提出→停止）。詳細 docs/prm-alter-bridge-design.md。なぜ: 「未来の自分が先に試す」の実体＝Alter が一般論でなく本人 tendency を内側に持って判断する PRM 6 層目・Human OS 5 層の Decision Engine★ への本人モデル注入・surface/feedback で止まれば観測の鏡止まり。接続(新機構を作らない): 既存 alter route の「判断傾向（内部参照のみ）」注入(app/api/stargazer/alter/route.ts:5402–5436・stargazer_alter_patterns を observation_count≥5∧confidence≥0.3 で trust-gated hint 注入)/heartBlock append(5334–5365 buildUnifiedHeartState→homeSystemPrompt+=)/counterfactualLive kill switch(STARGAZER_FLAGS 6591)を踏襲・PRM block をその隣に並置。新規は pure 2 関数のみ: resolvePrmContext(現在判断文脈の band/durationBucket 等に一致する M3 tendency だけ選別・relevance は fail-closed=導けねば注入しない・evidenceCount≥E_MIN(例4)∧counter 非支配・user_correction rejected 除外/direction_adjusted·context_refined 反映・上限 K(例2)・相反 tendency は潰さず揺れとして提示)+buildPrmTendencyBlock(pure 非断定「内部参照のみ」block・counterCount/stillPossible 同梱で過断定防止を Alter にも見せる・「今この人の発話を優先」「確信を上げる根拠にしない」明記・verbatim 引用禁止)。reader は新規実装せず A1-7-34 createSupabasePrmModelEntryReader.readSecondSelfTendencies 再利用(SecondSelfTendency[]・owner-RLS・user_visible∧非retracted・fail-open[])。声の制約は再利用: M3 certainty(≤tentative)→skeleton.confidence_level low/medium 写像で既存 validateResponseQuality(alterHomeAdapter.ts:7699–7800)の STRONG_ASSERT/MODERATE_ASSERT hedge enforcement(7741–7752)+prompt hedge(2603–2620)+sanitizeTraitInversions(7811–7892)がそのまま注入後文章に効く(新 hedge を作らない=最重要安全再利用)。ForceBalance(computeForceBalance 456–599)への数値上書きは v1 でしない(prompt hint と違い判断を静かに歪める・既存 decision-pattern も hint であって override でない・効果と安全を観測後 将来 sub-phase で慎重に)。gating: 既存 hasMinTrust(discreteTrustLevel≥1)+responseMode≠clarify 踏襲・新 env REALITY_ALTER_BRIDGE_LIVE(default OFF kill switch)・flag OFF/0 件/解決不能で homeSystemPrompt 一切変えず現行 Alter と bit 同一。原則: tendency-not-trait(文脈束縛のまま「夜の予定では見送りやすい傾向」≠「怠惰」)・断定しない(confirm 済でも確信上げない=A1-7-35 継承)・本人の現在入力>過去 tendency(directly-observed-now>inferred-history)・counter/stillPossible を Alter にも見せる・user_correction 尊重・verbatim 引用しない(内部参照のみ)・fail-open 既存挙動不変。失敗シナリオ/rollback: F1 歪み→hint のみ+現在発話優先/F2 過断定→certainty 写像で既存 hedge 通す/F3 stale→retracted_at 除外+rejected 除外+evidence/counter 同梱/F4 相反→揺れとして提示/F5 誤マッチ→relevance fail-closed+上限 K/F6 機械露出→verbatim 禁止+raw 元々なし/F7 訂正無視→user_correction 反映/F8 本線汚染→flag OFF で no-op・他経路不接触 append のみ・rollback=REALITY_ALTER_BRIDGE_LIVE=false で即時完全(read-only=データ変更なし)。実装最小 slice(承認後): flag→resolvePrmContext(pure+test)→buildPrmTendencyBlock(pure 非断定+test)→alter route 注入(flag-gated・readSecondSelfTendencies 再利用・heartBlock 隣 append・fail-open・他経路不接触)→shadow 監査(注入 ON/OFF で over-assert/歪み/hedge 違反比較・validateResponseQuality 違反増えない)→staging canary(operator dogfood)。CEO 判断: (a)hint のみ/ForceBalance override 含む(b)flag 置き場(STARGAZER_FLAGS.realityAlterBridge/PLAN_FLAGS import)(c)trust 開始点(d)K/E_MIN 妥当性(e)実装 GO=operator dev-preview/staging canary まで。不接触: 本 slice 設計のみ・実装 0・Alter 連結 0・Home/Stargazer 本線 0・公開 0・production 0**。**PRM⇄Alter Bridge を設計提出(「未来の自分が先に試す」の実体・Decision Engine★ に本人 tendency を内部参照 hint として注入)・既存「判断傾向（内部参照のみ）」注入と heartBlock append と counterfactual kill switch を踏襲し新機構を作らない・声の制約は certainty→confidence_level 写像で既存 hedge を再利用・ForceBalance 上書きは v1 でしない・read-only で flag 1 本巻き戻し・実装は CEO stop gate ゆえ設計のみで停止**。raw を同じ読み取り表面に置かない・column-restricted・fail-closed・seedRef を client に出さない・production hard block を全段で維持する。

> A1-5-0…§10.37 / **A1-7-35 実装 Confirm/Correct Loop（§10.38・**operator-only dev-preview・confirm/correct/reject を可逆に記録・staging controlled smoke 14/14 PASS・cleanup 残 0・Alter/本線/公開/production 不接触**・A1-7-35 完了）。flag REALITY_TENDENCY_FEEDBACK_WRITE(server default OFF)+NEXT_PUBLIC_REALITY_TENDENCY_FEEDBACK_UI・tendency-feedback-core.ts(pure executeTendencyFeedback: entries(server 再読込 id 付き)+rawRequest{tendencyKey,feedback,correctionKind}+m2/m3Insert/m3Update→parseRequest(unknown_feedback/invalid_correction_kind fail-closed)→entries.find(tendencyKey===)(entry_not_found fail-closed)→**confirm**=user M2(reviewer=user,approve)insert→approvedReviewToModelEntryRow({reviewDecisionId:ids[0],snapshot,supersedesId:entry.id})→m3Insert→m3Update.update(old,{retractedAtISO})=新 version が old を supersede し old を論理 retract(破壊削除でない)/**correct**=m3Update.update(id,{userCorrection:direction_adjusted|context_refined})=既存 M3 UPDATE のみ M2 作らない 非 retracted/**reject**=user M2(reject)insert→m3Update.update(id,{retractedAtISO})可逆・partial failure(model_entry_insert_failed/m3_retract_failed/old_retract_failed)を隠さず redacted・id/raw を return に出さない)・prm-model-entry-read.ts に PrmModelEntryFeedbackEntry/PRM_MODEL_ENTRY_FEEDBACK_COLUMNS(id 付き column-restricted)/tendencyKey(context_dimension:context_value:tendency_direction)追加・supabase-prm-model-entry-reader.ts に readModelEntriesForFeedback(server-only owner-RLS id 付き fail-open)・supabase-prm-model-entry-repository.ts に createSupabasePrmModelEntryUpdater(UPDATE のみ 破壊削除なし update(patch).eq(id).eq(user_id) owner-RLS 空 patch no-op fail-closed・updater client 型を insert 型と分離し既存 insert mock 不破壊)・second-self-presenter.ts SecondSelfCard に tendencyKey(feedback 宛先)・route app/api/reality/tendency-feedback(POST auth flag OFF→{disabled}write 0・reader で server 再読込→executeTendencyFeedback→redacted)・TendencyFeedbackButtons.tsx(別 file・fetch ここだけ・confirm/correct(direction/context)/reject・enum のみ free-text なし)・SecondSelfPreviewClient.tsx(feedbackEnabled で実ボタン/OFF で disabled)・dev-second-self page(feedbackEnabled=UI flag∧surface flag)・realityTendencyFeedbackCore.test.ts 8。staging smoke(scripts/reality-tendency-feedback-smoke.ts・staging-only ref guard+本番 denylist+service_role 検出 fatal+GO 必須+env 非永続): 3 M2+3 M3 seed→実 adapter+core を実 staging DB+RLS で confirm/correct/reject→14 checks 全 PASS(reader id 付き 3/3・confirm ok/reviewed/modelEntryCreated/retracted no-partial+user M2(approve)1+old retracted+新 version(supersedes)1+certainty≠high・correct corrected+user_correction=direction_adjusted 非 retracted+user M2 0・reject reviewed/retracted+user M2(reject)1+M3 retracted)→cleanup marker 一致 M3=4/M2=5→残 0(痕跡なし)・M1 生成 0・route HTTP gate は A1-7-33 構造同一+unit で担保(delta 明示)。守った条件: client snapshot 不信(server 再読込)・entry 無 fail-closed・破壊削除なし(retracted_at/supersedes 可逆)・certainty no high(≤tentative+DB CHECK)・enum code のみ raw/personality/free-text なし・id/raw を response/log に出さない・owner-RLS service_role 禁止・flag OFF write 0 不変・partial failure 隠さない・Alter 連結 0・Home/PlanClient/Stargazer 本線 0・notification 0・env 永続 0・production 0。tsc 55(baseline 不変)・reality 52 files 849 PASS(+8)。不接触: production 0・Vercel/deploy/remote push/PR 0・Alter 連結 0・Home/Stargazer 本線 0・notification 0・env 永続 0**。**confirm/correct/reject loop を operator-only dev-preview で実装+staging smoke 14/14 PASS・confirm=user M2+新 M3 version(supersedes)+old retracted/correct=M3 user_correction UPDATE/reject=M2+retracted_at すべて可逆 破壊削除なし・certainty は user confirm でも ≤tentative 不変(断定しない)・cleanup で痕跡 0・次は A1-7-36 Alter 連結は設計のみで停止**。raw を同じ読み取り表面に置かない・column-restricted・fail-closed・seedRef を client に出さない・production hard block を全段で維持する。

> A1-5-0…§10.36 / **A1-7-35 Confirm/Correct Loop 設計（§10.37・**second-self surface 後の confirm/correct loop 設計・実装一切なし・write/Alter/本線/公開は stop gate**・A1-7-35 設計提出→停止）。詳細 docs/prm-confirm-correct-loop-design.md。loop: events→review(operator M2)→model(M3)→user が surface を見る→「合っている/違う/向きが違う/文脈が違う」→confirm/correct 記録→model の provenance/重みを更新(certainty は ≤tentative のまま=断定しない)→次 surface に反映 co-create=本人が形作る第二の自己。affordance: 合っている(自己認識の確認 最強 signal「自分って そういう人間だったのか」の瞬間)/違う(reject)/向きが少し違う(direction)/文脈が違う(context)・copy 非断定共同編集。データモデル(M2/M3 既存列で完結 migration 不要): confirm=新 M2 review decision(reviewer=user・decision=approve・同 proposal_fingerprint・server 再導出 snapshot)insert=ユーザー自身が review した provenance(reviewer=user は M2 CHECK 済)・既存 M3 は certainty 不変・任意で user M2 を review_decision_id にした新 M3(supersedes_id=旧 M3)で version 化、correct=既存 M3 UPDATE(owner・M3 UPDATE policy あり)で user_correction set+reject は retracted_at=now で論理削除(surface から消える 可逆)・enum code のみ raw/personality 作らない。route: POST /api/reality/tendency-feedback({proposalFingerprint,feedback:confirm/reject/direction/context}・auth・flag REALITY_TENDENCY_FEEDBACK_WRITE default OFF・server 再導出で proposal/M3 解決 client 値不信・confirm→M2 insert(A1-7-30 repo 再利用)/correct→M3 UPDATE(新 repo method or RPC)・redacted return・fail-open/partial failure 明示)。model への効き(certainty 不変): confirm→provenance 強化(user 確認回数・decay 維持/上げ)だが certainty ≤tentative のまま(断定しない絶対原則・自己認識は変わりうる)・reject→retracted_at で surface 除外 可逆・direction/context→user_correction を surface 併記・将来 aggregation で重み調整・directly observed>inferred(user feedback を最優先証拠として将来 aggregation 反映 設計メモ)。安全契約全維持: owner-RLS service_role 禁止・flag default OFF・redacted・certainty no high(user confirm でも上げない)・correct は enum code のみ raw/personality なし・reject は retracted_at で可逆(破壊的でない)・Alter 連結/Home Stargazer 本線/本格 user 公開は別 gate。実装最小 slice(承認後): flag→M3 update repository method(user_correction/retracted_at owner-RLS fail-open)+mapper+test→tendency-feedback route core(server 再導出→confirm M2 insert/correct M3 update redacted partial failure)+test→route handler→second-self surface の disabled button を実 feedback ボタンに→staging controlled smoke(M3 作成→confirm/correct→検証→cleanup)。CEO 判断: (a)confirm 記録方式(user M2 のみ/+新 M3 version)(b)correct effect(reject=retracted/direction·context 将来 aggregation 重み)(c)copy(confirm=自分って そういう人間だったのか を起こす/correct=尊厳ある直せます)(d)write 実装 GO=correction write 有効化判断。不接触: 本 slice 設計のみ・実装 0・write 0・Alter 連結 0・Home/Stargazer 本線 0・production 0・user-facing 公開 0**。**confirm/correct loop を設計提出・user が confirm/correct で第二の自己を co-create・confirm=自己認識の最強 signal だが certainty は ≤tentative 不変(断定しない)・correct は M3 UPDATE/reject は retracted_at で可逆・migration 不要・write 実装は CEO stop gate ゆえ設計のみで停止**。raw を同じ読み取り表面に置かない・column-restricted・fail-closed・seedRef を client に出さない・production hard block を全段で維持する。
