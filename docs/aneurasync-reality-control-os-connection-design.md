...
> A1-5-0…§8.33 / **A1-5-5f real capture smoke（完了・§8.34・実 LLM 1 call + 実 RPC write 1 → staging hjcr に seed1+evidence1 atomic → candidateCount=1 → cleanup→0/0・厚い preflight・USER_A self-pin・service_role 0・raw/apiKey 非出・committed code 0）**。**capture pipeline 全段（実 Gemini→runCaptureService→real RPC→atomic write→read seam→candidate→cleanup）が実 staging で end-to-end 実証完了**。次は **A1-5-5g route 接続**（alter-morning/plan・flag+canary・observe→write・**必ず別 GO で停止**＝runtime/route/UI 接続境界）。**runtime/route/UI/実 LLM の route 接続は必ず別 GO で停止**。raw を同じ読み取り表面に置かない・column-restricted・fail-closed・no default duration を全段で維持する。
