...
> A1-5-0 は design / read-only audit。実装（A1-5-1 shadow wiring）は **本 doc bank 後の別 GO** で判断。raw を持ち込まない・column-restricted・redaction guard・fail-closed・flag off default・no DB・no push を全段で維持する。
