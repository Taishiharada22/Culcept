...
> A1-5-0…§8.37 / **A1-5-5g-4 write-mode 接続 + route write-mode staging smoke（landed・§8.38・runner write mode 実接続（observe/write 差=注入 writeClient のみ）+ helper decideCaptureMode（kill>LIVE>OBSERVE>none）+ `fireMorningCapture(…, rpcClient)`・route が認証済 client を RPC 先に・実 route POST(LIVE) で outcome=captured・seed1+evidence1・candidateCount=1・llmCall=1・rpcCall=1・cleanup→0・USER_A rows 総数 0・response 不変・raw発話保存なし・service_role 0・staging のみ・runner 3+helper 8 tests・reality 801）**。**capture pipeline 全段が実 route 経由で staging に end-to-end 実証済**（実 LLM→route→write-mode→atomic RPC→seed+evidence→candidate→cleanup）。observe/write は flag 切替（default 両 off=production 挙動変更ゼロ）。次は **A1-5-5h**（候補: UI/PlanClient 接続 or generateCandidates runtime 統合）。**UI/PlanClient/production/PRM·correction 実接続/remote は必ず別 GO で停止**。raw を同じ読み取り表面に置かない・column-restricted・fail-closed・no default duration を全段で維持する。
