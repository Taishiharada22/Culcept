...
> A1-5-0…§8.32 / **A1-5-5e Observe-Mode（landed・§8.33・pure `summarizeWouldCapture`（redacted would-capture projection・10 tests）+ observe smoke：実 Gemini 1 call → runCaptureService(fake write・DB write 0) → wouldCapture=true/wouldEvidence=true/validation ok・raw 非漏洩実測（payload·result 共に false））**。**capture pipeline 全段（gate→実 extractor→validate→intake→mapper→write seam→would-capture）が write OFF で end-to-end 実証**。次は **A1-5-5f real capture smoke**（実 extractor + 実 RPC write・staging・1 件・cleanup・**必ず別 GO で停止**）。**DB write/RPC/runtime/route/UI 接続は必ず別 GO で停止**。raw を同じ読み取り表面に置かない・column-restricted・fail-closed・no default duration を全段で維持する。
