...
> A1-5-0…§8.47 / **A1-5-7-8 Consumer Wiring Audit + Inert Submit Bridge（landed・§8.48・audit: production morning=/api/stargazer/alter(useAlterChat.sendMessage)・V2 route /api/alter-morning/plan は UI 未消費＝capture pipeline は別 route・live V2 fetch は重複(追加 LLM+capture write+surface read)ゆえ **live 延期**・正道は V2 migration or surface を /api/stargazer/alter へ移す(別 GO)・inert `buildCaptureCandidateRequestBody`(最小 body=utterance のみ)+`submitForCaptureCandidate`(gated・dormant・AneurasyncHome 非変更)・+7 tests・reality 930・production flow 完全不変）**。**V2 fetch 接続点を audit 確定・live は重複ゆえ延期・inert submit bridge 完成(dormant)**。次は **A1-5-7-9 / 別トラック**: ① V2 route を production morning route 化(Wave 3 migration・大規模・別 GO) ② capture surface を /api/stargazer/alter へ移す(別 GO)。いずれかで live 表示が筋良く完成。**live staging fetch / production 接続 / PlanClient 実 DB/API 変更 / PRM·correction 実接続 / remote は必ず別 GO で停止**。raw を同じ読み取り表面に置かない・column-restricted・fail-closed・no default duration を全段で維持する。
