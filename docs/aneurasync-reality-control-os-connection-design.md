...
> A1-5-0…§8.34 / **A1-5-5g-0/1 Route Audit + Capture Route Runner Skeleton（landed・§8.35・server-only `runCaptureRouteObserver`（observe=gate→extractor→capture service(dry-run fake write)→would-capture / write=fail-closed・never-throw=response 不変）+ `PLAN_FLAGS.realityCaptureObserve`（observe flag・live と別）・13 tests・route.ts 不変）**。第一接続先=alter-morning/plan（utterance 入力・fire-and-forget で response 不変）。次は **A1-5-5g-2 route wiring**（route.ts に fire-and-forget observe を差す・flag+canary・**runtime/route/UI 接続境界＝必ず別 GO で停止**）。**route.ts 変更/runtime/DB write/実 LLM runtime は必ず別 GO で停止**。raw を同じ読み取り表面に置かない・column-restricted・fail-closed・no default duration を全段で維持する。
