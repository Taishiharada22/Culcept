...
> A1-5-0…§8.40 / **A1-5-7-0/1 Candidate Surface Contract（landed・§8.41・pure presenter `presentCandidateSurface(summary, candidatePlacements?)→CandidateSurfaceDTO{hasCandidate,candidateCount,status,items}`・CandidateDraft 非 surface(UUID 源を境界に入れない)・summary+placement(seedRef drop) から構成・isSurfaceableCandidate で prm_typical/weak fail-closed・confidence band 化・断定は "候補があります" まで・18 tests・reality 840・route 未変更・response contract は additive 案を docs 化）**。**candidate path 末端に redaction 境界を固定・surface DTO 契約確定・route/UI 未接続**。次は **A1-5-7 visible route integration**（候補: route response に `captureCandidate?` を additive 追加・別 GO）→ その後 UI 接続（別 GO）。**route response 変更 / UI·PlanClient 接続 / production / PRM·correction 実接続 / remote は必ず別 GO で停止**。raw を同じ読み取り表面に置かない・column-restricted・fail-closed・no default duration を全段で維持する。
