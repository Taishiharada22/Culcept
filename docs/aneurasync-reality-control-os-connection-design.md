...
> A1-5-0…§8.41 / **A1-5-7-2 Route Surface Integration Plan + Response Assembler（landed・§8.42・fire-and-forget↔response surface を A/B/C 整理→**C案推奨**(pending seed read-only consumption・fail-open・LLM await 不要)・pure `appendCaptureCandidateToMorningResult<T>`(fail-open additive: candidate 無→元 result 完全一致 / 有→captureCandidate 1 key)+`redactCaptureCandidateSurface`(allowlist 再構築・最終 redaction)・generic decoupled・17 tests・reality 857・route.ts 不変）**。**response surface 方針を C案 に固定・additive fail-open contract 確定・route 未接続**。次は **A1-5-7-3 pending seed read-only consumption**（staging で pending seed を read→consumption→surface・別 GO）→ その後 visible route integration（route.ts に assembler 配線・別 GO）→ UI 接続（別 GO）。**route.ts 変更 / route response 実変更 / UI·PlanClient 接続 / DB / production / remote は必ず別 GO で停止**。raw を同じ読み取り表面に置かない・column-restricted・fail-closed・no default duration を全段で維持する。
