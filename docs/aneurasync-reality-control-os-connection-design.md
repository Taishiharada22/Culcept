...
> A1-5-0…§8.15 / **A1-5-4-0 capture boundary 設計 + A1-5-4a pure seed capture mapper（landed・§8.16・raw-free・DB INSERT 0・seed_explicit evidence draft・capture→candidateCount>0 を fixture 実証・barrel 非 export）**。次は A1-5-4b（実 capture: user-RLS client で atomic INSERT・raw parse は更に別段・要強い GO）。**capture→seed/evidence draft→（既存 read projection）→enrich→candidate の全経路が fixture で実証済**。raw を同じ読み取り表面に置かない・column-restricted・fail-closed・no default duration・no DB write を全段で維持する。
