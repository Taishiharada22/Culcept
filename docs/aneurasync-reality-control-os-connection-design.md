...
> A1-5-0…§8.39 / **A1-5-6-2 real consumption smoke（landed・§8.40・doc-only・untracked harness 実行後削除・file capture 1回・controlled write(runCaptureService 直)→real DB read(allowed col のみ)→runCapturedSeedConsumptionShadow→candidateCount=1/wouldCandidate=true→cleanup rows=0/userTotalReality=0・llmCall=1・rpcCall=1・seed1+evidence1・readRawClean・summaryRedacted・service_role 0・raw発話保存なし・staging のみ）**。**consumption runner が実 staging の実 row を read seam 経由で消費し candidate 化することを end-to-end 実証済**（capture pipeline: 書く→読む→候補化→cleanup の全周）。次は **A1-5-6 visible integration**（候補: consumption summary を route response / UI に反映 or generateCandidates runtime 統合・別 GO）。**route response 反映 / UI·PlanClient 接続 / production / PRM·correction 実接続 / remote は必ず別 GO で停止**。raw を同じ読み取り表面に置かない・column-restricted・fail-closed・no default duration を全段で維持する。
