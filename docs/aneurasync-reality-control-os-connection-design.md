...
> A1-5-0 は design / read-only audit。**A1-5-1a は no-call-site skeleton（landed・§8.6）**。実 runtime 接続（A1-5-1b：column-restricted real read + 実 call-site）は **別 GO** で判断。raw を持ち込まない・column-restricted・redaction guard・fail-closed・flag off default・no DB・no push を全段で維持する。
