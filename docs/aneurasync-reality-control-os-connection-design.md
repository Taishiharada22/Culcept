...
> A1-5-0…§8.31 / **A1-5-5d-2b-retry（§8.32・初回 実 LLM call 成功・model gemini-2.5-pro・API call 1・outcome extracted・validation ok・explicitDuration{60,high}・token 1108・latency 6.8s・redacted・committed code 0）**。host→core→Gemini→parse→map→validate の実経路を 1 回成功。raw/prompt/response/apiKey 非漏洩。capture pipeline の **実 LLM extractor 単体が end-to-end 実証完了**（host→core→実 Gemini→validateExtractorOutput ok）。次は **A1-5-5e observe-mode smoke**（実 extractor + capture service・write OFF・redacted would-capture・別 GO）。**DB write/runtime/capture service 接続は必ず別 GO で停止**。raw を同じ読み取り表面に置かない・column-restricted・fail-closed・no default duration を全段で維持する。
