...
- **固定 = 不可侵化** であり、以後の変更は rev 追記方式（既存本文削除禁止）
...
| 面 | 役割 | 発火条件 | 位置 |
|---|---|---|---|
| **対話面** | CoAlter と 2 人が**会話する場**。発話サイクル S0-S8 はすべてここで完結 | 常時（`enabled` 以上）。自動 S0 常駐、自動 S1 昇格、chip 応答、自由テキスト返答（`@coalter` 入力） | 画面上部の専用レイヤー（v1.1 §3.1） |
...
**`disabled` vs `inactive` の語義差（本契約で固定）**:
- **`disabled`**: ユーザー（ペアのいずれか / 両方）が**明示的に OFF** にした状態（opt-out）。再度 enable するには明示操作が必要。`pending_consent` には戻らず、enable 操作で直接 `enabled` へ
- **`inactive`**: 初期状態 or 同意未取得で**まだ起動していない**状態（not-yet-on）。同意フローを経て `pending_consent` → `enabled` へ進行する入口側

両者はともに「上部レイヤー非表示」になるが、**原因が異なる**（明示拒否 vs 未起動）ため遷移規則が別。master §5 の起動状態機械はこの差を前提に設計されている。
...
│  disabled ⇄ pending_consent → enabled ⇄ active             │
│                                        │                    │
│                                        ↓ active 中のみ        │
│  ┌─────────────────────────────────────────────────────┐   │
...
CEO が本書を固定判定する際、以下を確認してほしい（4 点締め付け反映済み）:

1. **契約点 ① 二層 surface**: 「対話面での自由テキスト会話」まで含めて良いか（§1.1 / §1.6-5）。§1.1.1 で追加した自由テキスト返答最小粒度（1 サイクル = 1 アクティブ返信、同時入力 FIFO、発話中上書き禁止、出力面の自動送信禁止）の妥当性
2. **契約点 ② 直交**: `active` 中に S0-S8 が複数サイクル回ることを許容して良いか（§2.2 重要ブロック）。§2.1 に追加した `disabled`（明示 OFF）vs `inactive`（未起動）の語義差の妥当性
3. **契約点 ③ 分離**: 「Stage 1 を走らせない通常モード S5」を許容して良いか（§3.6-4）
4. **契約点 ④ 命名**: external（`F-1`/`F-2`）↔ internal（`F1`/`F2`）の normalize 規則（§4.2）で良いか、あるいは external 側も `F_1`/`F_2` に統一する等を好むか
5. **§6 追記計画**: 11 行の追記案で過不足ないか。追記タイミングは本書固定**直後**か、P1 着手前まで遅延させるか
6. **snapshot rev2 固定**: §0.3 で明記した「本契約は handoff rev 2 を固定前提とし、rev 3 以降は本契約 rev 追記で対応」の運用方針で良いか

上記 6 点に CEO 判断があれば v0.2 rev で反映、無ければ v0.1 を固定確定として §6 追記計画の承認待ちに入る。
