...
CEO が本書を固定判定する際、以下を確認してほしい:

1. **契約点 ① 二層 surface**: 「対話面での自由テキスト会話」まで含めて良いか（§1.1 / §1.6-5）
2. **契約点 ② 直交**: `active` 中に S0-S8 が複数サイクル回ることを許容して良いか（§2.2 重要ブロック）
3. **契約点 ③ 分離**: 「Stage 1 を走らせない通常モード S5」を許容して良いか（§3.6-4）
4. **契約点 ④ 命名**: `F1` / `F2` の enum 表記（ハイフン無し）で良いか、あるいは `F_1` / `F_2` 等を好むか（§4.2）
5. **§6 追記計画**: 11 行の追記案で過不足ないか。追記タイミングは本書固定**直後**か、P1 着手前まで遅延させるか

上記 5 点に CEO 判断があれば v0.2 rev で反映、無ければ v0.1 を固定確定として §6 追記計画の承認待ちに入る。
