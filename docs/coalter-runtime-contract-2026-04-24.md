...
**不可侵規則（統合契約 §3.6-2 継承）**:
- signal 検出は `executor.understanding.*` bus に**一切 emit しない**（Stage 1 の生進捗バー化を防ぐ）
...
2. signal 検出は `presence.state.*` bus 経由。`executor.understanding.*` bus には一切 emit しない
...
2 人のクライアント間で同期すべき CoAlter 関連状態を網羅する:

| 同期対象 | 粒度 | 正本 |
|---|---|---|
| **executor availability** | ペア永続 | master §5 / 統合契約 §2.1 |
| **Presence 状態（S0-S8）** | 発話サイクル単位、ペア共有 | 統合契約 §2.2 |
| **Action Mode** | ターン単位、ペア共有（ただし UI 表現は Presence 非依存） | phase2 凍結 |
| **発話本文カード**（対話面） | 発話単位 | v1.1 §3.1 / UI spec §4 |
| **共有メモリ surface** | 項目単位 | v1.1 §10 / UI spec §8 |
| **提案カード** | 提案単位 | UI spec §4.3.8 |
| **mode**（通常 / Daily / Travel） | active セッション単位、ペア共有 | v1.1 §5 |

**非同期対象**（ペア共有しない）:
- 各クライアントの入力欄下書き（draft）
- 各クライアントのスクロール位置・焦点
- 各クライアントのローカル UI 状態（アニメ中間フレーム等）

### 2.2 同期媒体（source of truth）

**pair-chat server を source of truth とする**。client は server を購読する非対称モデル。
...
1. 同期 source of truth は **pair-chat server**。client → client 直接同期を禁止
2. **eventually consistent** 前提。完全同時整合性を要求しない
3. client optimistic update と server state 矛盾時は **server が勝つ**
4. Presence state は **pair 単位で一意**（デバイスごと・ユーザーごとに異なる state を持たない）
5. 入力欄下書き・スクロール位置・ローカル UI 状態は**同期対象外**
...
| **2** | dignity cooldown 中か | **発火を棄却、静寂維持**。応答しない。CoAlter は応答不在で尊厳を守る |
| **3** | rupture cooldown 中か | **S1 昇格を棄却**（発火自体は受け付ける）。S0 常駐を維持し、短い受け返しのみ（speech template 側で文面規定）。判断・提案には進まない |
...
- **rupture・dignity は超越 cooldown**。`@coalter` 強制でも上書きしない。関係の断絶や尊厳の危機に対して、ユーザーの明示要求があっても CoAlter は沈黙 / 最小応答を選ぶ（CoAlter は「呼ばれたら必ず出る」存在ではない）
...
4. **dignity / rupture は超越 cooldown**（`@coalter` 強制でも上書き不可）
