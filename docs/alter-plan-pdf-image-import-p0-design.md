...
## §0.4. P0-1 v1.2 補正（GPT 3 点補正の精査 + 私の深掘り 3 点）★★ 最新・最優先

> v1.1（§0.5）に対する更なる上書き補正。矛盾時は本セクション（§0.4）が最優先。GPT「P0-2 GO（3 補正つき）」を反映。

### GPT 3 点補正 → 全採用 + 深化

| # | GPT 補正 | 採用形 |
|---|---|---|
| 1 | real 18 件を **「同一テンプレ別月」で厚く** | ✅ 採用。再現性評価が欠けていた → **同一テンプレ別月を core に再編**（§0.4.1） |
| 2 | KPI を **auto / assisted で分ける** | ✅ 採用。K1 一本 → **K1a(auto) / K1b(assisted) に分割**、assisted で先に高精度（§0.4.2） |
| 3 | **golden JSON 注釈ルールを先に固定** | ✅ **最優先**。別ファイル `..-annotation-rules.md` で確定してから dataset 構築（§0.4.3） |

### §0.4.1 Golden dataset 再々編（同一テンプレ別月を core に）

v1.1 の real 18 件を、**再現性評価を core にする**構成へ:

| 層 | 内容 | 件数 | 狙い |
|---|---|---|---|
| **Core: 同一テンプレ別月** | CEO の航空運航シフト表 — **複数月（3-5 ヶ月分）** | **3-5** | 同形式での**再現性**（K9）— 最重要 |
| Near: 同系統別月 | 同業/近縁テンプレの別月 | 3-4 | テンプレ汎化 |
| Diverse: 他業種勤務表 | 看護/コンビニ/物流 等 | 5-6 | 形式多様性 |
| Dirty: 現実の汚さ | スキャン/撮影（faded/skewed） | 4 | ロバスト性（real 限定） |
| Synthetic edge | 月またぎ/連勤/略号衝突 | 7 | edge 補完（30%-、汚さ合成しない） |

→ **「広く少しずつ」ではなく「同じ型を確実に」**（GPT・⑤目標駆動）。CEO に**航空シフト表の複数月**提供を依頼するのが鍵。

### §0.4.2 KPI を auto / assisted で分割（v1.1 §0.5.3 K1 を上書き）

```
K1a row_selection_auto      : モデルが本人行を自動特定した正解率   目標 ≥ 85%（難）
K1b row_selection_assisted  : ユーザー1クリック指定時の正解率       目標 ≥ 99.5%（易・実用本命）
```

その上で event 精度も 2 系統:
- **K3b-auto**: auto row 正解 file 条件付き event F1
- **K3b-assisted**: assisted row（≈100% 正解前提）での event F1 ← **Phase 1 リリースの実用 KPI はこちら**

→ **私の独自補強**: assisted では K1b≈100% に押し上げられるので、**真の難所は K2(凡例)・K7(略号展開)・K6(日跨ぎ) に移る**。P0 はそこを厚く測る。

### §0.4.3 golden JSON 注釈ルール（別ファイルで先に確定）

GPT 補正 3「評価前にルール固定」を最優先。**dataset 構築の前に**注釈ルール定義書を起草:
- ファイル: `docs/alter-plan-pdf-image-import-annotation-rules.md`
- 確定すべき曖昧項目: `AL`/`HREQ`/休み系の event 化 / セル内複数名 / 注記欄(「MTG 14-15」) / 空セル / 色のみセル / 派生略号
- **私の独自補強（両面 golden）**: 「予定 or 候補」が割れる項目は golden を 1 つに固定せず `policy` で切替（`off_as_event: true/false`）→ ユーザー設定（休みもカレンダー化するか）に対応 + policy 別評価

### §0.4.4 私の深掘り 3 点（GPT 超え）

1. **再現性 KPI（K9）新設**: 同一テンプレ別月 N 件で **F1 の分散/std** を測る。平均が高くても月でブレるなら実用不可（Step 2 LLM の variance 分析の応用）。目標: **std ≤ 3%**
2. **両面 golden（policy 切替）**: §0.4.3 のとおり、休み系/注記の扱いを policy で吸収
3. **assisted の誤クリック頑健性**: ユーザーが間違った行をクリックした時、fingerprint mismatch 検出が効くか（K1b の頑健性サブ指標）

### §0.4.5 P0-2 着手前の追加ゲート（GPT 補正 3 起因）

**注釈ルール定義書（`-annotation-rules.md`）を先に commit・CEO 確認 → その後 golden 構築**。順序を固定:
```
P0-1 設計 (本書) ──→ 注釈ルール定義 ──→ ★CEO 確認 ──→ P0-2 golden 構築 ──→ P0-3 harness ──→ ...
```

---

## §0.5. P0-1 v1.1 補正（GPT 4 点補正の精査 + 私の深掘り 3 点）

> 本セクションは初版 P0-1（§1 以降）に対する **上書き補正**。§0.4（v1.2）が更に優先。初版本文は履歴として残置。
