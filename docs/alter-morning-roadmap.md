...
## 4.5 Roadmap gap — 追加候補（PR-14 以降の検討、PR-9 着手条件ではない）

北極星の slot テーブル全体性から逆算すると、現行 PR-7〜PR-14 では以下が未カバーである可能性がある。いずれも **PR-9 着手の前提ではない**。PR-9 の reducer / ConversationStatus / action 命名を future-proof 化する理由にもしない（CEO 確定 2026-04-23）。PR-14 merge 後に独立検討する候補として明記する。

### 4.5.1 PR-15 相当 — What Staircase（活動の具体化）

- **gap 内容**: 北極星の `what` slot は「具体活動（vague 非許容）」だが、現行 PR-9〜PR-14 に what の確定 staircase が明示的に存在しない。vague な what（「ランチ」「作業」「休憩」等）は PR-7 clarify loop で自然に問い返されるのみで、vocabulary-based な絞り込み経路はない。
- **必要性**: 北極星の slot 完全性という意味では必要。`activityVocabulary.ts` に素材あり。
- **PR-9 への影響**: なし。what は where と直交するため、PR-9 の reducer / ConversationStatus / action 命名は **where-first を維持** する。PR-15 相当の設計は PR-14 merge 後に独立検討する。

### 4.5.2 PR-16 相当 — 日次 plan persistence / 振り返り

- **gap 内容**: 北極星は「1 日の動きを可視化する」と定義。前日以前の plan 振り返りは北極星の外周。
- **判定**: 北極星外。PR-14 完成後、CEO 判断で独立検討。

### 4.5.3 PR-17 相当 — 実行後観測 / Stargazer reality anchoring 接続

- **gap 内容**: 実際に予定通り動けたかの観測。HDM Phase 5 の既存枠と接続しうる。
- **判定**: 北極星外。別 module 接続の議論として保留。

### 4.5.4 PR-18 相当 — 時刻到達時の呼びかけ / リマインダー

- **gap 内容**: 予定時刻になったら Alter が現実世界に介入する通知。
- **判定**: 北極星外 + CEO 方針（マネタイズ・大規模施策は後回し）と別軸。保留。

### 4.5.5 Slot ごとに解決方式が違う（CEO 確定原則 2026-04-23）

| slot | 解決方式 | PR |
|------|---------|-----|
| where | 外部 search（cache + places_api）から候補取得 → user 選択 | PR-9 |
| transport | 確定場所間の推論（mode + 経路） | PR-10 |
| who | 人物同定（正規化 + fuzzy match） | PR-11 |
| time | 時間範囲の確定（startTime + endTime or durationMin） | PR-12 |
| what | 活動内容の具体化（vocabulary 突合） | PR-15 相当（gap） |

PR-9 は where-first の最小実装。PR-10〜PR-15 は各 slot の解決方式に合わせて**独立設計**する（共通 framework 化しない）。
