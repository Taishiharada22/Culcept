...
### D-2. 24 Invariant Coverage（24/24 純粋カバレッジ・Slice 2F で完了）
| 検証場所 | Invariant |
|---|---|
| **invariant-check.ts（fail 可能・決定時）** | INV-1, **2**, 4, 5, 7, 15, **16, 19, 22, 23, 24**（11） |
| **module unit test 担保** | INV-3/8/11/21（lsat）, 10/14（receptivity）, 12（prm-event）, 18（authority）（8） |
| **residual pure harness（Slice 2F）** | INV-6（hysteresis 状態機械）, INV-9（monitoring cadence）, INV-13（authority-escalation policy）, INV-17（gap-meaning 分類器）, INV-20（post-event-recompute）（5） |

→ **24/24 を純関数・型・fixture で自動検証カバレッジ達成**（テストは強い根拠だが*形式的完全証明ではない*）。完全な *runtime* 検証（live 監視ループが実データで正しく fire/監視/昇格するか）は本番接続後。残り 6 を「本番接続と不可分」とした旧主張を撤回（純粋層で前倒し可能だった）。
