...
| **protectionReasons[]** | hard_external / user_declared / recovery_core / cascade_guard / tentative | **守る理由（複合可・配列）**。優先度 hard_external>user_declared>recovery_core>cascade_guard>tentative。例：承認された AI 散歩＝recovery_core∧cascade_guard。AI 生成でも承認・recovery_core 化で昇格 |

- **4 軸を分離**：origin=由来 / authority=所有・確定度 / flexibility=可動性 / **protectionReasons=守る理由（複合可）**。
...
## Part D — カバレッジマップ（自動検証の網羅性・GPT 監査）

### D-1. 35 Scenario Fixture Coverage（35/35）
| 区分 | シナリオ | 検証方法 |
|---|---|---|
| **決定 fixture**（golden-scenario harness 実走） | S1–S15, S19–S30, S32–S35（**30**） | `runScenario` で best / rejected / gate / deliveryMode / invariant を実走照合 |
| **学習 fixture**（PRM event レベル実走） | S16, S17, S18, S22, S31（**5**） | `effectivePolarity` / `validatePrmEvent` / `computeDedupeKey` で実走 |

→ **35/35 fixture 化。out-of-scope なし。** producing engine（DayGraph 変更・PRM 永続化）未実装の項は `expectedDayGraphChange` / `expectedPrmEvents` を documentary に保持。

### D-2. 24 Invariant Coverage（18/24 自動検証）
| 検証場所 | Invariant |
|---|---|
| **invariant-check.ts（fail 可能・決定時）** | INV-1, 4, 5, 7, 15, **16, 19, 22, 23, 24**（10） |
| **module unit test 担保** | INV-3/8/11/21（lsat）, 10/14（receptivity）, 12（prm-event）, 18（authority）（8） |
| scenario fixture でも担保 | INV-16/19/22/24（invariant-check 経由）/ INV-14（S28/29/30） |
| **design / 将来 runtime（未自動検証・6）** | INV-2（DECIDE/DELIVER 分離＝構造）, INV-6（ヒステリシス＝live 監視）, INV-9（監視経済＝geofence 予算）, INV-13（権限獲得＝PRM 的中昇格）, INV-17（空白意味づけ＝gap 分類）, INV-20（post-event 再計算＝Live Controller） |

→ **18/24 自動検証。残り 6 は producing engine（live 監視ループ・Best Action metrics 算出・権限昇格）未実装ゆえ design-level。本番接続フェーズで auto 化する。**
