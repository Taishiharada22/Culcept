...
→ **24/24 を純関数・型・fixture で自動検証。** ただし完全な *runtime* 検証（live 監視ループが実データで正しく fire/監視/昇格するか）は本番接続後。**pure policy（契約）は全て proven**。残り 6 を「本番接続と不可分」とした旧主張を撤回（純粋層で前倒し可能だった）。
