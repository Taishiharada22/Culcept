...
## B-1. `travelAdapterExternalLinksAttach.test.ts` が base で failed → ✅ 解消（2026-06-24）

- **状態**: 🟢 **fixed（stale test 期待値の最小更新・production code 不触）**
- **原因判定（実コード精査）**: 契約破壊ではなく **stale test**。`proposalsDisplay` は adapter（`travel-plan-display-adapter.ts:103/106`）が**常時 display に載せる正規 field**（候補3案表示用・engine.ts:79 で常時計算・追加 commit `f9ddec48c`「CoAlter S1 合意形成知性 live 配線」）。test の `Object.keys(r.display).toEqual(["cues","packet","projection"])` という**厳密3キーアサーションが proposalsDisplay 追加前のまま**だった（真の不変条件＝externalLinks 不在は PASS していた）。
- **修正**: `tests/unit/travelAdapterExternalLinksAttach.test.ts` の期待 key 集合に `proposalsDisplay` を追加（4キー）+ 許容理由をコメント/test 名に明記。**production code・型・adapter は不変**。
- **検証**: 該当 test 12/12 PASS・tsc 55 維持・**full suite 21980 passed / 0 failed**（B-1 が唯一の失敗だったため全 suite green 化）。
- 旧記録（経緯）: ↓
...
## B-7. production migration history drift + entangled pending（🔴 production apply 最大 blocker・INT-10 発見）

- **状態**: 🔴 **production apply GO 不可**（freeze-roundup の「8本 push」では成立しない）
- **INT-10 read-only 確認（2026-06-24・production `aljavfujeqcwnqryjmhl` に link→read-only→即 unlink）**:
  - production の `schema_migrations` 記録上の最終適用 = `20260502100000`。**記録上の未適用 = 32本**。
  - **History drift（記録ズレ）確認**: `external_anchors`/`profiles`/`stargazer_profiles`/`user_style_summary`/`external_anchor_sources` 等は **production に実在するのに該当 migration が記録上「未適用」**（過去に別経路 apply・schema_migrations 未記録）。`migration list` は記録比較なので drift 分が「未適用」に膨らむ。
  - **真に未適用（object 不在）**: freeze-roundup 15 table 全て（travel_* / location_note_* / plan_coalter_* / duration_confirmations）+ 他機能の `plan_seeds`/`prm_learning_events`/`prm_review_decisions`/`prm_model_entries`/`lifeops_structured_sources`/`plan_drift_events` 等。
  - production 総 table = 397。
- **未適用 32本の内訳**: freeze-roundup 8本 + **他機能 24本**（layer1 基盤[drift]・user_style/stargazer prereq[drift]・external_anchors variants[drift]・calendar OAuth(ics/google/microsoft)・shift import・plan_seeds・prm 3本・lifeops_structured 等）。
- **危険**: `supabase db push` は **pending 全 32本を順次適用**しようとする。①drift 分の古い migration が既存 object に当たり非冪等なら **失敗** ②freeze-roundup 以外の genuinely-pending（他機能）も巻き込んで適用される。→ **単純 push 不可**。
- **production apply に必要な前処理（別 CEO GO・DB owner 同席）**:
  1. **history drift の reconcile**: 既に適用済み（object 実在）の migration を `supabase migration repair --status applied <version>` で schema_migrations に記録（drift 解消）。※repair は本 preflight では禁止・別 GO。
  2. **genuinely-pending の scope 確定**: freeze-roundup 8本 + 他機能 16本程度（plan_seeds/prm/lifeops_structured/calendar/shift 等）のうち、**どれを production に出すか** を CEO 判断（他機能は freeze-roundup スコープ外・各機能の production 可否は別判断）。
  3. reconcile 後に backup → 対象のみ apply → 構造検証 → unlink。
- **分類**: 🔴 **production 前必須**（freeze-roundup の production deploy は、この production migration 整理が先決）。
...
|---|---|---|---|
| B-1 | travelAdapter test base failure | 🟢 | **解消済**（stale test 期待値更新・full suite 0 failed） |
| B-2 | Travel migration 8本 未 apply | 🟡→**B-7 に統合** | production apply は B-7 の整理が先決 |
| B-7 | production migration 記録ズレ + 他機能 pending 巻き込み | 🔴 | **必須**（repair reconcile + scope 確定・別 GO・DB owner 同席） |
