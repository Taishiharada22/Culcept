...
> **→ U1/U2 は `docs/reality-leaveby-upstream-provenance-u1-u2-0.md`（U1/U2-0）で詳細設計済**。adversarial workflow（`wf_9bf7f319`）が、U1 の exact 判定は read-path では不可能（all-day 00:00 / tzid 破棄 / confirmedAt の非時刻性）で **creation 時 persist が必須**、U2 の `originFreshness` は **orphan（実 asOf 追加が前提）**と確定。実装は pure-core を超える cross-cutting。
