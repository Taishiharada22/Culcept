...
| **次候補 = CoAlterPlanSession binding 設計（推奨・CEO GO 待ち）** | 「/plan CoAlter chat は何に bind されるか」を docs で確定（plan session 器 + culcept_relation 参加者出自）。thread は backing store 化。これが send/realtime/picker の前提 | ゼロ（docs-only） | — |
