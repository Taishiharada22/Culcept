...
| **TalkBridge-A thread context section（済 2026-06-12・実装）** | 「これまでの会話」**文脈セクション**（read-only・本文と分離）。`CoAlterThreadContextSection.tsx`（独立 card・読み取り専用バッジ・匿名話者 compact 行・入力欄なし・「thread」語彙なし）+ `useCoAlterThreadContext.ts`（既存 T1b `readTalkThreadDeduped` 再利用＝GET-only・dedupe・新 fetch ロジックなし・empty/失敗 fail-closed）。`attachedThreadRef` は **genome-connections.threadId** から`resolveAttachedThreadRef`（relation→thread・単一 counterpart・accepted+threadId ちょうど 1 件・C-1 の同一 fetch 再利用）。**本文不変**（session bubble list に混ぜない）・話者は匿名/表示専用（**SessionParticipant 非昇格・talk_pair_member 不生成・self 推論なし**・session 契約を import しない＝構造的に不可）・thread→session message 変換/複製なし・extraction 入力なし・raw userId 非表示。flag `NEXT_PUBLIC_PLAN_COALTER_THREAD_CONTEXT`（default OFF）。test 9・preview 3 状態実証（bound:文脈 card 表示+本文 fixture+GET-only+thread LIST 0 / threadId null:非表示 fetch0 / flag OFF:非表示 fetch0） | ゼロ | 同 flag 群 |
| **B session message body wiring（済 2026-06-12・fixture→契約）** | 本文を **session message 契約**から描画。CoAlterTab: 本文=`buildSessionMessagesFromFixture(session)`+ローカル送信分（`toSessionMessageFromFixture`）・本文 author 解決は `buildSessionParticipantsFromFixture`（resolved SessionParticipant・匿名なし）。CoAlterChatPanel: prop を `CoAlterSessionMessage[]`+`SessionParticipant[]` に変更（author kind で group・body/createdAt/reactions 描画・人間は userId で resolve・CoAlter は system author）。**thread messages は本文に入れない**（TalkBridge-A の文脈セクションのまま）・raw userId 非表示・draft は依然 author なし・message⊥projection 維持。chat-live(T1b) thread-as-body は本文から退避（fixture session body へ・badge/sendMode は adapter 維持）。**fetch/persistence/send なし**。renderToStaticMarkup test 5・plan 319 files 5706 PASS・preview 2 状態実証（flag OFF:本文 fixture session 同一・新 fetch0 / 文脈 ON:文脈 card+本文 coexist・本文 anon なし・GET-only・/api/coalter 0） | ゼロ | 既存 flag |
| **B closeout + T1b thread-as-body retire/freeze 設計（済 2026-06-12・docs-only）** | [coalter-session-body-closeout-t1b-retire-design.md](coalter-session-body-closeout-t1b-retire-design.md)。★grounded: B 後 **thread messages は本文に再侵入できない**（本文 prop=`CoAlterSessionMessage[]`・thread は別型 `CoAlterChatMessage[]`・CoAlterTab は getInitialMessages を本文に渡さない・render test 固定）＝thread は context section のみで描画（型+source 限定+test の三重担保）。発見: chat-live(T1b) は本文役を失い **wasted fetch + 誤認 readState バッジ**（「ライブ閲覧中」が本文 live を示唆）を残す（dev 経路のみ・既定無影響）。readState は legacy adapter 状態（session body でも context section でもない）。safety invariant 6 件固定。**推奨次=legacy chat-live retire/freeze**（readState badge 撤去・wasted fetch 停止・`createTalkThreadReadonlyAdapter` legacy freeze・chat-live flag deprecate・context 資産は不変）＝session body contract freeze 兼。CEO 判断 2 点 | ゼロ（docs-only） | — |
| **legacy T1b chat-live retire/freeze（済 2026-06-12・実装）** | small cleanup。CoAlterTab が `useCoAlterChatAdapter`（thread-as-body live-read）を**本文から撤去**（import/呼出なし）＝**wasted thread fetch 停止**。CoAlterChatPanel から **readState prop + 本文 live バッジ撤去**（本文は live でない・誤認解消）。本文は fixture session ＝local echo（canLocalEcho=true・getViewer→session.participants[0]）。`coalterChatLive`/`coalterChatDevThreadId` を @deprecated freeze（残置・新依存禁止）。`useCoAlterChatAdapter` hook + `readTalkThreadMetadataDeduped` を legacy freeze。★`readTalkThreadDeduped` は文脈セクション現役のため **ACTIVE 維持**。test 5（badge なし/CoAlterTab が hook 非使用/readTalkThreadDeduped 使用可/flag default OFF）。plan 320 files 5711 PASS。preview: 旧 chat-live ON でも legacy 本文 thread 非 fetch・誤認バッジなし・本文 fixture session・入力可・文脈は relation thread から表示（GET-only・/api/coalter 0） | ゼロ | — |
| **UI-track closeout + persistence/send preflight（済 2026-06-12・docs-only）** | [coalter-ui-track-closeout-persistence-preflight.md](coalter-ui-track-closeout-persistence-preflight.md)。UI/contract トラック締め。final layer map（header=relation/session participant・body=session message・previous conversation=thread context・Plan Intelligence=fixture/local）+ structurally 保証 9 件。★grounded persistence: 既存 `coalter_sessions`/`talk_messages` は **thread-rooted**（thread_id NOT NULL）＝backing にすると CoAlterPlanSession が /talk thread に再 root（legacy takeover）⇒ **新 store は participant/session-rooted の別物**・/talk は optional bridge のみ。high-level table 案（plan_coalter_sessions/participants/messages・RLS=session 参加者・thread/pair_states 非依存・**列/型/制約は migration preflight で確定**）。send authority=server stamp（client から authority 取らない）・M5=message⊥projection 別 table・read receipt 最後尾/opt-in・realtime は persistence/send 後。**推奨次=pure repository interface+in-memory harness（型/pure/test のみ・実 DB/migration/send なし）**。CEO 判断 2 点 | ゼロ（docs-only） | — |
| **session message repository 契約 + in-memory harness（済 2026-06-13・型/pure/test）** | `coalterSessionMessageRepository.ts`（pure 契約・additive・未配線）+ test 13。`CoAlterSessionMessageRepository`=list/appendParticipant/appendSystem。send authority=draft に author なし・branded `ServerStampedAuthorContext`（server boundary mint・client 構築不可）から stamp・membership は repo 境界検査・CoAlter/system 別経路で human 詐称不可。membership は `SessionMembershipResolver` 注入＝thread/pair 非依存。in-memory harness は tests only（now/nextId 注入・Date.now/random/env/fetch/DB なし） | plan 5724 PASS・tsc 55 不変 | `c58e4d75` |
| **session message schema / RLS design（済 2026-06-13・docs-only）** | [coalter-plan-session-message-schema-rls-design.md](coalter-plan-session-message-schema-rls-design.md)。新 4(+optional1) テーブル: plan_coalter_sessions / _participants / _messages / (read_cursors) / (projections)。★participant/session-rooted（legacy thread/pair-rooted `coalter_*` を backing にしない・additive・legacy 不変更）。RLS 核=`plan_coalter_session_participants` membership EXISTS（/talk thread・coalter_pair_states 非依存）。messages: author_kind∈{participant,coalter}+author_user_id(coalter=NULL CHECK)・body 共有のみ（legacy metadata JSONB 非継承）・insert WITH CHECK で author_user_id=auth.uid()（client 詐称 DB 層でも不可）・system は server-only 経路（特権=CEO 判断）・idempotency=UNIQUE(session_id,author_user_id,client_message_id)。threadId は attached_thread_id nullable optional のみ・pairStateId は列に持たない。read cursor は /talk read_at 非 mutate・realtime は session-scoped。migration safety=additive/non-destructive/flag OFF/production なし。**推奨次=migration draft（apply しない）or adapter skeleton**。CEO 判断4点 | ゼロ（docs-only） | — |
| **session message persistence migration DRAFT（済 2026-06-13・SQL draft・未 apply）** | `docs/sql-drafts/20260613120000_plan_coalter_session_messages_DRAFT.sql`（**非実行ディレクトリ・NOT APPLIED**・migrations/ から隔離）。新 4 テーブル CREATE（plan_coalter_sessions/_participants/_messages/_read_cursors）+ RLS。additive のみ・legacy `coalter_*`/`talk_messages` 不 ALTER/DROP・backfill なし・破壊なし。sessions=attached_thread_id nullable optional のみ・pair_state_id/必須 thread_id なし。participants=PK(session_id,user_id)・presentation 列なし（CEO: 参加者層解決）。messages=author_kind{participant,coalter}+author_user_id(CHECK 連動)・body 共有のみ・metadata JSONB なし・idempotency partial UNIQUE(session,author,client_message_id)。RLS 核=participants membership（own-row select で再帰回避・co-participant 可視化 DEFERRED）・message INSERT は author_user_id=auth.uid()+membership（人間 send・service_role 非依存）・**coalter insert policy=HOLD 未作成（既定 deny）**・message UPDATE/DELETE policy なし（MVP immutable）・read_cursors policy なし（read receipt HOLD）・realtime publication 不触。**apply/reset/gen types 未実行**。推奨次=static review→GO 後 local apply | apply 0・SQL のみ | — |
| **session message DB-backed adapter skeleton（済 2026-06-13・injected port）** | `coalterSessionMessageStore.ts`+test 10。同期 pure repo 契約は保持し async DB-facing store を additive。`SessionMessageDbPort`（Supabase 非依存・**system insert メソッドなし=特権 write HOLD を port 形で固定**）。author は server-stamped context のみ（詐称不可）・membership→rejection 写像・system append は HOLD throw・body 共有のみ・threadId/pairStateId 非依存。fake port のみで test | plan 5734 PASS・tsc 55 不変 | `35509852` |
| **send-route preflight design（済 2026-06-13・docs-only・未実装）** | [coalter-send-route-preflight.md](coalter-send-route-preflight.md)。POST route shape・server auth.getUser→stampServerAuthContext・membership 二層（RLS 最終ゲート）・idempotency(clientMessageId+partial UNIQUE)・AppendResult→HTTP(201/200/404/403/422/401)・**route 実装/実 port/`/api/coalter/*`/system write は HARD GATE** | ゼロ（docs-only） | — |
| **local-only persistence + human send bundle（済 2026-06-13・local apply・push なし）** | migration 昇格 `supabase/migrations/20260613120000_plan_coalter_session_messages.sql`（plan_window 版・additive・legacy 不 ALTER/DROP）+ **local apply + RLS smoke 16/16**。real user-RLS port `app/api/coalter/_lib/coalterSessionMessageSupabasePort.ts`（**service_role/admin/createClient なし**・inject のみ・idempotency 23505→既存返し・system insert メソッドなし）+ send route `app/api/coalter/sessions/[sessionId]/messages/route.ts`+`_lib/sendRouteHandler.ts`（flag PLAN_COALTER_SEND_LOCAL default OFF request 時評価→auth.getUser→author server stamp→body は {body,clientMessageId} のみ・author/userId/source 送れば 400→membership+DB RLS→AppendResult→HTTP）。★境界: supabase 依存コードは UI tab folder の supabase-free guard を守るため `app/api/coalter/_lib/`（非ルート）へ配置・pure contract/store は UI folder 据置。gen types は SKIP（partial local DB=broad diff STOP 条件・untyped client）。test port5+handler9+store10 | plan 5748 PASS・tsc 55・RLS 16/16・local only | `dc6a99ce` |
| **local-only UI read/send wiring（済 2026-06-13・default-OFF flag・push なし）** | GET list path（handleCoAlterList + route GET・user-RLS・非 member 空 fail-closed）。UI flags coalterLiveMessages + coalterDevSessionId（NEXT_PUBLIC・default OFF・dev sessionId 注入・thread を root にしない）。runtime `app/(culcept)/plan/coalter-runtime/`（**/api/coalter coupling 隔離**＝UI tab folder guard 維持）: fetch/post pure（**POST body は {body,clientMessageId} のみ**）+ selectCoAlterBody（live のみ live・他 fixture）+ useCoAlterLiveSession（GET dedupe・send→refetch）。CoAlterTab=live 時 実 session message 本文/送信・他 fixture+local echo。raw userId 非表示（未解決 author 中立ラベル）。test live client7+GET4+render4 | plan 5761 PASS・tsc 55・RLS 16/16・local only | `6fa90632` |
