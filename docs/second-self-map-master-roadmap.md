# 第二の自己マップ / Day Rehearsal / Reality Bridge track — マスターロードマップ v2.1（原典起算・track 内 漏れなし）

> 2026-06-08 / Build Unit / CEO 監査反映（v2.1）。CEO 指示「元々の計画から起算して今どの位置にいるか・何が残っているか・最終プロダクトか、track 内 漏れなく提出」
> 上位設計: `second-self-map-master-design.md` / `second-self-map-implementation-plan.md`。運用: フェーズ毎に Build 自律計画 → CEO 精査 → 実装 GO 可否 → commit → 次計画。
>
> ★**スコープ宣言（CEO 監査 v2.1）**: 本書は **Aneurasync Plan 全体の最終完成図ではない**。**Second Self Map / Day Rehearsal / Reality Bridge track**（本人モデル × 1日診断 × 介入橋渡し）の roadmap。
> ★**本 track の対象外（Aneurasync Plan の他 track・別 roadmap が要る）**: 予定作成/編集 UI・calendar CRUD/import・外部カレンダー連携（Google/Apple/shift）・通知/出発リマインド/遅延検知・native permission/location/privacy・onboarding/settings/権限レベル・実行後学習（execution feedback）・production rollout/monitoring。これらは本書では扱わない（薄い領域として明示）。
...
```
**現在地（CEO 監査 v2.1 で正確化）**: **Day Rehearsal を中心とする local diagnostic v1 は一区切り**。ただし **L2 理由観測・S3 あなたのペース・L5 context・cross-day・通知・介入層は未完成**。L5（介入・production）は pure layer 構築済で **HELD（GitHub 不可）**。
...
| **Wave 2** | Day Rehearsal(課金核) / energy curve / counterfactual / S3 ペース | ✅ **Day Rehearsal v1 一区切り**。**残: S3 ペース・energy curve 可視化・counterfactual 本格化・calibration/strain 飽和/magnitude/cross-day** |
...
| — | Day Rehearsal（課金核） | ✅ **v1 一区切り** | calibration / strain 飽和 / magnitude tier / cross-day / 実データ後閾値 / user correction loop 接続 は**未** |
...
| — | counterfactual「もし〜なら」 | ✅ **v0 入口のみ** | leave_earlier 中心の pure simulation + UI 1 行表示まで(`ad0c9ee7`/`e7b45272`)。**本格 What-if / Draft Preview / after timeline / apply は未** |
...
0. ★**A0 S6-0 理由観測 UI（SOUL prereq・最優先）**: 推奨と違う mode を選んだ時に 1-tap で理由（疲れ/景色/安い/急ぎ/気分）を観測。hypothesisFeedbackStore に reason field 追加（local・低侵襲・任意・可逆）。**これが無いと堀②/M5/鏡が永久に起動しない**。
   ★**A0 のスコープ厳守（CEO 監査 v2.1）**: **まず local reason capture に限定**（reason 観測 UI + local 蓄積のみ）。**Alter 接続 / Stargazer 合流 / DB 永続化は A0 では一気にやらない**（それぞれ後続 slice・Stargazer DB 合流は production gated）。A0 は **audit + mini design → CEO GO → 実装**の順。
...
S3 pace は精度向上として A1 に続けて。**次は A0 の audit + mini design**（理由観測のスキーマ・低侵襲 UX・local 蓄積）を提出。★**A0 は local reason capture に限定**（Alter/Stargazer/DB は一気にやらない・後続 slice）。
...
- → 本追記で **68 本 読了・cross-check 済**。漏れは Reality 粒度のみで、保全済。★**再現性のためファイル一覧を Appendix A に添付**（CEO 監査 v2.1 指摘：作業ログだけでは外部検証不能ゆえ）。direct-read = 正本 + 本セッション作成分、残りは read-only Explore agent が全文読了し本 roadmap と cross-check。
...
## Appendix A — 読了 plan .md 一覧（68本・2026-06-08・再現性のため）
`docs/` 配下の本 track 関連 .md（`second-self-map* / day-rehearsal* / mobility* / plan-map* / alter-plan-map*`）。direct-read = 正本(master-design #55 / implementation-plan #44 / calibration-backlog #6) + 本セッション作成分(#7-10,13,26-28,56 等)。残りは read-only Explore agent(5 クラスタ)が全文読了し本 roadmap と cross-check。FH 戦略原典 `plan-map-second-self-strategy.md` は main worktree 在(別 path・direct-read 済)。
1. alter-plan-map-9b-5-string-audit.md / 2. alter-plan-map-redesign-9-closeout-readiness.md / 3. alter-plan-map-redesign-9b-readiness.md / 4. alter-plan-map-redesign-impl-readiness.md / 5. alter-plan-map-redesign-spec-audit.md / 6. second-self-map-calibration-backlog.md / 7. …-day-rehearsal-batch3-f1-closeout.md / 8. …-day-rehearsal-batch4-whatif-audit.md / 9. …-day-rehearsal-diagnostic-batch-closeout.md / 10. …-day-rehearsal-energy-batch2-closeout.md / 11. …-day-rehearsal-evidence-ui-closeout.md / 12. …-day-rehearsal-evidence-ui-mini-design.md / 13. …-day-rehearsal-fullpath-batch1-closeout.md / 14. …-day-rehearsal-mini-design.md / 15. …-day-rehearsal-per-marker-why-closeout.md / 16. …-day-rehearsal-per-marker-why-mini-design.md / 17. …-day-rehearsal-repair-dedup-mini-design.md / 18. …-day-rehearsal-repair-draft-disposition-v0-closeout.md / 19. …-day-rehearsal-repair-draft-v0-mini-design.md / 20. …-day-rehearsal-repair-fullpath-audit.md / 21. …-day-rehearsal-repair-protect-signal-v0-closeout.md / 22. …-day-rehearsal-repair-protect-signal-v1-gap-resolver-closeout.md / 23. …-day-rehearsal-repair-reality-bridge-contract-audit.md / 24. …-day-rehearsal-repair-reality-bridge-mini-design.md / 25. …-day-rehearsal-repair-reality-coordination-checklist.md / 26. …-day-rehearsal-repair-simulation-v0-audit.md / 27. …-day-rehearsal-repair-simulation-v0-closeout.md / 28. …-day-rehearsal-repair-simulation-v0-ui-closeout.md / 29. …-day-rehearsal-repair-v0-closeout.md / 30. …-day-rehearsal-repair-v0-ui-closeout.md / 31. …-day-rehearsal-repair-v0-ui-mini-design.md / 32. …-day-rehearsal-repair-v0-whatif-mini-design.md / 33. …-day-rehearsal-repair-v1-audit-closeout.md / 34. …-day-rehearsal-step4-closeout.md / 35. …-day-rehearsal-timeline-marker-mini-design.md / 36. …-day-rehearsal-whatif-ui-mini-design.md / 37. …-day-rehearsal-whatif-v0-closeout.md / 38. …-day-rehearsal-wire-closeout.md / 39. …-day-rehearsal-wiring-mini-design.md / 40. …-day-rehearsal-wpm1-closeout.md / 41. …-day-rehearsal-wpm2-audit.md / 42. …-day-rehearsal-wpm2-closeout.md / 43. …-gaprecovery-protectedgaps-integration-mini-design.md / 44. second-self-map-implementation-plan.md / 45. …-l3-mini-design.md / 46. …-l3a-closeout.md / 47. …-l3b-mini-design.md / 48. …-l3b1-closeout.md / 49. …-l3b1-wiring-mini-design.md / 50. …-l3b2-closeout.md / 51. …-l4-closeout.md / 52. …-l4-mini-design.md / 53. …-l4b-closeout.md / 54. …-l4b-mini-design.md / 55. second-self-map-master-design.md / 56. second-self-map-master-roadmap.md(本書) / 57. …-reality-capture-surface-canary-readiness-audit.md / 58. …-reality-capture-surface-canary-rollout-plan.md / 59. …-reality-inv17-enforcement-audit.md / 60. …-reality-inv17-enforcement-mini-design.md / 61. …-reality-inv17-enforcement-v0-closeout.md / 62. …-reality-production-wiring-audit.md / 63. …-v0-closeout.md / 64. …-v0-smoke-plan.md / 65. …-v0f-mini-design.md / 66. …-wave1-l1-closeout.md / 67. …-wave1-l1-mini-design.md / 68. …-wave1-l1b-mini-design.md
