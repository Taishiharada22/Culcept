...
- rev 3.1 (2026-04-20 pm): ファイル名 `coalter-movie-two-stage-design.md` → `coalter-movie-three-stage-design.md` に rename（CEO lock B）。types.ts に [CEO lock 2026-04-20 A]（sourcedFrom の生テキスト / quote / summary を diagnostics・ログから除外）を明文化
- rev 3.2 (2026-04-24): **Bug-2 接続の明文化**（§6 Phase M2 内側に「Bug-2 接続」節を追加）。`docs/coalter-handoff-2026-04-19-retrieval-investigation.md` §4.2 / §5.3 / §8 を正式に三段式設計と接続。Bug-2 判定基準を構造 gate (B1-B3) + 品質 gate (H1-H5) + 移行期 maintenance gate の三層で定義。旧 diagnostics (Phase A.5/A.6) の継承/移植先を確定。旧実装 `parseMovieScreenings` / `NEAR_WINDOW` / `missing_where` hard drop の M2 期間中 **温存** 条件を明文化。本 rev は設計書追記のみ、コード変更なし
...
> **参照（統合契約 §3, 2026-04-24 rev 1 FIXED）**: Stage 1 Understand（executor 側、event bus `executor.understanding.*`）と v1.1 §8 Presence S4 理解更新中（UI 側、event bus `presence.state.*`）は **別レイヤー**。S4 は Stage 1 の生進捗バーではなく、Stage 1 を走らせない通常モード S5 橋渡しも許容される（統合契約 §3.2〜§3.4）。
...
※ 本節末の **Phase M2 Bug-2 接続** は rev 3.2 で追加された Bug-2 判定セクション（§6 M2 内側）。H1-H5 の後に読む。
...
#### Phase M2 Bug-2 接続（rev 3.2 追記）

**前提文書**: `docs/coalter-handoff-2026-04-19-retrieval-investigation.md` §4.2 / §5.3 / §8。

##### Bug-2 の正体（handoff 引用）

2026-04-19 preview 4 セッション連続で観測された構造崩壊:

| sessionId(抜粋) | queries | rawResults | candidates | catalog | **ranked** | **missingWhere** | **titleWithoutTheater** |
|---|---|---|---|---|---|---|---|
| a7a9ef93... | 3 | 9 | 8 | 6 | **0** | **5** | **5** |
| 7b925649... | 3 | 9 | 8 | 6 | 0 | 5 | 5 |
| b6ebe1e4... | 3 | 9 | 8 | 6 | 0 | 5 | 5 |
| 7585bed9... | 3 | 9 | 8 | 6 | 0 | 5 | 5 |

- **直接原因**: `lib/coalter/movieRanker.ts:166` の `if (movie.title && !movie.theater) reasons.push("missing_where")` による hard drop
- **構造原因**: EXA 300 字スニペットに劇場名がほぼ含まれない（`titleWithoutTheaterCount === catalogCount` 事象、§0 引用）
- **根本原因**: 「作品」と「劇場」を単一クエリ・単一段で同時解決しようとする設計崩壊（§0）

##### 三段式による構造的解消

Bug-2 は三段式アーキテクチャによって **構造的に** 解消される:

| Bug-2 症状 | 三段式での解消 | 参照 |
|---|---|---|
| `titleWithoutTheaterCount === catalogCount` 事象 | Stage 3 Resolve が劇場公式サイトを直接 fetch。EXA 300 字スニペット依存を廃止 | §2.4.2 |
| `missing_where` hard drop で ranker 0 件 | Stage 2 Curate は WHERE 空欄を許容（Skeleton UI）。hard drop は Stage 2 側では発生しない | §1 原則 3 / §2.1.1 / §2.3.2 |
| 単一層で Tier 0 のみ探索して失敗 | Concentric Area Expansion (Tier 0→1→2) を Stage 3 Resolve 内部ループに組み込む | §2.4.1 |
| Bug-1（感情語で retrieval skip）と Bug-2（catalog drop）が同一層で混線 | Stage 0 Analysis と Stage 3 Resolve に責務分離。Bug-1 は Stage 0、Bug-2 は Stage 3 | §1 原則 1 |

##### Bug-2 判定基準（三層）

Bug-2 が「直った」の判定は **三層** で定義する。handoff §8 北極星 3 指標（`catalogCount ≥ 5 / rankedCount ≥ 3 / missingWhereRejectCount ≤ 30%`）を三段式構造下で **再解釈** する。

**層 1: 構造 gate（M2 実装完了時の内在的性質）**

| 基準 | 充足条件 | 判定方法 |
|---|---|---|
| **B1** Stage 2 Curate が `missing_where` hard drop を発生させない | Stage 2 Curate 実装コード（`lib/coalter/movie/curator.ts`）に `missing_where` reject ロジックが存在しない | コードレビューで構造的に確認（単体テスト不要、存在しないことが gate） |
| **B2** Stage 3 Resolve が theater fact authority を持つ | `lib/coalter/movie/theaterResolver.ts` に（公式サイト → eiga.com → Yahoo映画 → EXA 補助）の 3+1 段 fallback が実装されている | コードレビューで構造確認 + 単体テスト（mock fetch） |
| **B3** Stage 3 Tier fail を UI に誠実に返す | Tier 2 fail 時に `{ state: "tier2_fail", message: "この近辺では上映が弱い", altSignal: true }` を返し、narration + 別作品提案を出す（§4 表） | 単体テスト + 統合テスト |

構造 gate は「Stage 3 Resolve の実装が三段式設計（§2.4）に準拠していること」で自動充足する。**品質閾値ではなく構造検証**。

**層 2: 品質 gate（既存 H1-H5 = 旧北極星の置換系）**

Stage 3 Resolve の品質は **既存 H1-H5** で判定する。旧北極星 3 指標との対応は以下:

| 既存 M2 gate | handoff §8 北極星との対応 |
|---|---|
| **H1** Tier 0 劇場確定率 ≥ 55% | 旧 `catalogCount ≥ 5` / `rankedCount ≥ 3` の置換系。Stage 3 の Tier 0 成功率として再定義 |
| **H2** Tier 0+1 劇場確定率 ≥ 75% | H1 の近隣拡張版。Concentric Area Expansion の効果を測る |
| **H3** Tier 2 fail 再起動率 ≥ 60% | 失敗時の受容率。handoff 北極星には直接対応指標なし、Fail Honestly の測定として新規 |
| **H4** 1 分 budget 超過率 ≤ 10% | 品質と速度の両立 |
| **H5** narration 一貫性 | 人格の一貫性（Bug-2 と直接関係しないが M2 全体 gate） |

旧北極星 `missingWhereRejectCount ≤ catalog の 30%` は **三段式実装後は構造的に 0**（B1 により）。M2 品質 gate の指標から **除外**。代わりに B1 で構造保証される。

**層 3: 移行期 maintenance gate（M2 着手前 = preview 継続期間）**

M0/M1/M2 実装中は **現行 single-stage 実装が稼働** する。この期間の Bug-2 関連 maintenance gate は handoff §8 北極星そのものを使う:

| 指標 | 閾値 | 源 |
|---|---|---|
| `searchCandidatesCount` ≥ 5 | preview 本カウント中央値 | Bug-1 対策の効果確認も兼ねる（`lib/coalter/engine.ts:193-201`） |
| `catalogCount` ≥ 5 | preview 本カウント中央値 | 現行 `parseMovieScreenings` の健全性 |
| `rankedCount` ≥ 3 | preview 本カウント中央値 | 現行 `movieRanker` の健全性 |
| `missingWhereRejectCount` ≤ catalog の 30% | preview 本カウント | 現行 `missing_where` drop が暴走していないか |

移行期 maintenance gate は **preview 30 件 full observation gate**（handoff §7.4）と同じ枠で稼働する。**M2 品質 gate（H1-H5）とは別建て**。

##### 既存 Phase A.5/A.6 diagnostics の継承/移植

`lib/coalter/movieOrchestrator.ts:247-253` の既存 diagnostics は以下のとおり三段式に継承される:

| 旧フィールド | 三段式での位置づけ | 継承の扱い |
|---|---|---|
| `searchCandidatesCount` | Stage 0 Analysis 出力の観測点（Bug-1 対策の検証にも使用） | 維持（意味不変） |
| `catalogCount` | Stage 2 Curate candidate pool サイズ（Soft Availability Filter 通過後） | 意味再定義（値の解釈が変わる） |
| `rankedCount` | Stage 2 LLM ranker top 候補数 | 意味ほぼ不変（ただし Stage 2 層で閉じる） |
| `missingWhereRejectCount` | **Stage 2 側では常に 0**。構造 gate B1 の実行時検証として残す | 意味変質（検証用に温存） |
| `titleWithoutTheaterCount` | Stage 2 pool 通過後に WHERE 未確定の件数（Stage 3 が引き受けるべき件数） | 維持（Stage 3 入力観測） |

**新規 diagnostics フィールド（M2 で追加する観測項目）**:

| フィールド | 意味 | 型 |
|---|---|---|
| `stage3Tier0SuccessCount` | Stage 3 Tier 0 で劇場確定した件数 | number |
| `stage3Tier1SuccessCount` | Stage 3 Tier 1 拡張で劇場確定した件数 | number |
| `stage3Tier2FailCount` | Stage 3 Tier 2 fail となった件数（別作品再起動の発火源） | number |
| `stage3ResolverBudgetMs` | Stage 3 Resolve 全体の実時間 | number |
| `stage3FallbackSourceUsed` | 公式 / eiga.com / Yahoo / EXA のどれが最終 hit したか | "official" \| "eiga" \| "yahoo" \| "exa" \| null |

これらは `[CoAlter] movie.stage3.diagnostics` 名前空間で emit する（既存 `movie.diagnostics` とは分離、既存 KPI SQL を壊さない）。

##### 旧実装の温存条件（M2 期間中の不可触線）

M2 期間中、以下は **触らない**:

| 対象 | 温存理由 |
|---|---|
| `lib/coalter/webConnector.ts` の `parseMovieScreenings` / `NEAR_WINDOW` / theater 抽出正規表現 | Stage 3 Resolve が稼働するまでの fallback として必要 |
| `lib/coalter/movieRanker.ts:166` の `if (movie.title && !movie.theater) reasons.push("missing_where")` | 現行実装の missing_where gate。Stage 3 未稼働時の劣化防止として維持 |
| `lib/coalter/movieOrchestrator.ts` の既存 ranker 呼び出しフロー | 移行期 maintenance gate の観測対象 |

Stage 3 Resolve（`lib/coalter/movie/theaterResolver.ts`）が H1-H3 を満たして正式投入された後、別 rev（rev 3.3 以降 or Phase M3 の kill switch flip 時）で **旧実装削除の CEO 審議** を行う。M2 期間中は kill switch `COALTER_THREE_STAGE=false` で現行単一段に即時 rollback 可（§M3）。

##### Bug-1 / Bug-2 責務分離（まとめ）

| Bug | 層 | 設計書 | 判定指標（主） |
|---|---|---|---|
| **Bug-1** | Stage 0 Analysis（retrieval gate） | `docs/coalter-bug1-emotion-retrieval-design.md` rev v0.2 | `searchCandidatesCount ≥ 5`（handoff §8 / Bug-1 §8.4） |
| **Bug-2** | Stage 3 Resolve（theater fact authority） | 本文書 §6 Phase M2 Bug-2 接続（本節） | 構造 gate B1-B3 + 品質 gate H1-H5 |

**境界線**: `searchCandidatesCount ≥ 5` までが Bug-1 の到達点。そこから先（Stage 2 pool → Stage 3 Tier 成功）は Bug-2 / M1 / M2 の責務。
...
- [x] **F. Bug-2 接続の明文化（rev 3.2, 2026-04-24）** — §6 Phase M2 内側に「Phase M2 Bug-2 接続」節追加。`docs/coalter-handoff-2026-04-19-retrieval-investigation.md` §4.2 / §5.3 / §8 を三段式設計と正式接続。Bug-2 判定を構造 gate (B1-B3) / 品質 gate (H1-H5) / 移行期 maintenance gate の三層で定義。旧 diagnostics 継承/移植先・旧実装温存条件を明記
...
docs/
└── coalter-movie-three-stage-design.md     ← 本ファイル（rev 3 で三段式に昇格、rename 済み）
```
...
**[CEO lock 2026-04-20 A] diagnostics 出力制約**: U2 測定は `source_coverage.{a,b}` の **カウント値** のみで判定し、raw `quote` / `summary` は一切ログに出さない。StargazerSourceRef / AlterSourceRef / BehavioralSourceRef の生テキスト相当フィールドは Stage 2 narration 生成の内部でのみ参照される。diagnostics emitter (`lib/coalter/understanding/diagnostics.ts`) は型レベルで `UnderstandingDiagnostics` のみを受け取り、生テキストへのアクセス経路を持たない。
