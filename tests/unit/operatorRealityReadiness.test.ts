...
describe("RD3b-P1 #8-#14 raw anchor を safe DTO に出さない（leak guard 通過）", () => {
  ...
  it("evidenceRefs / sourceRefs / missingInputRefs / leaveByInstant / arrivalTargetInstant / timeContract が出ない", async () => {
    const p = await buildOperatorDayRealPayload({ operatorUserId: OP, referenceInstantUtc: REF }, depsOf(ANCHORS_TWO_WITH_LOC));
    // RD3b-P1/RD3x-P2: schema-state safe key `leaveByComputedPresentCount`（readiness）/ `leaveByComputedPresent`（RD3x-P2 boolean）
    //   は意図的（substring "leavebycomputed" を含むが exact instant でない）。"leavebycomputedpresent" 除去で count prefix も消える。
    const json = JSON.stringify(p).toLowerCase().split("leavebycomputedpresent").join("");
