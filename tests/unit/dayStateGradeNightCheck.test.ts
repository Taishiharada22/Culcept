...
describe("gradeRecoveryNeed — 同規約（±1 match・両端のみ over/under）", () => {
  it("凍結 high × felt5（actual low）→ over（高く見すぎ）", () => {
    expect(gradeRecoveryNeed("high", 5)).toBe("over");
  });
  it("凍結 low × felt1（actual high）→ under（低く見すぎ）", () => {
    expect(gradeRecoveryNeed("low", 1)).toBe("under");
  });
  it("凍結 medium はどの felt でも match（±1 吸収）", () => {
    ([1, 2, 3, 4, 5] as DayFelt[]).forEach((felt) => expect(gradeRecoveryNeed("medium", felt)).toBe("match"));
  });
