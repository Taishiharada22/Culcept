...
describe("RD2f-wiring-P2 #3/#4 operator output に leaveByComputed / exact instant token が出ない", () => {
  it("ON payload に exact-instant content token（leaveByInstant/arrivalTarget/timeContract/*Ref）不出現", async () => {
    // RD3b-P1: readiness.leaveByComputedPresentCount は意図的 safe schema-state count（substring "leavebycomputed" を含むが
    //   exact instant でない）。よって走査前に carve-out してから残り content token のみ検証する（dogfood と同 contract）。
    const on = await payloadWithFlag(true);
    const json = JSON.stringify(on).toLowerCase().split("leavebycomputedpresentcount").join("");
