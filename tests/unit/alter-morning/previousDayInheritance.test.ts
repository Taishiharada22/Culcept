...
describe("preserveStrongPriorOrigin — STRONG source 守る + samePlanDate 連動", () => {
  ...
  it("[CEO/GPT 規律 PR B-2e] user_override (origin clarify 由来) → 守る", () => {
    // CEO/GPT 2026-05-02 PR B-2e で意味論拡張:
    //   user_override は origin clarify (PR B-2e) の user 回答で確定した anchor。
    //   journeyOrigin に入った場合は origin override として STRONG_PRIOR_ORIGIN_SOURCES
    //   に含む。samePlanDate=true で守られる、samePlanDate=false で守らない。
    const result = preserveStrongPriorOrigin(
  ...
  it("[CEO/GPT 規律 PR B-2e] user_override + samePlanDate=false → 守らない (stale 解除)", () => {
    // 別日 plan に切り替わったら、古い user_override は守られない。
    const result = preserveStrongPriorOrigin(
      exactOrigin("user_override"),
      { samePlanDate: false },
    );
