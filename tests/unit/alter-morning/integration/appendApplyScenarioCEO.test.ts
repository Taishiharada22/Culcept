...
describe("CEO Case 3: 予定追加 (このあと新宿で高橋とミーティング)", () => {
  it("[正式成功条件] event_1 維持 + event_2 追加 (上書きされない)", () => {
    ...
    // PR-50 Commit 12: canonical identity 必須 (when + activity + place_ref)。
    // LLM が context bind で event_1 を再構築 (= where/what も完備で出力) する想定。
    const cur1 = mkEvent({
      event_id: "event_1",
      turn_mode: "create",
      when: {
  ...
  it("[create + 同一性なし] cur が新規 turn_mode='create' でも append として処理される", () => {
    ...
    //   旧: prior 上書き risk → 新: prior 不変 + 新規追加 (CEO Case 3 真因 fix)
    expect(session.persistedEvents).toHaveLength(2);
    const event1 = session.persistedEvents!.find(
      (e) => e.event_id === "event_1",
    );
    expect(event1).toBeDefined();
    expect(event1!.where.place_ref).toBe("スターバックス TSUTAYA"); // 不変
    expect(event1!.when.startTime).toBe("09:00");
    const newEvent = session.persistedEvents!.find(
      (e) => e.event_id !== "event_1",
    );
    expect(newEvent).toBeDefined();
    expect(newEvent!.where.place_ref).toBe("新宿");
    expect(newEvent!.what.activity).toBe("ランチ");
  });
});
