...
describe("c26 — 配線整合・gate（⑯⑰）", () => {
  ...
  it("⑯本線 wiring は builder(mode)・action は同 selector を使用（表示と照合の代表集合が同一）", () => {
    // P16 test-drift fix: HOME-SWIPE-PLAN-PARITY FIX(2026-06-25)で buildLifeOpsMainlineCardDto(model, sourceMode)
    //   呼び出しは page.tsx → planClientFeatureProps.ts に移動。挙動は不変・file 位置のみ移動。
    const featureProps = read("app/(culcept)/plan/planClientFeatureProps.ts");
    expect(featureProps).toContain("buildLifeOpsMainlineCardDto(model, sourceMode)");
