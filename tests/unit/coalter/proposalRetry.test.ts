...
describe("proposalGenerator retry — 1回目で accepted が 1つ以上あれば retry しない", () => {
  it("全候補 accepted → runAI 1回だけ呼ばれる", async () => {
  ...
  it("一部 reject、一部 accepted → retry しない、accepted のみ残す", async () => {
    runAIMock.mockResolvedValueOnce(
      await mockAIResponse({
        summary: "OK",
        priorities: { userA: "A", userB: "B", common: null },
        candidates: [
          ...
          },
          {
            ...
            oneLiner: "抽象",
            slots: {
              what: { label: "恋愛映画", status: "proposed" }, // ジャンルのみ
              where: { label: "駅周辺", status: "proposed" }, // 抽象
            },
          },
        ],
        reasoning: "r",
    ...
    expect(card.validation?.rejectReasons.length).toBeGreaterThan(0);
  });
});
