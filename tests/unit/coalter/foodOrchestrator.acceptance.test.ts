 ...
 *      - pageTypeDistribution 6 key ゼロ埋め
 *      - missingWhereRateBySourceKind / insufficientInfoRateBySourceKind 6 key ∈ [0,1]
...
describe("§6.4 (6)-5 acceptance — D. observability 指標が期待形", () => {
  ...
  it("pageTypeDistribution / blockedBy / rate は固定 6-key shape", async () => {
    runAIMock.mockImplementation(() => mockBrief());
    const result = await generateFoodProposalV2({
      turns,
      analysis: makeAnalysis(),
