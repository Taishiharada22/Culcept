...
describe("ShiftReviewGrid risk 表示 — confusable_code（A1B・似た形で紛らわしい）", () => {
  ...
  it("A1-tune-1: medium（H）→ panel に「休み種別が紛らわしい」件数 summary（cell amber は出さない）", () => {
    const cells = cleanMonth().map((c) => (c.day === 4 ? { ...c, rawCode: "H" } : c));
    const html = render({ cells, riskReviewEnabled: true });
    expect(html).toContain('data-testid="shift-review-risk-confusable_code"');
    expect(html).toContain("休み種別が紛らわしい日が1日");
    expect(html).toMatch(/shift-review-cell-4"[^>]*data-confusable="false"/); // H は cell amber 外
  });

  it("A1-tune-1: weak（N）のみ → confusable_code panel にも cell amber にも出さない（observation only）", () => {
    const cells = cleanMonth().map((c) => (c.day === 4 ? { ...c, rawCode: "N" } : c));
    const html = render({ cells, riskReviewEnabled: true });
    expect(html).not.toContain('data-testid="shift-review-risk-confusable_code"');
    expect(html).toMatch(/shift-review-cell-4"[^>]*data-confusable="false"/); // N は UI 非表示
  });
});
