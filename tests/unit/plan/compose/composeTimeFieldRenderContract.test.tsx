...
describe("空白(—)は間隔のみ（CEO）", () => {
  ...
  it("開始/終了ホイールには空白(—)が無い", () => {
    const html = render({ mode: "both", startMin: 900, endMin: 1020 });
    expect(html).not.toContain('data-testid="compose-time-start-hour-opt-null"');
    expect(html).not.toContain('data-testid="compose-time-end-min-opt-null"');
  });
});
