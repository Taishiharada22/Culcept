...
// ───────────────────────── flags（2026-06-19 canary: const 両 true・dev ON / production は env gate） ─────────────────────────
describe("P4 flags（UI/fetch 分離・canary ON・production は env gate）", () => {
  it("★fetch flag canary ON（const true・dev で有効）", () => {
    expect(PLACE_DETAILS_ENRICH_FETCH_ENABLED).toBe(true);
    vi.stubEnv("NODE_ENV", "development");
    expect(isPlaceDetailsFetchEnabled()).toBe(true); // dev は envGateOpen=true ゆえ有効
  });

  it("★UI flag canary ON（const true・dev で有効・fetch と独立）", () => {
    expect(PLACE_DETAILS_ENRICH_UI_ENABLED).toBe(true);
    vi.stubEnv("NODE_ENV", "development");
    expect(isPlaceDetailsUiEnabled()).toBe(true);
  });

  it("★production hard block（const true でも env 未設定なら両 flag false）", () => {
...
describe("P4-e production env gate（isProdEnrichAllowed / envGateOpen / 二重スイッチ）", () => {
  // 公開関数は const && envGateOpen() の AND。canary で const は両方 true（dev ON / production は env で gate）。
  it("★(7) canary 状態（const 両 true・env 未設定）→ dev ON / production は env gate", () => {
    expect(PLACE_DETAILS_ENRICH_FETCH_ENABLED).toBe(true);
    expect(PLACE_DETAILS_ENRICH_UI_ENABLED).toBe(true);
    expect(isProdEnrichAllowed()).toBe(false); // env 未設定
  });

  it("★(1) dev では canary 有効（envGateOpen=true・const true → 公開関数 true）", () => {
    vi.stubEnv("NODE_ENV", "development");
    ...
    expect(isPlaceDetailsFetchEnabled()).toBe(true); // canary ON
  });

  it("★(2) production + env 未設定 → gate closed → fetch/ui false（const true でも本番排他維持）", () => {
  ...
  it("★(5) production + env=1（実 const=true）→ 本番有効化される（canary 本番 ON 条件＝二重スイッチ充足）", () => {
    vi.stubEnv("NODE_ENV", "production");
    vi.stubEnv("PLACE_DETAILS_ENRICH_PROD_ALLOWED", "1");
    expect(isProdEnrichAllowed()).toBe(true);
    expect(envGateOpen()).toBe(true);
    expect(isPlaceDetailsFetchEnabled()).toBe(true); // const true && env=1 → 本番 ON
    expect(isPlaceDetailsUiEnabled()).toBe(true);
  });

  it("★(3) production + env=1 + const true → true（gate open かつ const true）", () => {
    vi.stubEnv("NODE_ENV", "production");
    vi.stubEnv("PLACE_DETAILS_ENRICH_PROD_ALLOWED", "1");
    expect(envGateOpen()).toBe(true);
    expect(true && envGateOpen()).toBe(true);
  });

  it("★(4) const flag false 相当（env=1・production）→ false（const が必要・二重スイッチ）", () => {
    vi.stubEnv("NODE_ENV", "production");
    vi.stubEnv("PLACE_DETAILS_ENRICH_PROD_ALLOWED", "1");
    expect(false && envGateOpen()).toBe(false); // const false なら env=1 でも無効
  });

  it("★(6) env 無（production）→ false（const true でも env が必要・二重スイッチ）", () => {
    vi.stubEnv("NODE_ENV", "production"); // env 未設定
    expect(envGateOpen()).toBe(false); // gate closed
    expect(isPlaceDetailsFetchEnabled()).toBe(false); // ★実 const true でも本番は env 無で OFF
    expect(isPlaceDetailsUiEnabled()).toBe(false);
  });
