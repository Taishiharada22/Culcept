...
describe("S2 CoAlter personalization live（demo 軸→derive→soft preference→engine + 2 人 readout）", () => {
  ...
  it("TRAVEL: pair readout は一致点のみ（pace 一致）・相違点（novelty 差）は出さない＝forecast の領域", () => {
    // S3-1 外科縮小: readout = 一致点のみ。差分（新しさ・段取り）は coalterConflictForecast へ移管。
    ...
    expect(r.pairReadout.some((l) => l.includes("ゆっくり"))).toBe(true); // pace 一致 → readout に残る
    expect(r.pairReadout.some((l) => l.includes("新しさ"))).toBe(false); // novelty 差 → readout には出さない
    expect(r.pairReadout.every((l) => l.includes("お二人とも"))).toBe(true); // 残る pair 行は一致点表現のみ
  });

  it("DAILY: 一致軸なし → pair readout は空（対人の差は forecast 側で出す）", () => {
    // daily は self/partner で語れる一致軸がない → pairReadout 空。対人の差は readout に出さない。
    const { self, partner } = COALTER_DEMO_PERSONALIZATION.daily;
    const r = buildCoAlterPairTraitReadout(self, partner, "Mio");
    expect(r.pairReadout.some((l) => l.includes("対人") || l.includes("人混み"))).toBe(false);
    expect(r.pairReadout.every((l) => l.includes("お二人とも"))).toBe(true); // 一致点表現のみ（空でも真）
  });
