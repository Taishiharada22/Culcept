---
name: flow-audit
description: ユーザージャーニー検証。指定した画面遷移に沿って全データ依存・API呼び出し・状態管理が正しく接続されているか検証する。
user_invocable: true
---
...
- フロー名（例: `stargazer-observation`, `rendezvous-matching`, `onboarding`, `genome-card-exchange`）
