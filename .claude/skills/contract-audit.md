---
name: contract-audit
description: API契約検証。APIレスポンスの実際の形状とUI側の型/参照の一致性を検証し、型不一致・欠落フィールド・余剰フィールドを検出する。
user_invocable: true
---
...
- 機能領域名（例: `stargazer`, `rendezvous`）
- 引数なしの場合は全 API エンドポイントを対象に高速スキャン
...
**UI が期待するもの:**
```json
{ "archetype": "...", "scores": [...], "updatedAt": "..." }  // ← camelCase
```
