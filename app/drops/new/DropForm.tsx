...
function TagEditor({ name }: { name: string }) {
    ...
    return (
        <div className="grid gap-2">
            ...
            <div className="text-xs font-semibold text-zinc-500">最大20個。重複は自動で弾く。</div>
        </div>
    );
}
        ...
        <form action={formAction} className="grid gap-4">
            ...
            <div className="grid gap-2">
                ...
                <input
                    ...
                    multiple
                    onChange={(e) => {
                        const files = Array.from(e.currentTarget.files ?? []).slice(0, 12);
                        previews.forEach((u) => URL.revokeObjectURL(u));
                        setPreviews(files.map((f) => URL.createObjectURL(f)));
                    }}
                    className="block w-full rounded-md border border-zinc-200 px-3 py-2 text-sm font-semibold"
                />
                <div className="text-xs font-semibold text-zinc-500">最大12枚（重い場合は減らして）</div>
            </div>
