// app/products/page-v2.tsx
import { randomUUID } from "crypto";
import { Suspense } from "react";
import { supabaseServer } from "@/lib/supabase/server";
import ProductsPageClient from "./ProductsPageClient";
import TrendingTags from "@/components/products/TrendingTags";
...
export default async function ProductsPageV2({ searchParams }: { searchParams?: SP }) {
    ...
    let query = supabase
        ...
        .select(
            "id,title,brand,size,condition,price,cover_image_url,display_price,highest_bid_30d,sale_mode,is_auction_live,hot_score,shop_slug,shop_name_ja,shop_name_en,shop_avatar_url,shop_headline,tags,description,url,purchase_url"
        );
    ...
    const showSidebar = !q && !shop && !brand && !size && !condition && !tags;
                ...
                {showSidebar ? (
                    <div className="grid lg:grid-cols-[320px_1fr] gap-8">
                        ...
                        <aside className="space-y-6">
                            {/* Trending Tags */}
                            <Suspense fallback={<div className="h-96 rounded-3xl bg-white/50 animate-pulse" />}>
                                <TrendingTags limit={15} />
                            </Suspense>
                            ...
                            <div className="rounded-3xl border-2 border-slate-200 bg-white p-6 shadow-lg">
                                ...
                                <div className="space-y-3">
                                    <div className="flex items-center justify-between">
                                        <span className="text-xs font-semibold text-slate-600">Products Listed</span>
                                        <span className="text-sm font-black text-slate-900">{data?.length ?? 0}</span>
                                    </div>
                                    <div className="flex items-center justify-between">
                                        <span className="text-xs font-semibold text-slate-600">Live Auctions</span>
                                        <span className="text-sm font-black text-orange-600">
                                            {data?.filter((p: any) => p.is_auction_live).length ?? 0}
                                        </span>
                                    </div>
                                </div>
                            </div>
