...
import { COLOR_OPTIONS, uid } from "../_lib/constants";
...
export default function QuickAddWizard({ onSave, onClose, itemCount }: QuickAddWizardProps) {
    ...
    const goNext = useCallback(() => goTo(((step + 1) as Step), 1), [step, goTo]);
    const goBack = useCallback(() => goTo(((step - 1) as Step), -1), [step, goTo]);
    ...
    const handleCustomColorConfirm = useCallback(() => {
        ...
        setSelectedColor({ value: "custom", label: "カスタム", hex: customHex });
        setShowCustom(false);
        setDirection(1);
        setStep(3);
    }, [customHex]);
    ...
    const handleSave = useCallback(
        (openDetail: boolean) => {
            ...
            if (openDetail) {
                // Save happened; caller can navigate to edit view via item id
                onClose();
            }
        },
        [categoryMain, subcategory, selectedColor, name, autoName, season, formality, itemCount, onSave, onClose],
    );

    // Restart for another item
            ...
            {([1, 2, 3, 4] as Step[]).map((s) => (
                <motion.div
                    ...
                    animate={{
                        width: s === step ? 24 : 8,
                        height: 8,
                        backgroundColor: s === step ? "#7c3aed" : s < step ? "#a78bfa" : "#e2e8f0",
                    }}
    ...
    const renderStep2 = () => (
        <motion.div
            ...
            className="flex flex-col items-center"
        >
            <h2 className="text-xl font-bold text-slate-900 mb-1">カラー選択</h2>
            <p className="text-sm text-slate-500 mb-6">メインカラーをタップしてください</p>
                    ...
                    >
                        <span
                            ...
                            style={{
                                ...
                                borderColor:
                                    selectedColor?.value === c.value ? "#7c3aed" : c.value === "white" ? "#e2e8f0" : "transparent",
                            }}
                        />
            ...
            {!showCustom ? (
                <button
                    ...
                    className="mt-2 text-sm text-violet-600 font-medium hover:text-violet-800 transition-colors"
                >
                    + カスタムカラー
                </button>
            ) : (
                <div className="flex items-center gap-2 mt-2">
                    <input
                        ...
                        placeholder="#FF5500"
                        maxLength={7}
                        className="w-28 rounded-xl bg-white/80 border border-slate-200 px-3 py-2 text-sm text-slate-800 placeholder-slate-400 focus:outline-none focus:border-violet-400"
                    />
                    {/^#[0-9a-fA-F]{6}$/.test(customHex) && (
                        <span className="block w-8 h-8 rounded-full border border-slate-200" style={{ backgroundColor: customHex }} />
                    )}
                    <GlassButton size="sm" variant="primary" onClick={handleCustomColorConfirm} disabled={!/^#[0-9a-fA-F]{6}$/.test(customHex)}>
                        OK
                    </GlassButton>
                </div>
            )}
    ...
    const renderStep3 = () => (
        ...
        >
            ...
            <p className="text-sm text-slate-500 mb-6">名前と属性を設定</p>
        ...
        >
            {!saved ? (
                <>
                        ...
                        <div className="flex items-center gap-4">
                            ...
                            <div className="flex-1 min-w-0">
                                <div className="flex items-center gap-2 mb-1">
                                    <span className="text-xl">{categoryMain ? CATEGORY_ICONS[categoryMain] : ""}</span>
                    ...
                    <GlassButton variant="primary" fullWidth onClick={() => handleSave(false)}>
                        保存する
                    </GlassButton>

                    <button
                        onClick={() => handleSave(true)}
                        className="mt-3 text-sm text-violet-600 font-medium hover:text-violet-800 transition-colors"
                    >
                        もっと詳しく入力 →
                    </button>
                </>
    ...
    return (
        ...
        >
            ...
            >
                ...
                <div className="px-6 pt-14 pb-8 min-h-[420px] flex flex-col">
                    {renderStepDots()}
