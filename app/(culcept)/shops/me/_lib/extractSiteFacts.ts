...
function pickSiteFromJsonLd(list: any[], baseUrl: string) {
    ...
    const addressText = cleanStr(
        ws?.address?.streetAddress ??
        ws?.address?.addressLocality ??
        ws?.location?.address?.streetAddress ??
        ws?.location?.name ??
        null
    );

    return { name, url, logo, description, addressText };
}
