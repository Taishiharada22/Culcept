            ...
            {rows.length > 0 ? (
                <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                    {rows.map((d: any, idx: number) => (
                        <div
                            key={d.id}
                            className="rounded-2xl overflow-hidden bg-white/5 backdrop-blur-sm border border-white/10 hover:border-white/20 hover:bg-white/10 transition-all"
                        >
                            <DropCard
                                d={d}
                                imp={imp}
                ...
                <div className="rounded-3xl border border-white/10 bg-white/5 backdrop-blur-sm p-16 text-center">
                    ...
                    <p className="text-white/60 mb-6">Try adjusting your search or browse all products</p>
                    <Link
                        href={addQuery("/drops", { imp })}
                        className="inline-block px-6 py-3 bg-gradient-to-r from-orange-500 to-pink-500 rounded-full text-sm font-black shadow-lg shadow-orange-500/30 hover:shadow-xl transition-all"
                    >
                        View All Products
                    </Link>
                </div>
