...
import { AuctionModePageShell } from "@/app/drops/_components/AuctionModePageShell";
...
export default async function EditDropPage({
    ...
    const [{ data: images, error: iErr }, { data: fitProfile }, { data: bodyProfile }, { data: measurements }] =
        await Promise.all([
            supabase
                .from("drop_images")
            ...
            supabase.from("garment_fit_profiles").select("*").eq("product_id", dropId).maybeSingle(),
    ...
    const userFootReference = extractStoredBodyFootReference({
        measurements: measurements?.[0]?.measurements ?? null,
        displayLabels: bodyProfile?.display_labels ?? null,
    });

    return (
        <AuctionModePageShell initialSaleMode={defaults.sale_mode}>
                    ...
                    <section className="rounded-xl border border-zinc-200 bg-white p-5 shadow-sm">
                        ...
                        <div className="mt-5">
                            <DropMetaForm
                                dropId={dropId}
                                defaults={defaults}
                                initialFitProfile={fitProfile ?? null}
