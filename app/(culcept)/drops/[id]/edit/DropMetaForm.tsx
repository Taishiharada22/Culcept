...
import { useAuctionModePage } from "@/app/drops/_components/AuctionModePageShell";
...
export default function DropMetaForm({
    ...
    initialFitProfile?: GarmentFitProfile | null;
    ...
    const [fitValues, setFitValues] = React.useState(() => buildInitialFitValues(initialFitProfile ?? null));
    const [categoryMain, setCategoryMain] = React.useState(() => fitValues.style_category_main ?? "");
    const [subcategoryId, setSubcategoryId] = React.useState(() => fitValues.style_subcategory_id ?? "");
    const { setSaleMode: setPageSaleMode } = useAuctionModePage();

    React.useEffect(() => {
        setPageSaleMode(saleMode);
    }, [saleMode, setPageSaleMode]);
    ...
    return (
        <form action={formAction} className="grid gap-4">
            ...
            {/* Auction fields */}
            {saleMode === "auction" ? (
                <div className="grid gap-3 rounded-xl border border-violet-200 bg-violet-50/80 p-4 md:grid-cols-2">
                    <div className="grid gap-2">
            ...
            <div className="rounded-xl border border-zinc-200 bg-zinc-50 p-4">
                <div className="mb-4">
                    <div className="text-sm font-extrabold text-zinc-900">Style</div>
                    <div className="text-xs font-semibold text-zinc-500">
                        アイテムカテゴリを選ぶと、下の体型フィット項目がカテゴリ別に切り替わります。
                    </div>
                </div>
            ...
            <div className="rounded-xl border border-zinc-200 bg-zinc-50 p-4">
                <div className="mb-4">
                    <div className="text-sm font-extrabold text-zinc-900">Fit</div>
                    <div className="text-xs font-semibold text-zinc-500">
                        新規出品と同じ定義を使い、edit では1ページ内に集約表示します。
                    </div>
                </div>
                <FitProfileEditor
                    categoryMain={categoryMain}
