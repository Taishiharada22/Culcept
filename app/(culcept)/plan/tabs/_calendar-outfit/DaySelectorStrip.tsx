...
/** diary ドットの色（進行度で濃く。 ラベンダー/violet 系のみ・極小） */
const DIARY_DOT_COLOR: Record<Exclude<DiaryDayStatus, "none">, string> = {
  selected: "bg-violet-200",
  worn: "bg-violet-400",
  rated: "bg-violet-600",
};
/** スクリーンリーダ向けの状態語 */
const DIARY_LABEL: Record<Exclude<DiaryDayStatus, "none">, string> = {
  selected: "選択済み",
  worn: "着用済み",
  rated: "評価済み",
};
  ...
  return (
    ...
    >
      {days.map((d) => {
        ...
        const status: DiaryDayStatus | undefined = diaryStatus[d.iso];
        const weekdayColor =
        ...
        return (
          ...
          >
            ...
            {/* diary ドット用の固定スロット（none でも高さ確保＝レイアウトシフトなし） */}
            <span className="mt-0.5 flex h-1.5 items-center justify-center" aria-hidden="true">
              {status && status !== "none" && (
                <span className={`h-1.5 w-1.5 rounded-full ${DIARY_DOT_COLOR[status]}`} />
              )}
            </span>
            {status && status !== "none" && <span className="sr-only">{DIARY_LABEL[status]}</span>}
          </button>
        );
      })}
    </div>
