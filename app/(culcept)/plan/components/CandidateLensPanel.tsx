...
export function CandidateLensPanel({ candidates, title, gapMinutes, affinityReasonFor, onSelect, onSkip }: CandidateLensPanelProps) {
  ...
  // ════════════════════ ② 詳細モード（候補領域全体が詳細に切替・見出しを畳む・どこで？直下から大きく） ════════════════════
  if (view === "detail") {
    return (
      <div data-testid="lens-detail" className="mt-3 rounded-[20px] bg-white p-4 shadow-[0_1px_2px_rgba(0,0,0,0.04),0_18px_48px_rgba(124,58,237,0.12)] ring-1 ring-purple-200">
        <div className="flex items-start justify-between gap-2">
          <div className="min-w-0">
            <h3 className="text-[19px] font-semibold leading-snug tracking-tight text-slate-900">{v.name}</h3>
        ...
        {/* 大メディア領域（主役・place tile ＋ 簡易 map・横長で大きく） */}
        <div className="mt-3 flex gap-2">
          <PlaceTile category={v.category} h="h-[8.5rem]" w="w-[40%]" />
          <MapTile h="h-[8.5rem]" w="flex-1" />
        </div>

        <p className="mt-3 flex items-start gap-1.5 text-[13.5px] leading-relaxed text-slate-600"><span aria-hidden className="mt-px">📍</span><span className="min-w-0">{v.address ?? "住所は未取得です"}</span></p>
        <ChipRow view={v} withEvidence className="mt-3" />

        {v.whyLine && (
          <div className="mt-4 rounded-2xl bg-purple-50/70 px-4 py-4">
            <p className="text-[12.5px] font-semibold text-purple-700">なぜここを選ぶ？</p>
            <p className="mt-2 text-[14.5px] leading-relaxed text-slate-700">{v.whyLine}</p>
            <p className="mt-2 text-[12.5px] leading-relaxed text-slate-500">{PURPOSE_LENS_LABEL[lens]}の目的に照らすと、距離と接続のしやすさからこの候補が向いています。</p>
          </div>
        )}
            ...
            <div className="flex-1 overflow-y-auto px-4 py-3">
              ...
              <div className="flex items-stretch gap-2">
                <button type="button" onClick={() => confirmSide("left")} data-testid="lens-compare-left" className={headCls("left")}>
                  <div className="flex items-center gap-2"><PlaceTile category={left.category} h="h-9" w="w-9" /><span className="min-w-0"><span className="block truncate text-[13px] font-semibold text-slate-900">{left.name}</span><span className="text-[10px] text-slate-400">{left.category}{left.affinityBadge && " · 相性"}</span></span></div>
                </button>
                <div className="flex min-w-0 flex-1 items-stretch gap-1"
                  ...
                  <button type="button" onClick={() => confirmSide("right")} data-testid="lens-compare-right" className={headCls("right")}>
                    <div className="flex items-center gap-2"><PlaceTile category={right.category} h="h-9" w="w-9" /><span className="min-w-0"><span className="block truncate text-[13px] font-semibold text-slate-900">{right.name}</span><span className="text-[10px] text-slate-400">{right.category}{right.affinityBadge && " · 相性"}</span></span></div>
                  </button>
              ...
              {/* ★比較表（主役・最大面積） */}
              <p className="mb-1.5 mt-4 px-0.5 text-[13px] font-semibold text-slate-800">候補を比較</p>
              <div className="overflow-hidden rounded-2xl bg-white shadow-[0_1px_2px_rgba(0,0,0,0.04),0_10px_30px_rgba(0,0,0,0.07)] ring-1 ring-black/5">
                <div className="grid grid-cols-[4.5rem_1fr_1fr] bg-slate-50 px-3 py-2 text-[11px] font-semibold text-slate-500"><span>項目</span><span className="truncate">{left.name}</span><span className="truncate">{right.name}</span></div>
                <div>
                  {comp.mainRows.map((row, idx) => (
                    <div key={row.key} data-testid="lens-row" data-key={row.key} className={`grid min-h-[56px] grid-cols-[4.5rem_1fr_1fr] items-stretch gap-px ${idx % 2 ? "bg-slate-50/40" : "bg-white"}`}>
                      <span className="flex items-center gap-1 px-3 py-2 text-[12px] text-slate-500"><span aria-hidden className="text-[14px]">{ATTR_ICON[row.key]}</span>{row.label}</span>
              ...
              {comp.unconfirmedLabels.length > 0 && (
                <p data-testid="lens-unconfirmed" className="mt-2 px-1 text-[11px] leading-relaxed text-slate-400">未確認: {comp.unconfirmedLabels.join("・")}（このデータはまだ持っていません）</p>
              )}
              ...
              {comp.recommendation && (
                <div data-testid="lens-recommendation" className="mt-3 rounded-2xl bg-gradient-to-br from-purple-50 to-indigo-50 px-4 py-4 ring-1 ring-purple-100">
                  ...
                  <p className="mt-1 text-[15px] font-semibold leading-relaxed text-slate-900">{comp.recommendation.side === "left" ? left.name : right.name}</p>
                  <p className="mt-0.5 text-[13px] leading-relaxed text-slate-600">{comp.recommendation.basisPhrase}</p>
                </div>
              )}
            </div>
