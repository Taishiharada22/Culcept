...
// Concept 14 — Spots。単体スポットを 地元民のスポット / 旅行者の発見 で並べ、エリアから探す。
"use client";
...
export function SpotView({ data, savedIds, onToggleSave, onAddToItinerary, onGoToAdd }: LocationViewProps) {
  ...
  const rest = spots.filter((i) => i.id !== hero.id);
    ...
    <div className="space-y-6">
      ...
      {localSpots.length > 0 && (
        <section>
          ...
          <Grid2>
            {localSpots.map((it) => <SpotGridCard key={it.id} item={it} saved={savedIds.has(it.id)} onToggleSave={() => onToggleSave(it.id)} />)}
          </Grid2>
        </section>
      )}
      {travelerSpots.length > 0 && (
        <section>
          ...
          <Grid2>
            {travelerSpots.map((it) => <SpotGridCard key={it.id} item={it} saved={savedIds.has(it.id)} onToggleSave={() => onToggleSave(it.id)} />)}
          </Grid2>
        </section>
      )}
      ...
      <section>
        ...
        <div className="flex flex-wrap gap-2">
          {data.areaChips.map((a) => (
            <button key={a.label} className="inline-flex items-center gap-1.5 rounded-full border px-3 py-1.5 text-[12px]" style={{ borderColor: T.border, background: T.card, color: T.ink2 }}>
              <span className="font-medium" style={{ color: T.ink }}>{a.label}</span>
