...
place スロットに話題・人名を入れないこと（CEO 方針 2026-04-18 Bug 7）:
place は「物理的な場所」のみ。人名・話題・動作は入れない。
- 誤: place="仙洞田さんの名字の由来"（人名+話題が place に混入）
- 正: companions=["仙洞田さん"], activity="話す"（話題は activity ヒントに。place は null）
- 誤: place="田中さんと打ち合わせ"（人名+活動）
- 正: companions=["田中さん"], activity="打ち合わせ", place=null
- 誤: place="名字の由来について"
- 正: activity="話す", place=null
判別ヒント:
  - 「〜について」「〜の話」「〜の件」「〜の由来」「〜の理由」「〜の気持ち」→ 話題。place ではない
  - 「〜さんと」「〜さんの」で始まり場所を指す単語（店/駅/公園/...）で終わらない → 人名。place ではない
  - 「話す」「相談」「報告」「確認」「伝える」「聞く」で終わる → 動作文。place ではない
  - 物理的に「そこに行ける」場所でなければ place に入れない`;
...
/**
 * place フィールドに人名・話題・動作文が混入した場合を検出して分離する。
 *
 * 実機事故（CEO実機検証 2026-04-18）:
 *   入力: 「仙洞田さんと名字の由来について話す」
 *   誤抽出: place="仙洞田さんの名字の由来"
 *   正抽出: place=null, companions=["仙洞田さん"], activity（話題 hint 付き）
 *
 * 検出パターン（place として妥当でない形態）:
 *   A. 敬称 + 話題接続助詞: 「〜さん(の|と|が)...」
 *   B. 話題マーカー: 「について」「の話」「の件」「の由来」「の理由」「の原因」
 *   C. 動作文: 「話す」「相談」「報告」「お願い」「確認」「伝える」「聞く」
 *   D. 名詞化された抽象名詞: 「由来」「理由」「原因」「背景」「事情」「気持ち」
 *
 * Action:
 *   - place を null に戻す（topic/人名は place ではない）
 *   - 検出した人名（「〜さん」等）があれば companions に追加
 *   - 取り除いた topic 部分は placeSearchHint.originalQuery に残す（デバッグ用）
 *     ※ 再解決用のヒントとして保持し、誤って search に投げないよう searchCategory は立てない
 */
const PERSON_HONORIFIC_RE = /([一-龥ぁ-んァ-ヴー々〆ヵヶ・]{1,8})(さん|様|君|くん|ちゃん|殿|氏)(?:の|と|が|に|は)/;
const TOPIC_MARKER_RE = /(について|の話(?:し)?|の件|の由来|の理由|の原因|の背景|の事情|の気持ち|の意見|の件で)/;
const ACTION_VERB_RE = /(話す|相談|報告|お願い|確認|伝える|聞く|質問|連絡|議論|打ち合わ)/;
const ABSTRACT_NOUN_RE = /^(.+?)(の)?(由来|理由|原因|背景|事情|気持ち|意見|話|件)$/;

/**
 * place が「場所らしくない」と判定されるか。
 *
 * 注意: 店名・施設名に敬称が含まれるケース（例:「田中さんの店」）は誤検出リスクがある。
 * そのため「人名+敬称+話題接続」+「話題マーカー or 動作動詞」の両方がある場合のみ落とす。
 */
function detectNonPlaceContent(rawPlace: string): {
  isNonPlace: boolean;
  extractedPerson?: string;
  reason?: "person_with_topic" | "topic_marker" | "action_verb" | "abstract_noun";
} {
  const trimmed = rawPlace.trim();

  const personMatch = trimmed.match(PERSON_HONORIFIC_RE);
  const hasTopicMarker = TOPIC_MARKER_RE.test(trimmed);
  const hasActionVerb = ACTION_VERB_RE.test(trimmed);

  // (A) 人名+敬称 + 話題/動作 → 高確度で非 place
  if (personMatch && (hasTopicMarker || hasActionVerb)) {
    const name = personMatch[1] + personMatch[2]; // 「仙洞田さん」
    return { isNonPlace: true, extractedPerson: name, reason: "person_with_topic" };
  }

  // (B) 話題マーカー単独（人名なくても「〜について」「〜の由来」等は話題）
  if (hasTopicMarker) {
    return { isNonPlace: true, reason: "topic_marker" };
  }

  // (C) 動作動詞で終わる（「〜を話す」「〜を相談」等）
  //     ※「〜店」「〜駅」等の場所で止まる形は誤検出しない（末尾のみ判定）
  if (hasActionVerb && /(話す|相談|報告|お願い|確認|伝える|聞く|質問|連絡|議論|打ち合わ)(?:せ)?(?:する)?$/.test(trimmed)) {
    return { isNonPlace: true, reason: "action_verb" };
  }

  // (D) 抽象名詞で終わる（「〜の由来」「〜の理由」等）
  //     PERSON_HONORIFIC の有無に関わらず、形態的に明らかに場所でない
  const absMatch = trimmed.match(ABSTRACT_NOUN_RE);
  if (absMatch) {
    return { isNonPlace: true, reason: "abstract_noun" };
  }

  return { isNonPlace: false };
}

/**
 * scrubNonPlaceContent — Bug 7 post-filter。
 *
 * 入力: raw place / existing companions
 * 出力: { place, addedCompanions } —
 *   - isNonPlace なら place = null、検出人名があれば addedCompanions に追加
 *   - そうでなければ place をそのまま返す
 */
function scrubNonPlaceContent(
  ...
  if (!rawPlace) return { place: null, addedCompanions: [] };
  const trimmed = rawPlace.trim();
  if (!trimmed) return { place: null, addedCompanions: [] };

  const detection = detectNonPlaceContent(trimmed);
  if (!detection.isNonPlace) {
    return { place: trimmed, addedCompanions: [] };
  }

  const added: string[] = [];
  if (detection.extractedPerson) {
    const already = existingCompanions.some(
      c => c.trim() === detection.extractedPerson || c.includes(detection.extractedPerson!)
    );
    if (!already) added.push(detection.extractedPerson);
  }

  return {
    place: null,
    addedCompanions: added,
...
function normalizeSegment(
  ...
  // CEO方針 2026-04-18 Bug 7 安全弁: 話題/人名混入を place から分離
  //   scrubQuestionPlace の後段で実施（疑問文パターンでないケースのみ対象）
  //   - 検出: place=null、検出人名を companions に追加
