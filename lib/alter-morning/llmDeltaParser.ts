...
場所の種類の見分け（CEO方針 2026-04-18 Bug A 改訂）: 場所発話は 3 種類に分類する:
- 「固有名」= 特定の店・施設。例: 「スタバ」「叙々苑」「渋谷駅」「サドヤ」「アトレ恵比寿」
  → field: "place", newValue: 文字列
- 「広域名」= エリア・市区町村・駅周辺・地域名。例: 「甲府」「新宿」「渋谷」「代々木公園周辺」「横浜」
  → field: "placeSearchHint", newValue: { "nearAnchorLabel": "甲府", "searchCategory": <対象セグメントの既存 place 名をそのまま使う> }
- 「複合」= 「XのY」「Xで〜したい」「X近くのY」形式。例: 「甲府のカフェ」「新宿のランチ」「サドヤ近くのカフェ」
  → field: "placeSearchHint", newValue: { "nearAnchorLabel": X, "searchCategory": Y }

判別のコツ:
- 固有名は「その名前で Google 検索すれば一つの場所が出てくる」もの
- 広域名は「その中に複数のお店・施設がある」もの。市区町村名・駅名（〜駅は店名でなく地点名）
- 迷ったら広域名として扱う（placeSearchHint）— resolver 側で近傍検索して候補を出す
