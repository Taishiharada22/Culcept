...
function isEngagedShortMessage(content: string): boolean {
  ...
  // ── fragment continuation: 話題の具体化・補足は離脱ではない ──
  // 「人間と」「人間とAI」のように短い補足で前の話題を具体化しているパターン。
  // 前のAlter質問への回答の一部（断片的に入力）として認識する。
  // 特徴: 名詞・名詞句で終わる短文、助詞で終わる短文（「〜と」「〜の」「〜が」）
  if (trimmed.length <= 20) {
    // 助詞で終わる（入力途中・補足の兆候）: 「人間と」「仕事の」「友達が」
    if (/[がはをにでとものへや]$/.test(trimmed)) return true;
    // 名詞で終わる短い回答（「起業」「仕事」「人間とAI」等）— Alterの質問への直接回答
    if (/^[\u4e00-\u9fafぁ-んァ-ヶA-Za-z\s]{1,15}$/.test(trimmed) && !/^(うん|そう|まあ|別に|いいよ|はい|ふーん)$/.test(trimmed)) return true;
  }
  // ask_me質問への回答パターン: 選択肢から1つ選んだ回答
  // 「仕事かな」「後者」「前者」「Bかな」「体調」
  if (/^(?:前者|後者|[ABCabc]|[①②③])[かだ]?(?:な|かな|ね)?[。！!]?$/.test(trimmed)) return true;
  if (trimmed.length <= 10 && /かな[。]?$/.test(trimmed)) return true;
  // 命令・要求（「教えて」「聞いて」「答えて」等）
