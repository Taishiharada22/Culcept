...
// dayFelt のアンカー語（NIGHT_CHECK_CHIPS と同義。Morning Reveal の actualAnchor 用・C-4）
export const DAYFELT_ANCHOR: Record<DayFelt, string> = {
  5: "かなり余った",
  4: "少し余った",
  3: "ちょうど",
  2: "足りなかった",
...
// followup（設計 §5.2）: 主問の後、予定があった日のみ。planVerdict を取る
const NIGHT_CHECK_FOLLOWUP_QUESTION = "予定は、見立て通りに運びましたか？";
export const NIGHT_CHECK_FOLLOWUP_CHIPS = ["だいたい通り", "一部ずれた", "大きくずれた"];
...
function buildMorningReveal(
  ...
  if (energyVerdict !== undefined) {
    items.push({
      ...
      actualAnchor: DAYFELT_ANCHOR[nc.dayFelt], // C-4: 「少し余った」等の具体語（設計 §3.5'）
...
function nightCheckState(
  ...
  if (record.nightCheck) {
    // C-3: 主問に回答済みで、予定があった日（anchorCount>0）かつ followup 未回答 → followup。
    //      予定が無い日 or planVerdict 回答済み → answered。
    const followupPending = record.nightCheck.planVerdict === undefined && record.facts.anchorCount > 0;
    return followupPending
      ? { state: "followup", question: NIGHT_CHECK_FOLLOWUP_QUESTION, chips: NIGHT_CHECK_FOLLOWUP_CHIPS }
      : { state: "answered", question: NIGHT_CHECK_QUESTION, chips: NIGHT_CHECK_CHIPS };
  }
