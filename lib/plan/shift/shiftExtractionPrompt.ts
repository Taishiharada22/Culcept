...
export function buildDayKeyedExtractionPrompt(
  ...
  const lines: string[] = [
    ...
    "",
    `# 読めないセルの扱い（最重要・絶対厳守）`,
    `- セルに**何か書かれているが判読できない**（かすれ・重なり・潰れ等で記号を確定できない）場合も、rawCode は "" にしますが、その日の **confidence を 0.3 以下**に必ず下げてください。`,
    `- **確実に何も書かれていない**と判断できる空セルだけ、confidence を高め（0.8 以上）にして "" を出してください。`,
    `- つまり "" は「空欄」を表しますが、**確実に空なのか / 読めなくて空に見えるのか**は confidence で区別します（高い = 確実に空 / 低い = 読めない）。`,
    `- 読めないセルを当て推量で適当なコードに**しないでください**（"" + 低 confidence のほうが安全です）。`,
  ];
