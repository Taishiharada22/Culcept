...
const SYSTEM_PROMPT_BASE_V2 = [
  ...
  "",
  // v3.4.2 (= CEO 2026-05-25 v3.4.1 後 補正): 同 prompt 内で variation 選択
  "**重要 — 表現の多様性 (= 1 prompt 内でも複数候補から選ぶ)**:",
  "  - 同じ profile + 同じ anchor でも、 **自然な観測の言い回しは複数存在** します。",
  "    例: 「集中型 user + 夜カフェ + 読書」 anchor の自然観測 (= 候補から 1 つ選ぶ):",
  "      ✓ 「夜のカフェなら、 ひとりで没頭しやすそう」",
  "      ✓ 「夕方のカフェで、 自分の世界に潜れそうです」",
  "      ✓ 「カフェで過ごす夜、 内側に向かう静けさが宿る」",
  "      ✓ 「夜カフェだと、 ページに沈み込めそうですね」",
  "      ✓ 「ひととき、 一人で深く読み進められそう」",
  "  - **同じ言い回しを毎回繰り返さない** (= 文末 / 動詞 / 名詞 を変える)。",
  "  - 「思考を潜らせる」 「集中する」 等の **同じ表現に固定** しない (= LLM 自身の自由度を生かす)。",
  "  - anchor の具体内容 (= 「読書」) と profile 動詞 (= 「沈む / 没頭」) を組み合わせる **角度** は",
  "    毎回違ってよい (= 場所視点 / 時間視点 / 動作視点 / 質感視点 / ひとときの感覚 等)。",
].join("\n");
...
export function buildUserPromptV2(ctx: AlterNoteContext): string {
  ...
  // v3.4.2: 日付コンテキスト (= 月曜の朝 / 土曜の夜 等の自然な文脈、 cache 多様化)
