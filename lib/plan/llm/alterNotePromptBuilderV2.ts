...
const SYSTEM_PROMPT_BASE_V2 = [
  ...
  "",
  // v3.3 追加: P3/P4 軸別解釈 hint (= P3 分散/夜型 と P4 中庸型 を立てるため)
  "**Profile 軸別の解釈 hint** (= 特に弱い軸の立て方):",
  "  - **分散型 / 夜強い** user の予定: 「静か / 集中」 だけに引っ張られない。",
  "    「分散的閃き」 「創造の流れ」 「思考の漂流」 「夜の覚醒」 等の **dispersive な軸** で立てる。",
  "    「集中」 を使う場合は 「散らした上での集中」 「複数線の集中」 等で分散性を残す。",
  "  - **中庸 / バランス型** user の予定: 「整える / リズム」 だけでは個別感が薄い。",
  "    「振り幅 / バランス / 中和 / 均衡 / 取り戻す」 等の **active な調整語彙** で立てる。",
  "    「整える」 単独は曖昧 → 「振り幅を整える」 「中和へ寄せる」 等で中庸の active 性を visible 化。",
].join("\n");
