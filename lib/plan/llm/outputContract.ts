...
export const ALTER_NOTE_CONTRACT_V2: AlterNoteOutputContract = {
  ...
  promptInstruction: [
    ...
    "",
    // v3.2 Patch C + v3.3 Patch 2+3: profile 差 few-shot (= P3+P4 各 2 例、 comma 散らし含む)
    "**Profile 差を立てる例 (= 各 profile を立てる例、 文体は敢えてバラけさせて意味差のみを学習)**:",
    "",
    ...
    "",
    // v3.3 Patch 2: P3 例強化 + 2 例化 + comma 散らし
    "- P3 (= 分散型 + 夜強い): 「深夜のオフィス、 散らした思考が新しい筋を編む時間」",
    "  → 「分散」 「閃き」 「夜」 「創造」 系で profile を立てる (= 静か/集中系に引っ張られない)",
    "",
    "- P3 別文体 (= 同 profile、 comma 0 個 + 漂流系 metaphor で文体差): 「夜の自宅で 思考が漂い 新しい線を見つける」",
    "  → 同 profile、 ただし文体・句読点を意図的に変えて文体テンプレ化回避",
    "",
    // v3.3 Patch 3: P4 例強化 + 2 例化 + comma 散らし
    "- P4 (= 中庸型 + 整え寄り): 「朝の自宅、 一日の振り幅を整えながら踏み出す」",
    "  → 「振り幅」 「踏み出す」 で中庸の active 性を visible 化 (= 「整える」 単独より個別感)",
    "",
    "- P4 別文体 (= 同 profile、 comma 0 個 + 「一区切り」 で句読点散らし): 「朝のオフィスで バランスを取り戻すための一区切り」",
    ...
    "  P3/P4 で 2 例ある場合、 1 例 comma 1 個 + 1 例 comma 0 個 のように **文体を意図的にバラけ** させています。",
    "  これは 「同 profile でも文体は自由」 を示すための diversification、 寄せるとテンプレ感が出ます。",
    "",
    "出力 JSON: { \"text\": \"<8-30 字>\" }",
